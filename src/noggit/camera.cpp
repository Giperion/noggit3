...
namespace noggit
{
	camera::camera(math::vector_3d const& position, math::degrees yaw_,
				   math::degrees pitch_)
		: position(position),
		  move_speed(200.6f),
		  _roll(0.0f),
	...
	math::degrees camera::yaw() const { return _yaw; }
	...
	math::degrees camera::yaw(math::degrees value)
	{
		...
		//! [-180, 180)
		while (_yaw._ >= 180.0f) _yaw._ -= 360.0f;
		while (_yaw._ < -180.0f) _yaw._ += 360.0f;
	...
	math::degrees camera::pitch() const { return _pitch; }
	...
	math::radians camera::fov() const { return _fov; }

	math::vector_3d camera::look_at() const { return position + direction(); }
