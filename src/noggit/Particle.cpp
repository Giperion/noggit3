...
ParticleSystem::ParticleSystem(Model* model_, const MPQFile& f,
							   const ModelParticleEmitterDef& mta, int* globals)
	: model(model_),
	  emitter_type(mta.EmitterType),
	  emitter(
		  mta.EmitterType == 1
			  ? std::unique_ptr<ParticleEmitter>(
					std::make_unique<PlaneParticleEmitter>())
			  : mta.EmitterType == 2
					? std::unique_ptr<ParticleEmitter>(
						  std::make_unique<SphereParticleEmitter>())
					: throw std::logic_error("unimplemented emitter type: " +
											 std::to_string(mta.EmitterType))),
	  ...
	  tofs(misc::frand())
{
	...
	memcpy(colors2, f.getBuffer() + mta.p.colors.ofsKeys,
		   sizeof(math::vector_3d) * 3);
	for (size_t i = 0; i < 3; ++i)
	{
		float opacity = *reinterpret_cast<int16_t const*>(
			f.getBuffer() + mta.p.opacity.ofsKeys + i * 2);
		colors[i] =
			math::vector_4d(colors2[i].x / 255.0f, colors2[i].y / 255.0f,
							colors2[i].z / 255.0f, opacity / 32767.0f);
		sizes[i] = (*reinterpret_cast<float const*>(
					   f.getBuffer() + mta.p.sizes.ofsKeys + i * 4)) *
				   mta.p.scales[i];
	}
...
void ParticleSystem::update(float dt)
{
	...
	if (emitter)
	{
		...
		else
		{
			...
			if ((tospawn + particles.size()) >
				MAX_PARTICLES)  // Error check to prevent the program from
								// trying to load insane amounts of particles.
			...
			if (en)
			{
				...
				{
					Particle p =
						emitter->newParticle(this, manim, mtime, manimtime, w,
											 l, spd, var, spr, spr2);
					// sanity check:
					// if (particles.size() < MAX_PARTICLES) // No need to check
					// this every loop iteration. Already checked above.
	...
	for (ParticleList::iterator it = particles.begin(); it != particles.end();)
	{
		...
		p.color = lifeRamp<math::vector_4d>(rlife, mid, colors[0], colors[1],
											colors[2]);
...
void ParticleSystem::draw(math::matrix_4x4 const& model_view,
						  ...
						  GLuint const& transform_vbo, int instances_count)
{
	if (!_uploaded)
	{
		upload();
	 ...
	 * type:
	 * 0   "normal" particle
	 * 1  large quad from the particle's origin to its position (used in
	 * Moonwell water effects) 2  seems to be the same as 0 (found some in the
	 * Deeprun Tram blinky-lights-sign thing)
	 */
	if (type == 0 || type == 2)
	{
		...
		if (billboard)
		{
			...
			{
				if (tiles.size() - 1 <
					it->tile)  // Alfred, 2009.08.07, error prevent
				{
					break;
				}
	...
	gl.bufferData<GL_ARRAY_BUFFER, math::vector_3d>(_vertices_vbo, vertices,
													GL_STREAM_DRAW);
	gl.bufferData<GL_ARRAY_BUFFER, math::vector_4d>(_colors_vbo, colors_data,
													GL_STREAM_DRAW);
	gl.bufferData<GL_ARRAY_BUFFER, math::vector_2d>(_texcoord_vbo, texcoords,
													GL_STREAM_DRAW);
	gl.bufferData<GL_ELEMENT_ARRAY_BUFFER, std::uint16_t>(_indices_vbo, indices,
														  GL_STREAM_DRAW);
	...
	{
		opengl::scoped::buffer_binder<GL_ARRAY_BUFFER> const vertices_binder(
			_vertices_vbo);
		shader.attrib("position", 3, GL_FLOAT, GL_FALSE, 0, 0);
	}
	if (billboard)
	{
		gl.bufferData<GL_ARRAY_BUFFER, math::vector_3d>(_offsets_vbo, offsets,
														GL_STREAM_DRAW);

		opengl::scoped::buffer_binder<GL_ARRAY_BUFFER> const offset_binder(
			_offsets_vbo);
		shader.attrib("offset", 3, GL_FLOAT, GL_FALSE, 0, 0);
	}
	{
		opengl::scoped::buffer_binder<GL_ARRAY_BUFFER> const texcoord_binder(
			_texcoord_vbo);
		shader.attrib("uv", 2, GL_FLOAT, GL_FALSE, 0, 0);
	}
	{
		opengl::scoped::buffer_binder<GL_ARRAY_BUFFER> const colors_binder(
			_colors_vbo);
	...
	opengl::scoped::buffer_binder<GL_ELEMENT_ARRAY_BUFFER> const indices_binder(
		_indices_vbo);
	gl.drawElementsInstanced(GL_TRIANGLES, indices.size(), GL_UNSIGNED_SHORT,
							 nullptr, instances_count);
}
...
namespace
{
	...
	math::matrix_4x4 CalcSpreadMatrix(float Spread1, float Spread2, float w,
									  float l)
	{
		int i, j;
		float a[2], c[2], s[2];
		...
		for (i = 0; i < 3; ++i)
			for (j = 0; j < 3; j++) SpreadMat(i, j, SpreadMat(i, j) * Size);

		return SpreadMat;
	}
}  // namespace

Particle PlaneParticleEmitter::newParticle(ParticleSystem* sys, int anim,
										   int time, int animtime, float w,
										   float l, float spd, float var,
										   float spr, float /*spr2*/)
{
	...
	// 0x1  / 1    = Pretty much everything I know of except Basilisks have this
	// flag..  Billboard? 0x2  / 2    = 0x4  / 4    = 0x8  / 8    = 0x10  / 16
	// = Position Relative to bone pivot? 0x20  / 32  = 0x40  / 64  = 0x80 / 128
	// = 0x100 / 256  = 0x200 / 512  = 0x400 / 1024 = 0x800 / 2048 = 0x1000/
	// 4096 = 0x0000/ 1593 = [1,8,16,32,512,1024]"Warp Storm" - aura type
	// particle effect 0x419 / 1049 = [1,8,16,1024] Forest Wind shoulders 0x411
	// / 1041 = [1,16,1024] Halo 0x000 / 541  = [1,4,8,16,512] Staff glow 0x000
	// / 537 = "Warp Storm" 0x31 / 49 = [1,16,32] particle moving up? 0x00 / 41
	// = [1,8,32] Blood elf broom, dust spread out on the ground (X, Z axis)
	// 0x1D / 29 = [1,4,8,16] particle being static
	// 0x19 / 25 = [1,8,16] flame on weapon - move up/along the weapon
	// 17 = 0x11 = [1,16] glow on weapon - static, random direction.  -
	// Aurastone Hammer 1 = 0x1 = perdition blade 4121 = water ele 4097 = water
	// elemental 1041 = Transcendance Halo 1039 = water ele
	...
	{  // Trans Halo
		p.pos = sys->parent->mat *
				(sys->pos + math::vector_3d(misc::randfloat(-l, l), 0,
											misc::randfloat(-w, w)));

		const float t =
			misc::randfloat(0.0f, 2.0f * (float)math::constants::pi);

		p.pos =
			math::vector_3d(0.0f, sys->pos.y + 0.15f, sys->pos.z) +
			math::vector_3d(
				cos(t) / 8, 0.0f,
				sin(t) / 8);  // Need to manually correct for the halo - why?

		// var isn't being used, which is set to 1.0f,  whats the importance of
		// this? why does this set of values differ from other particles
	...
	{  // Weapon Flame
		p.pos = sys->parent->pivot +
				(sys->pos + math::vector_3d(misc::randfloat(-l, l),
											misc::randfloat(-l, l),
											misc::randfloat(-w, w)));
		math::vector_3d dir = mrot * math::vector_3d(0.0f, 1.0f, 0.0f);
		p.dir = dir.normalize();
		// math::vector_3d dir = sys->model->bones[sys->parent->parent].mrot *
		// sys->parent->mrot * math::vector_3d(0.0f, 1.0f, 0.0f); p.speed =
		// dir.normalize() * spd;
	}
	...
	{  // Weapon with built-in Flame (Avenger lightsaber!)
		p.pos = sys->parent->mat *
				(sys->pos + math::vector_3d(misc::randfloat(-l, l),
											...
											misc::randfloat(-w, w)));
		math::vector_3d dir =
			math::vector_3d(sys->parent->mat(1, 0), sys->parent->mat(1, 1),
							sys->parent->mat(1, 2)) +
	...
	{
		p.pos = sys->pos + math::vector_3d(misc::randfloat(-l, l), 0,
										   misc::randfloat(-w, w));
...
Particle SphereParticleEmitter::newParticle(ParticleSystem* sys, int anim,
											int time, int animtime, float w,
											float l, float spd, float var,
											float spr, float spr2)
{
	...
	math::radians t(0);
	if (spr == 0)
		t._ = misc::randfloat((float)-math::constants::pi,
							  (float)math::constants::pi);
	else
	...
	*/
	...
	{  // Faith Halo
		math::vector_3d bdir(w * math::cos(t) * 1.6f, 0.0f,
							 l * math::sin(t) * 1.6f);
		...
		{
			dir = sys->parent->mrot *
				  (bdir.normalize());  // mrot * math::vector_3d(0, 1.0f,0);
			p.speed = dir.normalize() * spd *
					  (1.0f + misc::randfloat(-var, var));  // ?
		}
	}
	else
	{
...
RibbonEmitter::RibbonEmitter(Model* model_, const MPQFile& f,
							 ModelRibbonEmitterDef const& mta, int* globals)
	: model(model_),
	  ...
	  // just use the first texture for now; most models I've checked only had
	  // one
	  ,
...
// in CoT, res and len are like 10 but the trails are supposed to be much longer
// (too short here)
{
	_texture_ids = Model::M2Array<uint16_t>(f, mta.ofsTextures, mta.nTextures);
	_material_ids =
		Model::M2Array<uint16_t>(f, mta.ofsMaterials, mta.nMaterials);
...
void RibbonEmitter::setup(int anim, int time, int animtime)
{
	...
	tpos = ntpos;
	tcolor = math::vector_4d(color.getValue(anim, time, animtime),
							 opacity.getValue(anim, time, animtime));
