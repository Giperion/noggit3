...
class MPQArchive : public AsyncObject
{
   ...
   public:
	...
	static void loadMPQ(AsyncLoader*, const std::string& filename,
						bool doListfile = false);
...
class MPQFile
{
   ...
   public:
	explicit MPQFile(
		const std::string&
			pFilename);  // filenames are not case sensitive, the are if u dont
						 // use a filesystem which is kinda shitty...
	...
	void setBuffer(std::vector<char> const& vec) { buffer = vec; }
