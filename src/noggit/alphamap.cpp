...
Alphamap::Alphamap() { createNew(); }

Alphamap::Alphamap(MPQFile* f, unsigned int flags, bool use_big_alphamaps,
...
void Alphamap::readCompressed(MPQFile* f)
{
	...
	{
		compressed_mcal_entry const* e =
			reinterpret_cast<compressed_mcal_entry const*>(input);

		int count = e->count;

		if (offset_output + count > 4096)
		{
			LogError << "Invalid MCAL, uncompressed size is greater than 4096"
					 << std::endl;
...
void Alphamap::createNew() { memset(amap, 0, 64 * 64); }
...
void Alphamap::setAlpha(unsigned char* pAmap) { memcpy(amap, pAmap, 64 * 64); }

unsigned char Alphamap::getAlpha(size_t offset) const { return amap[offset]; }

const unsigned char* Alphamap::getAlpha() { return amap; }
...
std::vector<uint8_t> Alphamap::compress() const
{
	...
	auto const current_copy_entry([&] {
		return reinterpret_cast<compressed_mcal_entry*>(
			&*(result.begin() + *current_copy_entry_offset));
	});

	for (; current != end; ++current)
	{
		int8_t const fill(consume_fill());
		if (fill)
		{
			...
			compressed_mcal_entry* e(reinterpret_cast<compressed_mcal_entry*>(
				&*(result.rbegin() + 1)));
		...
		{
			if (current_copy_entry_offset == std::nullopt || column_pos == 64)
			{
