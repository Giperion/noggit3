...
namespace noggit
{
	...
	struct model_instance_update : public instance_update
	{
		...
		virtual void apply(World* const world) override
		{
			...
			for (int z = start.z; z <= end.z; ++z)
			{
				...
				{
					world->mapIndex.update_model_tile(
						tile_index(x, z), update_type, instance->uid);
				}
			}
		}
	...
	struct wmo_instance_update : public instance_update
	{
		...
		virtual void apply(World* const world) override
		{
			for (int z = start.z; z <= end.z; ++z)
			{
				for (int x = start.x; x <= end.x; ++x)
				{
					world->mapIndex.update_model_tile(tile_index(x, z),
													  update_type, uid);
				}
			}
		}
		...
		: _world(world)
	{
		_thread = std::make_unique<std::thread>(
			&world_tile_update_queue::process_queue, this);
	}
	...
	void world_tile_update_queue::queue_update(ModelInstance* instance,
											   model_update type)
	{
		{
	...
	void world_tile_update_queue::queue_update(WMOInstance* instance,
											   model_update type)
	{
		std::lock_guard<std::mutex> const lock(_mutex);
