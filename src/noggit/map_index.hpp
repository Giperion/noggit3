...
/*!
\brief This class is only a holder to have easier access to MapTiles and their
flags for easier WDT parsing. This is private and for the class World only.
*/
class MapTileEntry
{
   private:
...
class MapIndex
{
   public:
	...
	struct tile_iterator
		: std::iterator<std::forward_iterator_tag, MapTile*, std::ptrdiff_t,
						MapTile**, MapTile* const&>
	{
		template <typename Pred>
		tile_iterator(MapIndex* index, tile_index tile, Pred pred)
			: _index(index), _tile(std::move(tile)), _pred(std::move(pred))
		{
		...
		{
			return std::tie(_index, _tile) ==
				   std::tie(other._index, other._tile);
		}
		bool operator!=(tile_iterator const& other) const
		{
		...
		MapTile* operator->() const { return operator*(); }
	...
	template <bool Load>
	tile_iterator<Load> tilesStart(
		std::function<bool(tile_index const&, MapTile*)> pred =
			[](tile_index const&, MapTile*) { return true; })
	{
		return tile_iterator<Load>{this, {0, 0}, pred};
	}

	template <bool Load>
	tile_iterator<Load> tilesEnd()
	{
		return tile_iterator<Load>{};
	}

	tile_iterator<false> LoadedTilesStart()
	{
		return tilesStart<false>([](tile_index const&, MapTile* tile) {
			return !!tile && tile->finishedLoading();
		});
	}

	tile_iterator<false> LoadedTilesEnd() { return tilesEnd<false>(); }

	tile_iterator<true> TilesInRangeStart(math::vector_3d const& pos,
										  float radius)
	{
		return tilesStart<true>(
			[this, pos, radius](tile_index const& index, MapTile*) {
				return hasTile(index) &&
					   misc::getShortestDist(pos.x, pos.z, index.x * TILESIZE,
											 index.z * TILESIZE,
											 TILESIZE) <= radius;
			});
	}

	tile_iterator<true> TilesInRangeEnd() { return tilesEnd<true>(); }
	...
	void update_model_tile(const tile_index& tile, model_update type,
						   uint32_t uid);
	...
	void unloadTiles(const tile_index& tile);  // unloads all tiles more then x
											   // adts away from given
	...
	inline bool use_mclq_green_lava() const { return _map_id == 530; }

	bool uid_fix_all_in_progress() const { return _uid_fix_all_in_progress; }
