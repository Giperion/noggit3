...
void MPQArchive::loadMPQ(AsyncLoader* loader, std::string const& filename,
						 bool doListfile)
{
	_openArchives.emplace_back(
		filename, std::make_unique<MPQArchive>(filename, doListfile));
	loader->queue_for_load(_openArchives.back().second.get());
}

MPQArchive::MPQArchive(std::string const& filename, bool doListfile)
	: AsyncObject(filename), _archiveHandle(nullptr)
{
	if (!SFileOpenArchive(filename.c_str(), 0,
...
void MPQArchive::finishLoading()
{
	...
	std::lock_guard lock2(gMPQFileMutex);
	std::lock_guard lock(gListfileLoadingMutex);

	if (SFileOpenFileEx(_archiveHandle, "(listfile)", 0, &fh))
	{
		size_t filesize = SFileGetFileSize(
			fh, nullptr);  // last nullptr for newer version of StormLib

		std::vector<char> readbuffer(filesize);
		SFileReadFile(fh, readbuffer.data(), filesize, nullptr,
					  nullptr);  // last nullptrs for newer version of StormLib
		SFileCloseFile(fh);
	...
	{
		LogDebug << "Completed listfile loading: " << gListfile.size()
				 << " files\n";
	}
}
...
{
	if (_archiveHandle) SFileCloseArchive(_archiveHandle);
}
...
void MPQArchive::unloadAllMPQs() { _openArchives.clear(); }

bool MPQArchive::hasFile(std::string const& InFilename) const
{
	return SFileHasFile(
		_archiveHandle,
		noggit::mpq::normalized_filename_insane(InFilename).c_str());
}
...
bool MPQArchive::openFile(std::string const& InFileName,
						  HANDLE* fileHandle) const
{
	assert(fileHandle);
	return SFileOpenFileEx(
		_archiveHandle,
		noggit::mpq::normalized_filename_insane(InFileName).c_str(), 0,
		fileHandle);
}

namespace
{
	std::filesystem::path getDiskPath(std::string const& pFilename)
	{
		QSettings settings;
		return std::filesystem::path(
				   settings.value("project/path").toString().toStdString()) /
...
MPQFile::MPQFile(std::string const& filename)
	: eof(true), pointer(0), External(false), _disk_path(getDiskPath(filename))
{
	if (filename.empty()) throw std::runtime_error("MPQFile: filename empty");

	std::lock_guard lock(gMPQFileMutex);

	std::ifstream input(_disk_path.string(),
	...
	for (ArchivesMap::reverse_iterator i = _openArchives.rbegin();
		 i != _openArchives.rend(); ++i)
	{
		HANDLE fileHandle;

		if (!i->second->openFile(filename, &fileHandle)) continue;

		eof = false;
		buffer.resize(SFileGetFileSize(fileHandle, nullptr));
		SFileReadFile(fileHandle, buffer.data(), buffer.size(), nullptr,
					  nullptr);  // last nullptrs for newer version of StormLib
...
bool MPQFile::isEof() const { return eof; }
...
void MPQFile::close() { eof = true; }

size_t MPQFile::getSize() const { return buffer.size(); }

size_t MPQFile::getPos() const { return pointer; }

char const* MPQFile::getBuffer() const { return buffer.data(); }
