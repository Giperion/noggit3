...
class ChunkWater
{
   public:
	...
	void save(sExtendableArray& adt, int base_pos, int& header_pos,
			  int& current_pos);

	void draw(math::frustum const& frustum, const float& cull_distance,
			  const math::vector_3d& camera, bool camera_moved,
			  liquid_render& render, opengl::scoped::use_program& water_shader,
			  int animtime, int layer, display_mode display);
   ...
   private:
	...
	void copy_height_to_layer(liquid_layer& target, math::vector_3d const& pos,
							  float radius);
