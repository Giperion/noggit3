...
ModelInstance::ModelInstance(std::string const& filename) : model(filename) {}
	...
	: model(filename)
{
	...
	// scale factor - divide by 1024. blizzard devs must be on crack, why not
	// just use a float?
							 ...
							 bool is_current_selection)
{
	...
	if (is_current_selection)
	{
		...
		opengl::primitives::wire_box(_extents[0], _extents[1])
...
void ModelInstance::intersect(math::matrix_4x4 const& model_view,
							  math::ray const& ray, selection_result* results,
							  int animtime)
{
...
void ModelInstance::recalcExtents()
{
	...
	auto const corners_in_world(math::apply(misc::transform_model_box_coords,
											relative_to_model.all_corners()));

	auto const rotated_corners_in_world(_transform_mat_transposed.transposed() *
										corners_in_world);
	...
	size_cat = (bounding_of_rotated_points.max - bounding_of_rotated_points.min)
				   .length();
...
wmo_doodad_instance::wmo_doodad_instance(std::string const& filename,
										 MPQFile* f)
	: ModelInstance(filename)
{
	...
	light_color = math::vector_3d(color.bgra.r / 255.f, color.bgra.g / 255.f,
								  color.bgra.b / 255.f);
}
...
void wmo_doodad_instance::update_transform_matrix_wmo(WMOInstance* wmo)
{
	...
	math::matrix_4x4 mat(wmo->transform_matrix() * m2_mat);
