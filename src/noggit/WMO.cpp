...
WMO::WMO(const std::string& filenameArg)
	: AsyncObject(filenameArg), _finished_upload(false)
{
}
...
void WMO::finishLoading()
{
	...
	CArgb ambient_color;
	unsigned int nTextures, nGroups, nP, nLights, nModels, nDoodads,
		nDoodadSets, nX;
	// header
	f.read(&nTextures, 4);
	f.read(&nGroups, 4);
	...
	auto load_texture([&](std::uint32_t ofs) {
		char const* texture(texbuf[ofs] ? &texbuf[ofs]
	...
	if (size > 4)
	{
		std::string path = noggit::mpq::normalized_filename(
			std::string(reinterpret_cast<char const*>(f.getPointer())));
		if (StrEndsWith(path, "mdx"))
		{
			size_t strLen = path.size();
			path.replace(path.begin() + (strLen - 3), path.begin() + (strLen),
						 "m2");
		}
	...
	for (auto& group : groups) group.load();
...
void WMO::draw(opengl::scoped::use_program& wmo_shader,
			   ...
			   bool boundingbox, math::frustum const& frustum,
			   const float& cull_distance, const math::vector_3d& camera,
			   bool  // draw_doodads
			   ,
			   bool draw_fog, liquid_render& render, int animtime,
			   bool world_has_skies, display_mode display)
{
	...
	for (auto& group : groups)
	{
		...
		group.draw(wmo_shader, frustum, cull_distance, camera, draw_fog,
				   world_has_skies);

		group.drawLiquid(transform_matrix_transposed, render, draw_fog,
						 animtime);
	}
	...
	if (boundingbox)
	{
		...
		for (auto& group : groups)
		{
			opengl::primitives::wire_box(group.BoundingBoxMin,
										 group.BoundingBoxMax)
				.draw(model_view, projection, transform_matrix_transposed,
					  {1.0f, 1.0f, 1.0f, 1.0f});
		}
...
bool WMO::draw_skybox(
	math::matrix_4x4 const& model_view, math::vector_3d const& camera_pos,
	opengl::scoped::use_program& m2_shader, math::frustum const& frustum,
	const float& cull_distance, int animtime, bool draw_particles,
	math::vector_3d aabb_min, math::vector_3d aabb_max,
	std::map<int, std::pair<math::vector_3d, math::vector_3d>> const&
		group_extents) const
{
	...
	for (int i = 0; i < groups.size(); ++i)
	{
		...
		if (camera_pos.is_inside_of(extent.first, extent.second))
		{
			...
			skybox->get()->draw(model_view, sky, m2_shader, frustum,
								cull_distance, camera_pos, animtime,
								draw_particles, false, display_mode::in_3D);
...
std::map<uint32_t, std::vector<wmo_doodad_instance>> WMO::doodads_per_group(
...
void WMOGroup::upload()
{
	...
	gl.bufferData<GL_ARRAY_BUFFER>(
		_texcoords_buffer, _texcoords.size() * sizeof(*_texcoords.data()),
		_texcoords.data(), GL_STATIC_DRAW);

	gl.bufferData<GL_ELEMENT_ARRAY_BUFFER, std::uint16_t>(
		_indices_buffer, _indices, GL_STATIC_DRAW);

	if (header.flags.has_two_motv)
	{
		gl.bufferData<GL_ARRAY_BUFFER, math::vector_2d>(
			_texcoords_buffer_2, _texcoords_2, GL_STATIC_DRAW);
	}

	gl.bufferData<GL_ARRAY_BUFFER>(
		_vertex_colors_buffer,
		_vertex_colors.size() * sizeof(*_vertex_colors.data()),
		_vertex_colors.data(), GL_STATIC_DRAW);
...
void WMOGroup::setup_vao(opengl::scoped::use_program& wmo_shader)
{
	opengl::scoped::index_buffer_manual_binder indices(_indices_buffer);
	{
		...
		wmo_shader.attrib("position", _vertices_buffer, 3, GL_FLOAT, GL_FALSE,
						  0, 0);
		wmo_shader.attrib("normal", _normals_buffer, 3, GL_FLOAT, GL_FALSE, 0,
						  0);
		wmo_shader.attrib("texcoord", _texcoords_buffer, 2, GL_FLOAT, GL_FALSE,
						  0, 0);
		...
		// even if the 2 flags are set there's only one vertex color vector, the
		// 2nd chunk is used for alpha only
		if (header.flags.has_vertex_color ||
			header.flags.use_mocv2_for_texture_blending)
		{
			wmo_shader.attrib("vertex_color", _vertex_colors_buffer, 4,
...
void WMOGroup::load()
{
	...
	BoundingBoxMin =
		::math::vector_3d(header.box1[0], header.box1[2], -header.box1[1]);
	BoundingBoxMax =
		::math::vector_3d(header.box2[0], header.box2[2], -header.box2[1]);
	...
	::math::vector_3d const* vertices =
		reinterpret_cast<::math::vector_3d const*>(f.getPointer());
	...
	if (header.flags.has_water)
	{
		...
		lq = std::make_unique<wmo_liquid>(
			&f, hlq, wmo->materials[hlq.material_id], header.group_liquid,
			(bool)wmo->flags.use_liquid_type_dbc_id, (bool)header.flags.ocean);
...
void WMOGroup::fix_vertex_color_alpha()
{
	...
	for (int i = 0; i < _vertex_colors.size(); ++i)
	{
		...
		// I removed the color = color/2 because it's just multiplied by 2 in
		// the shader afterward in blizzard's code
		...
		color.w = 1.f;  // default value used in the shader so I simplified it
						// here, it can be overriden by the 2nd mocv chunk
	}
}
...
)
{
	...
	int has_mocv = header.flags.has_vertex_color |
				   header.flags.use_mocv2_for_texture_blending;
	...
	for (wmo_batch& batch : _batches)
	{
		...
		gl.drawRangeElements(
			GL_TRIANGLES, batch.vertex_start, batch.vertex_end,
			batch.index_count, GL_UNSIGNED_SHORT,
			reinterpret_cast<void*>(sizeof(std::uint16_t) * batch.index_start));
	}
}
						 ...
						 std::vector<float>* results) const
{
	...
	{
		for (size_t i(batch.index_start);
			 i < batch.index_start + batch.index_count; i += 3)
		{
...
void WMOGroup::drawLiquid(math::matrix_4x4 const& transform,
						  liquid_render& render, bool  // draw_fog
						  ,
						  int animtime)
{
	// draw liquid
...
{
	f->read(this, 0x30);
	color = math::vector_4d(((color1 & 0x00FF0000) >> 16) / 255.0f,
...
{
	_.apply([&](std::string const&, WMO& wmo) { wmo.show(); });
}
