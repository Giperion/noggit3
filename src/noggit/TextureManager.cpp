...
void blp_texture::upload()
{
	...
	if (!_compression_format)
	{
		...
		{
			gl.texImage2D(GL_TEXTURE_2D, i, GL_RGBA8, width, height, 0, GL_RGBA,
						  GL_UNSIGNED_BYTE, _data[i].data());
	...
	else
	{
		for (int i = 0; i < _compressed_data.size(); ++i)
		{
			gl.compressedTexImage2D(
				GL_TEXTURE_2D, i, _compression_format.value(), width, height, 0,
				_compressed_data[i].size(), _compressed_data[i].data());
...
void blp_texture::loadFromUncompressedData(BLPHeader const* lHeader,
										   char const* lData)
{
	unsigned int const* pal =
		reinterpret_cast<unsigned int const*>(lData + sizeof(BLPHeader));
	...
	for (int i = 0; i < 16; ++i)
	{
		...
		{
			buf = reinterpret_cast<unsigned char const*>(
				&lData[lHeader->offsets[i]]);
			...
			for (int y = 0; y < height; y++)
			{
				for (int x = 0; x < width; x++)
				{
					...
					k = ((k & 0x00FF0000) >> 16) | ((k & 0x0000FF00)) |
						((k & 0x000000FF) << 16);
					int alpha = 0xFF;
					if (hasalpha)
					{
...
void blp_texture::loadFromCompressedData(BLPHeader const* lHeader,
										 char const* lData)
{
	//                         0 (0000) & 3 == 0                1 (0001) & 3 ==
	//                         1                    7 (0111) & 3 == 3
	const int alphatypes[] = {GL_COMPRESSED_RGB_S3TC_DXT1_EXT,
	...
	_compression_format = format == GL_COMPRESSED_RGB_S3TC_DXT1_EXT
							  ? (lHeader->attr_1_alphadepth == 1
	...
	for (int i = 0; i < 16; ++i)
	{
		...
		// make sure the vector is of the right size, blizzard seems to fuck
		// those up for some small mipmaps
		int size = std::floor((width + 3) / 4) * std::floor((height + 3) / 4) *
				   blocksizes[alpha_type];
		...
		std::copy(start, start + lHeader->sizes[i],
				  _compressed_data[i].begin());
...
namespace noggit
{
	QPixmap render_blp_to_pixmap(std::string const& blp_filename, int width,
								 int height)
	{
		...
		std::vector<math::vector_2d> vertices = {{-half_w, -half_h},
												 {-half_w, half_h},
												 {half_w, half_h},
												 {half_w, -half_h}};
		std::vector<math::vector_2d> texcoords = {
			{0.f, 0.f}, {0.f, h}, {w, h}, {w, 0.f}};
		std::vector<std::uint16_t> indices = {0, 1, 2, 2, 3, 0};

		gl.bufferData<GL_ARRAY_BUFFER, math::vector_2d>(vertices_vbo, vertices,
														GL_STATIC_DRAW);
		gl.bufferData<GL_ARRAY_BUFFER, math::vector_2d>(
			texcoords_vbo, texcoords, GL_STATIC_DRAW);
		gl.bufferData<GL_ELEMENT_ARRAY_BUFFER, std::uint16_t>(
			indices_vbo, indices, GL_STATIC_DRAW);
		...
		opengl::program program({{GL_VERTEX_SHADER, R"code(
#version 330 core
...
out_color = vec4(texture(tex, f_tex_coord/2.f + vec2(0.5)).rgb, 1.);
		...
		{
			opengl::scoped::buffer_binder<GL_ARRAY_BUFFER> vertices_binder(
				vertices_vbo);
			shader.attrib("position", 2, GL_FLOAT, GL_FALSE, 0, 0);
		}
		{
			opengl::scoped::buffer_binder<GL_ARRAY_BUFFER> texcoords_binder(
				texcoords_vbo);
			shader.attrib("tex_coord", 2, GL_FLOAT, GL_FALSE, 0, 0);
		}
		opengl::scoped::buffer_binder<GL_ELEMENT_ARRAY_BUFFER> indices_binder(
			indices_vbo);
		...
		if (pixmap.isNull())
		{
			throw std::runtime_error("failed rendering " + blp_filename +
									 " to pixmap");
		}
...
scoped_blp_texture_reference::scoped_blp_texture_reference(
	scoped_blp_texture_reference const& other)
	: _blp_texture(other._blp_texture
					   ? TextureManager::_.emplace(other._blp_texture->filename)
					   : nullptr)
{
}

void scoped_blp_texture_reference::Deleter::operator()(
	blp_texture* texture) const
{
...
bool scoped_blp_texture_reference::operator==(
	scoped_blp_texture_reference const& other) const
{
