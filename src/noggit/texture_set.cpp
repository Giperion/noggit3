...
TextureSet::TextureSet(MapChunkHeader const& header, MPQFile* f, size_t base,
					   MapTile* tile, bool use_big_alphamaps,
	  ...
	  _do_not_convert_alphamaps(do_not_convert_alphamaps)
{
	for (int i = 0; i < 64; ++i)
	{
		...
		// it's a uint2 array so we need to read the bits as they come on the
		// disk, this means reading the highest bits from the uint8 first
		const size_t bit_index((3 - ((i) % 4)) * 2);

		_lod_texture_map.push_back(
			((header.low_quality_texture_map[array_index]) >> (bit_index)) & 3);
	}
	...
	if (nTextures)
	{
		...
		for (size_t i = 0; i < nTextures; ++i)
		{
			...
			textures.emplace_back(
				tile->mTextureFilenames[_layers_info[i].textureID]);
		}
		...
		for (unsigned int layer = 0; layer < nTextures; ++layer)
		{
			if (_layers_info[layer].flags & 0x100)
			{
				...
				// alphamaps[layer - 1] = boost::in_place (f,
				// _layers_info[layer].flags, use_big_alphamaps,
				// do_not_fix_alpha_map);
				alphamaps[layer - 1] =
					Alphamap(f, _layers_info[layer].flags, use_big_alphamaps,
							 do_not_fix_alpha_map);
			}
		}
...
void TextureSet::replace_texture(
	...
	scoped_blp_texture_reference replacement_texture)
{
	...
	if (texture_to_replace_level != -1)
	{
		...
		if (replacement_texture_level != -1 &&
			replacement_texture_level != texture_to_replace_level)
		{
...
void TextureSet::swap_layers(int layer_1, int layer_2)
{
	...
	if (lower_texture_id >= 0 && upper_texture_id >= 0 &&
		lower_texture_id < nTextures && upper_texture_id < nTextures)
	{
		apply_alpha_changes();

		std::swap(textures[lower_texture_id], textures[upper_texture_id]);
		std::swap(_layers_info[lower_texture_id],
				  _layers_info[upper_texture_id]);
...
void TextureSet::eraseTexture(size_t id)
{
	...
	// erase the old info as a precaution but it's overriden when adding a new
	// texture
...
math::vector_2d TextureSet::anim_uv_offset(int id, int animtime) const
{
	...
	float f = ((static_cast<int>(animtime * (spd / 7.0f))) % animspd) /
...
bool TextureSet::eraseUnusedTextures()
{
	...
	if (tmp_edit_values)
	{
		tmp_edit_alpha_values& amaps = tmp_edit_values.value();
...
bool TextureSet::paintTexture(float xbase, float zbase, float x, float z,
							  Brush* brush, float strength, float pressure,
							  scoped_blp_texture_reference texture)
{
	...
	for (int j = 0; j < 64; j++)
	{
		...
		for (int i = 0; i < 64; ++i)
		{
			dist = misc::dist(x, z, xPos + TEXDETAILSIZE / 2.0f,
				...
				if (!misc::float_equals(current_alpha, strength))
				{
					for (int layer = 0; layer < nTextures; ++layer)
					{
						...
						else
						{
							amaps[layer][offset] -=
								alpha_change *
								(amaps[layer][offset] / sum_other_alphas);
						}
					}
...
bool TextureSet::replace_texture(
	float xbase, float zbase, float x, float z, float radius,
	scoped_blp_texture_reference const& texture_to_replace,
	scoped_blp_texture_reference replacement_texture)
{
	...
	if (old_tex_level == -1 ||
		(new_tex_level == -1 && nTextures == 4 && !eraseUnusedTextures()))
	{
		return false;
	}
	...
	create_temporary_alphamaps_if_needed();
	tmp_edit_alpha_values& amap = tmp_edit_values.value();
	...
	for (int j = 0; j < 64; j++)
	{
		...
		for (int i = 0; i < 64; ++i)
		{
			dist = misc::dist(x, z, x_pos + TEXDETAILSIZE / 2.0f,
							  z_pos + TEXDETAILSIZE / 2.0f);
...
unsigned int TextureSet::flag(size_t id) { return _layers_info[id].flags; }

unsigned int TextureSet::effect(size_t id) { return _layers_info[id].effectID; }
...
void TextureSet::change_texture_flag(scoped_blp_texture_reference const& tex,
	...
	if (nTextures > 1)
	{
		...
		else
		{
			...
			if (_do_not_convert_alphamaps)
			{
				...
				{
					memcpy(tab + (k * 64 * 64), alphamaps[k]->getAlpha(),
						   64 * 64);
				}
			}
			else
			...
			auto const combine_nibble([&](int layer, int pos) {
...
{
	auto alpha(
		[&](int layer, int pos = 0) { return dest + layer * 4096 + pos; });
...
void TextureSet::merge_layers(size_t id1, size_t id2)
{
	...
	for (int i = 0; i < 64 * 64; ++i)
	{
		...
		// no need to set the id alphamap to 0, it'll be done in
		// "eraseTexture(id2)"
	}
	...
	if (_need_amap_update)
	{
		if (nTextures)
		{
			if (tmp_edit_values)
			{
				...
				tmp_edit_alpha_values& tmp_amaps = tmp_edit_values.value();
				...
				gl.texImage2D(GL_TEXTURE_2D, 0, GL_RGB, 64, 64, 0, GL_RGB,
							  GL_FLOAT, amap.data());
			}
			else
			{
...
namespace
{
	misc::max_capacity_stack_vector<std::size_t, 4> current_layer_values(
		std::uint8_t nTextures, std::optional<Alphamap> const* alphamaps,
		std::size_t pz, std::size_t px)
	{
...
void TextureSet::update_lod_texture_map()
{
	...
	for (std::size_t z = 0; z < 8; ++z)
	{
		...
		{
			misc::max_capacity_stack_vector<std::size_t, 4>
				dominant_square_count(nTextures);

			for (std::size_t pz = z * 8; pz < (z + 1) * 8; ++pz)
			{
				for (std::size_t px = x * 8; px < (x + 1) * 8; ++px)
				{
					++dominant_square_count[max_element_index(
						current_layer_values(nTextures, alphamaps.data(), pz,
											 px))];
				}
			}
...
namespace
{
	inline std::uint8_t float_alpha_to_uint8(float a)
	{
		return static_cast<std::uint8_t>(
			std::max(0.f, std::min(255.f, std::round(a))));
	}
}  // namespace
...
bool TextureSet::apply_alpha_changes()
{
	...
	tmp_edit_alpha_values& new_amaps = tmp_edit_values.value();
	...
	for (int alpha_layer = 0; alpha_layer < nTextures - 1; ++alpha_layer)
	{
		...
		{
			values[i] =
				float_alpha_to_uint8(new_amaps[alpha_layer + 1][i] * 255.f);
		}
...
void TextureSet::create_temporary_alphamaps_if_needed()
{
	...
	for (int i = 0; i < 64 * 64; ++i)
	{
		...
		{
			float f =
				static_cast<float>(alphamaps[alpha_layer]->getAlpha(i)) / 255.f;
