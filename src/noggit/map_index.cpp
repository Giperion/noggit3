	  ...
	  _world(world)
{
	...
	/// this is the theory. Sadly, we are also compiling on 64 bit machines with
	/// size_t being 8 byte, not 4. Therefore, we can't do the same thing,
	/// Blizzard does in its 32bit executable.
	...
	for (int j = 0; j < 64; ++j)
	{
		for (int i = 0; i < 64; ++i)
		{
			...
			adt_filename << "World\\Maps\\" << basename << "\\" << basename
						 << "_" << i << "_" << j << ".adt";
	...
	{
		//! \note We actually don't load WMO only worlds, so we just stop
		//! reading here, k? \bug MODF reads wrong. The assertion fails every
		//! time. Somehow, it keeps being MWMO. Or are there two blocks?
		//! \nofuckingbug  on eof read returns just without doing sth to the var
		//! and some wdts have a MWMO without having a MODF so only checking for
		//! eof above is not enough
...
void MapIndex::saveall(World* world)
{
	...
	for (tile_iterator<false> It = LoadedTilesStart(); It != LoadedTilesEnd();
		 It++)
	{
		MapTile* tile = *It;
		tile->saveTile(world);
...
void MapIndex::update_model_tile(const tile_index& tile, model_update type,
								 uint32_t uid)
{
	if (!hasTile(tile))
	{
...
void MapIndex::setChanged(MapTile* tile) { setChanged(tile->index); }
...
MapTile* MapIndex::loadTile(const tile_index& tile, bool reloading)
{
	...
	std::stringstream filename;
	filename << "World\\Maps\\" << basename << "\\" << basename << "_" << tile.x
			 << "_" << tile.z << ".adt";
	...
	mTiles[tile.z][tile.x].tile = std::make_unique<MapTile>(
		tile.x, tile.z, filename.str(), mBigAlpha, true, use_mclq_green_lava(),
		reloading, _world);
...
void MapIndex::unloadTiles(const tile_index& tile)
{
	...
	{
		for (tile_iterator<false> It = LoadedTilesStart();
			 It != LoadedTilesEnd(); It++)
		{
			MapTile* adt = *It;
...
bool MapIndex::hasAGlobalWMO() { return mHasAGlobalWMO; }
...
bool MapIndex::hasAdt() { return noadt; }

void MapIndex::setAdt(bool value) { noadt = value; }
...
MapTile* MapIndex::getTileAbove(MapTile* tile) const
{
	...
	if (tile->index.z == 0 ||
		(!tileLoaded(above) && !tileAwaitingLoading(above)))
	{
		return nullptr;
	}
...
uint32_t MapIndex::newGUID()
{
	...
	if (settings->value("project/mysql/enabled", false).toBool())
	{
		mysql::updateUIDinDB(
			_map_id,
			highestGUID + 1);  // update the highest uid in db, note that if the
							   // user don't save these uid won't be used (not
							   // really a problem tho)
	}
#endif
	return ++highestGUID;
}

uid_fix_status MapIndex::fixUIDs(World* world,
								 bool cancel_on_model_loading_error)
{
	...
	for (int z = 0; z < 64; ++z)
	{
		for (int x = 0; x < 64; ++x)
		{
			...
			std::stringstream filename;
			filename << "World\\Maps\\" << basename << "\\" << basename << "_"
					 << x << "_" << z << ".adt";
			...
			ENTRY_MDDF const* mddf_ptr =
				reinterpret_cast<ENTRY_MDDF const*>(file.getPointer());
			...
			ENTRY_MODF const* modf_ptr =
				reinterpret_cast<ENTRY_MODF const*>(file.getPointer());
			...
			{
				char const* lCurPos =
					reinterpret_cast<char const*>(file.getPointer());
	...
	std::map<std::size_t,
			 std::map<std::size_t, std::forward_list<std::uint32_t>>>
		uids_per_tile;
	...
	for (ModelInstance& instance : models)
	{
		...
		std::size_t sx =
			std::max((std::size_t)(extents[0].x / TILESIZE), (std::size_t)0);
		std::size_t sz =
			std::max((std::size_t)(extents[0].z / TILESIZE), (std::size_t)0);
		std::size_t ex =
			std::min((std::size_t)(extents[1].x / TILESIZE), (std::size_t)63);
		std::size_t ez =
			std::min((std::size_t)(extents[1].z / TILESIZE), (std::size_t)63);

		auto const real_uid(
			world->add_model_instance(std::move(instance), false));
	...
	for (WMOInstance& instance : wmos)
	{
		...
		// no need to check if the loading is finished since the extents are
		// stored inside the adt to avoid going outside of bound
		std::size_t sx = std::max(
			(std::size_t)(instance.extents[0].x / TILESIZE), (std::size_t)0);
		std::size_t sz = std::max(
			(std::size_t)(instance.extents[0].z / TILESIZE), (std::size_t)0);
		std::size_t ex = std::min(
			(std::size_t)(instance.extents[1].x / TILESIZE), (std::size_t)63);
		std::size_t ez = std::min(
			(std::size_t)(instance.extents[1].z / TILESIZE), (std::size_t)63);

		auto const real_uid(
			world->add_wmo_instance(std::move(instance), false));
	...
	for (int z = 0; z < 64; ++z)
	{
		for (int x = 0; x < 64; ++x)
		{
			...
			MapTile tile(x, z, filename.str(), mBigAlpha, false,
						 use_mclq_green_lava(), false, world,
	...
	return loading_error ? uid_fix_status::done_with_errors
						 : uid_fix_status::done;
}
	...
	for (int z = 0; z < 64; ++z)
	{
		for (int x = 0; x < 64; ++x)
		{
			...
			filename << "World\\Maps\\" << basename << "\\" << basename << "_"
					 << x << "_" << z << ".adt";
			highestGUID =
				std::max(highestGUID, getHighestGUIDFromFile(filename.str()));
		}
	}
