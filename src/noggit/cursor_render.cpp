...
namespace noggit
{
	void cursor_render::draw(mode cursor_mode, math::matrix_4x4 const& mvp,
							 math::vector_4d color, math::vector_3d const& pos,
							 float radius, float inner_radius_ratio)
	{
		if (!_uploaded)
		{
			upload();
		...
		opengl::scoped::vao_binder const _(
			_vaos[static_cast<int>(cursor_mode)]);

		gl.drawElements(GL_LINES, _indices_count[cursor_mode],
	...
	void cursor_render::create_circle_buffer(
		opengl::scoped::use_program& shader)
	{
		...
		gl.bufferData<GL_ARRAY_BUFFER>(
			_vbos[id * 2], vertices.size() * sizeof(*vertices.data()),
			vertices.data(), GL_STATIC_DRAW);
		gl.bufferData<GL_ELEMENT_ARRAY_BUFFER>(
			_vbos[id * 2 + 1], indices.size() * sizeof(*indices.data()),
			indices.data(), GL_STATIC_DRAW);

		opengl::scoped::index_buffer_manual_binder indices_binder(
			_vbos[id * 2 + 1]);

		{
			opengl::scoped::vao_binder const _(_vaos[id]);

			opengl::scoped::buffer_binder<GL_ARRAY_BUFFER> vertices_vbo(
				_vbos[id * 2]);
	...
	void cursor_render::create_sphere_buffer(
		opengl::scoped::use_program& shader)
	{
		std::vector<math::vector_3d> vertices;
		std::vector<std::uint16_t> indices;
		...
		{
			math::degrees rotation(360.f * r /
								   static_cast<float>(rotation_plane));

			math::matrix_4x4 m(
				math::matrix_4x4::rotation_xyz,
				math::degrees::vec3(math::degrees(0.f), math::degrees(0.f),
									rotation));
	...
	void cursor_render::create_square_buffer(
		opengl::scoped::use_program& shader)
	{
		std::vector<math::vector_3d> vertices = {{-0.5f, 0.f, -0.5f},
												 {0.5f, 0.f, -0.5f},
												 {0.5f, 0.f, 0.5f},
												 {-0.5f, 0.f, 0.5f}};
