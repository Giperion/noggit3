...
class ParticleEmitter
{
   public:
	...
	virtual Particle newParticle(ParticleSystem* sys, int anim, int time,
								 int animtime, float w, float l, float spd,
								 float var, float spr, float spr2) = 0;
};
...
class PlaneParticleEmitter : public ParticleEmitter
{
   public:
	...
	Particle newParticle(ParticleSystem* sys, int anim, int time, int animtime,
						 float w, float l, float spd, float var, float spr,
						 float spr2);
};
...
class ParticleSystem
{
	...
	Animation::M2Value<float> speed, variation, spread, lat, gravity, lifespan,
		rate, areal, areaw, deacceleration;
   ...
   public:
	...
	void draw(math::matrix_4x4 const& model_view,
			  opengl::scoped::use_program& shader, GLuint const& transform_vbo,
			  int instances_count);
...
struct RibbonSegment
{
	...
	float len, len0;
	RibbonSegment(::math::vector_3d pos_, float len_) : pos(pos_), len(len_) {}
};
...
class RibbonEmitter
{
   ...
   public:
	RibbonEmitter(Model*, const MPQFile& f, ModelRibbonEmitterDef const& mta,
				  int* globals);
	...
	void setup(int anim, int time, int animtime);
	void draw(opengl::scoped::use_program& shader, GLuint const& transform_vbo,
			  int instances_count);
