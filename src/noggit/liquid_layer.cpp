...
liquid_layer::liquid_layer(math::vector_3d const& base, float height,
						   int liquid_id)
	  ...
	  pos(base)
{
	for (int z = 0; z < 9; ++z)
	{
		for (int x = 0; x < 9; ++x)
		{
			...
			_vertices.emplace_back(pos.x + UNITSIZE * x, height,
								   pos.z + UNITSIZE * z);
		}
	}
...
liquid_layer::liquid_layer(math::vector_3d const& base, mclq& liquid,
						   int liquid_id)
	: _liquid_id(liquid_id),
	  ...
	  pos(base)
{
	...
	for (int z = 0; z < 8; ++z)
	{
		for (int x = 0; x < 8; ++x)
		{
			misc::set_bit(_subchunks, x, z,
						  !liquid.tiles[z * 8 + x].dont_render);
		}
	}
	...
	for (int z = 0; z < 9; ++z)
	{
		for (int x = 0; x < 9; ++x)
		{
			...
			{
				_depth.emplace_back(1.0f);
				_tex_coords.emplace_back(static_cast<float>(v.magma.x) / 255.f,
										 static_cast<float>(v.magma.y) / 255.f);
			}
			else
			{
			...
			_vertices.emplace_back(
				pos.x + UNITSIZE * x
				// sometimes there's garbage data on unused tiles that mess
				// things up
				,
...
liquid_layer::liquid_layer(MPQFile& f, std::size_t base_pos,
						   math::vector_3d const& base,
	  ...
	  pos(base)
{
	...
	for (int z = 0; z < info.height; ++z)
	{
		...
		{
			setSubchunk(x + info.xOffset, z + info.yOffset,
						(infomask >> offset) & 1);
			offset++;
		}
	}
	...
	for (int z = 0; z < 9; ++z)
	{
		for (int x = 0; x < 9; ++x)
		{
			_tex_coords.emplace_back(default_uv(x, z));
			_depth.emplace_back(1.0f);
			_vertices.emplace_back(pos.x + UNITSIZE * x, _minimum,
								   pos.z + UNITSIZE * z);
		}
	}
		...
		if (_liquid_vertex_format == 1)
		{
			for (int z = info.yOffset; z <= info.yOffset + info.height; ++z)
			{
				for (int x = info.xOffset; x <= info.xOffset + info.width; ++x)
				{
					...
					_tex_coords[z * 9 + x] = {static_cast<float>(uv.x) / 255.f,
...
void liquid_layer::save(sExtendableArray& adt, int base_pos, int& info_pos,
						int& current_pos) const
{
	...
	if (_liquid_vertex_format == 0 || _liquid_vertex_format == 1)
	{
		...
		for (int z = info.yOffset; z <= info.yOffset + info.height; ++z)
		{
			for (int x = info.xOffset; x <= info.xOffset + info.width; ++x)
			{
				memcpy(adt.GetPointer<char>(current_pos),
					   &_vertices[z * 9 + x].y, sizeof(float));
	...
	if (_liquid_vertex_format == 1)
	{
		...
		for (int z = info.yOffset; z <= info.yOffset + info.height; ++z)
		{
			...
			{
				mh2o_uv uv;
				uv.x = static_cast<std::uint16_t>(
					std::min(_tex_coords[z * 9 + x].x * 255.f, 65535.f));
				uv.y = static_cast<std::uint16_t>(
					std::min(_tex_coords[z * 9 + x].y * 255.f, 65535.f));
	...
	if (_liquid_vertex_format == 0 || _liquid_vertex_format == 2)
	{
		...
		for (int z = info.yOffset; z <= info.yOffset + info.height; ++z)
		{
			for (int x = info.xOffset; x <= info.xOffset + info.width; ++x)
			{
				std::uint8_t depth = static_cast<std::uint8_t>(
					std::min(_depth[z * 9 + x] * 255.0f, 255.f));
				memcpy(adt.GetPointer<char>(current_pos), &depth,
					   sizeof(std::uint8_t));
...
void liquid_layer::update_buffers()
{
	...
	{
		opengl::scoped::buffer_binder<GL_ELEMENT_ARRAY_BUFFER> const
			index_buffer(_index_buffer[i]);
		gl.bufferData(GL_ELEMENT_ARRAY_BUFFER,
					  _indices_by_lod[i].size() * sizeof(uint16_t),
					  _indices_by_lod[i].data(), GL_STATIC_DRAW);
	}

	gl.bufferData<GL_ARRAY_BUFFER>(_vertices_vbo,
								   sizeof(*_vertices.data()) * _vertices.size(),
								   _vertices.data(), GL_STATIC_DRAW);
	gl.bufferData<GL_ARRAY_BUFFER>(_depth_vbo,
								   sizeof(*_depth.data()) * _depth.size(),
								   _depth.data(), GL_STATIC_DRAW);
	gl.bufferData<GL_ARRAY_BUFFER>(
		_tex_coord_vbo, sizeof(*_tex_coords.data()) * _tex_coords.size(),
...
void liquid_layer::draw(liquid_render& render,
						...
						math::vector_3d const& camera, bool camera_moved,
						int animtime)
{
	...
	if (lod_changed)
	{
		gl.bindBuffer(GL_ELEMENT_ARRAY_BUFFER,
					  _index_buffer[_current_lod_level]);
	}

	gl.drawElements(GL_TRIANGLES, _current_lod_indices_count, GL_UNSIGNED_SHORT,
					nullptr);
}
		...
		for (int z = 0; z < 8; ++z)
		{
			for (int x = 0; x < 8; ++x)
			{
				if (hasSubchunk(x, z))
				{
					...
					if (_vertices[water_index].y <
							chunk->mVertices[terrain_index].y &&
						_vertices[water_index + 1].y <
							chunk->mVertices[terrain_index + 1].y &&
						_vertices[water_index + 9].y <
							chunk->mVertices[terrain_index + 17].y &&
						_vertices[water_index + 10].y <
							chunk->mVertices[terrain_index + 18].y)
					{
...
void liquid_layer::paintLiquid(math::vector_3d const& cursor_pos, float radius,
							   bool add, math::radians const& angle,
							   math::radians const& orientation, bool lock,
							   math::vector_3d const& origin,
							   bool override_height, MapChunk* chunk,
							   float opacity_factor)
{
	...
	for (int z = 0; z < 8; ++z)
	{
		for (int x = 0; x < 8; ++x)
		{
			if (misc::getShortestDist(cursor_pos, _vertices[id], UNITSIZE) <=
				radius)
			{
				if (add)
				{
					for (int index : {id, id + 1, id + 9, id + 10})
					{
						...
						bool in_range =
							misc::dist(cursor_pos, _vertices[index]) <= radius;

						if (no_subchunk || (in_range && override_height))
						{
							_vertices[index].y = misc::angledHeight(
								ref, _vertices[index], angle, orientation);
						}
						if (no_subchunk || in_range)
						{
							update_vertex_opacity(index % 9, index / 9, chunk,
												  opacity_factor);
						}
					}
				}
...
void liquid_layer::update_min_max()
{
	...
	// lvf = 2 means the liquid height is 0, switch to lvf 0 when that's not the
	// case
	if (_liquid_vertex_format == 2 && (!misc::float_equals(0.f, _minimum) ||
									   !misc::float_equals(0.f, _maximum)))
	{
		_liquid_vertex_format = 0;
	}
	// use lvf 2 when possible to save space
	else if (_liquid_vertex_format == 0 && misc::float_equals(0.f, _minimum) &&
			 misc::float_equals(0.f, _maximum))
	{
...
void liquid_layer::update_vertex_opacity(int x, int z, MapChunk* chunk,
										 float factor)
{
	float diff = _vertices[z * 9 + x].y - chunk->mVertices[z * 17 + x].y;
	_depth[z * 9 + x] =
		diff < 0.0f ? 0.0f
					: (std::min(1.0f, std::max(0.0f, (diff + 1.0f) * factor)));
}
