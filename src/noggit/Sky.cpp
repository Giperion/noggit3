...
{
	pos = math::vector_3d(data->getFloat(LightDB::PositionX) / skymul,
	...
	int light_int_start =
		light_param_0 * NUM_SkyColorNames - 17;  // cromons light fix ;) Thanks
	...
	try
	{
		...
		_ocean_deep_alpha =
			light_param.getFloat(LightParamsDB::ocean_deep_alpha);

		if (skybox_id)
		{
			skybox.emplace(gLightSkyboxDB.getByID(skybox_id).getString(
...
const math::degrees angles[] = {math::degrees(90.0f), math::degrees(30.0f),
								math::degrees(15.0f), math::degrees(5.0f),
								math::degrees(0.0f),  math::degrees(-30.0f),
								math::degrees(-90.0f)};
	...
	: stars(ModelInstance("Environments\\Stars\\Stars.mdx"))
{
	...
	// smaller skies will have precedence when calculating weights to achieve
...
void Skies::update_sky_colors(math::vector_3d pos, int time)
{
	...
	for (size_t j = 0; j < skies.size(); j++)
	{
		...
		if (sky.weight > 0)
		{
			...
			for (int i = 0; i < NUM_SkyColorNames; ++i)
			{
				if ((sky.colorFor(i, time).x > 1.0f) ||
					(sky.colorFor(i, time).y > 1.0f) ||
...
bool Skies::draw(math::matrix_4x4 const& model_view,
				 ...
				 int animtime, bool draw_particles,
				 OutdoorLightStats const& light_stats)
{
...

	{
...

		{
			...
			gl.drawElements(GL_TRIANGLES, _indices_count, GL_UNSIGNED_SHORT,
							nullptr);
		}
	}
	...
	for (Sky& sky : skies)
	{
		...
		{
			ModelInstance& model = sky.skybox.value();
			model.model->trans = sky.weight;
			model.pos = camera_pos;
			model.scale = 0.1f;
			model.recalcExtents();

			model.model->draw(model_view, model, m2_shader, frustum,
	...
	if (light_stats.nightIntensity > 0)
	{
		...
		stars.model->draw(model_view, stars, m2_shader, frustum, cull_distance,
						  camera_pos, animtime, draw_particles, false,
						  display_mode::in_3D);
	}
...
void Skies::upload()
{
	_program.reset(new opengl::program({{GL_VERTEX_SHADER, R"code(
#version 330 core
...
void main()
{
  out_color = vec4(f_color, 1.);
	...
	for (int h = 0; h < hseg; h++)
	{
		...
		{
			basepos1[i] = basepos2[i] = math::vector_3d(
				math::cos(angles[i]) * rad, math::sin(angles[i]) * rad, 0);
			math::rotate(0, 0, &basepos1[i].x, &basepos1[i].z,
						 math::radians(math::constants::pi * 2.0f / hseg * h));
			math::rotate(
				0, 0, &basepos2[i].x, &basepos2[i].z,
				math::radians(math::constants::pi * 2.0f / hseg * (h + 1)));
		}
...
void Skies::update_vao(opengl::scoped::use_program& shader)
{
...

	{
		...
		opengl::scoped::buffer_binder<GL_ARRAY_BUFFER> vertices_buffer(
			_vertices_vbo);
		shader.attrib("position", 3, GL_FLOAT, GL_FALSE, 0, 0);

		opengl::scoped::buffer_binder<GL_ARRAY_BUFFER> colors_buffer(
			_colors_vbo);
...
void Skies::update_color_buffer()
{
	...
	gl.bufferData<GL_ARRAY_BUFFER, math::vector_3d>(_colors_vbo, colors,
													GL_STATIC_DRAW);
...
void OutdoorLightStats::interpolate(OutdoorLightStats* a, OutdoorLightStats* b,
									float r)
{
	float ir = 1.0f - r;
	time = 0;  // unused
