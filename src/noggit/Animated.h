...
namespace Animation
{
	...
	{
		//! \todo C++0x: Change namespace to "enum class Type : int16_t", remove
		//! typedef.
		namespace Type
		{
	...
	struct Conversion
	{
		inline TO operator()(const FROM& value) { return TO(value); }
	};

	template <>
	inline math::quaternion
	Conversion<math::packed_quaternion, math::quaternion>::operator()(
		const math::packed_quaternion& value)
	{
		//! \todo Check if this is really correct.
		return math::quaternion(
			static_cast<float>(
				(value.x > 0 ? value.x - 32767 : value.x + 32767) / 32767.0f),
			static_cast<float>(
				(value.y > 0 ? value.y - 32767 : value.y + 32767) / 32767.0f),
			static_cast<float>(
				(value.z > 0 ? value.z - 32767 : value.z + 32767) / 32767.0f),
			static_cast<float>(
				(value.w > 0 ? value.w - 32767 : value.w + 32767) / 32767.0f));
	}
	...
	//! \note The conversion from DataType to AnimatedType is done via
	   ...
	   public:
							  ...
							  int animtime)
		{
			...
			if (!timestampVector.empty())
			{
				...
				{
					if (time >= timestampVector[i] &&
						time < timestampVector[i + 1])
					{
						pos = i;
						break;
					}
				...
				else
				{
					...
					const float percentage =
						(time - t1) / static_cast<float>(t2 - t1);

					switch (_interpolationType)
					{
						case Animation::Interpolation::Type::LINEAR:
						{
							result = math::interpolation::linear(
								percentage, dataVector[pos],
								dataVector[pos + 1]);
						}
						break;
						...
						{
							result = math::interpolation::hermite(
								percentage, dataVector[pos],
								dataVector[pos + 1], inVector[pos],
								outVector[pos]);
						}
						break;
					}
		...
		//! \todo Use a vector of MPQFile& for the anim files instead for
		//! safety.
		M2Value(const AnimationBlock& animationBlock, const MPQFile& file,
				int32_t* globalSequences,
				const std::vector<std::unique_ptr<MPQFile>>& animation_files =
					std::vector<std::unique_ptr<MPQFile>>())
		{
			...
			if (_globalSequenceID != NO_GLOBAL_SEQUENCE)
			{
				assert(_globalSequences &&
					   "Animation said to have global sequence, but pointer to "
					   "global sequence data is nullptr");
			}

			const AnimationBlockHeader* timestampHeaders =
				file.get<AnimationBlockHeader>(animationBlock.ofsTimes);
			const AnimationBlockHeader* keyHeaders =
				file.get<AnimationBlockHeader>(animationBlock.ofsKeys);

			for (size_t j = 0; j < animationBlock.nTimes; ++j)
			{
				const TimestampType* timestamps =
					j < animation_files.size() && animation_files[j]
						? animation_files[j]->get<TimestampType>(
							  timestampHeaders[j].ofsEntries)
						: file.get<TimestampType>(
							  timestampHeaders[j].ofsEntries);
			...
			for (size_t j = 0; j < animationBlock.nKeys; ++j)
			{
				const DataType* keys =
					j < animation_files.size() && animation_files[j]
						? animation_files[j]->get<DataType>(
							  keyHeaders[j].ofsEntries)
