...
TileWater::TileWater(MapTile* pTile, float pXbase, float pZbase,
	...
	: tile(pTile), xbase(pXbase), zbase(pZbase)
{
	for (int z = 0; z < 16; ++z)
	{
		for (int x = 0; x < 16; ++x)
		{
			chunks[z][x] = std::make_unique<ChunkWater>(xbase + CHUNKSIZE * x,
														zbase + CHUNKSIZE * z,
														use_mclq_green_lava);
		}
	}
...
void TileWater::draw(math::frustum const& frustum, const float& cull_distance,
					 ...
					 int layer, display_mode display)
{
	for (int z = 0; z < 16; ++z)
	{
		for (int x = 0; x < 16; ++x)
		{
			chunks[z][x]->draw(frustum, cull_distance, camera, camera_moved,
							   render, water_shader, animtime, layer, display);
		}
	}
}

ChunkWater* TileWater::getChunk(int x, int z) { return chunks[z][x].get(); }
...
void TileWater::saveToFile(sExtendableArray& lADTFile, int& lMHDR_Position,
						   int& lCurrentPosition)
{
	...
	lADTFile.GetPointer<MHDR>(lMHDR_Position + 8)->mh2o =
		lCurrentPosition - 0x14;  // setting offset to MH2O data in Header
