...
bool World::IsEditableWorld(int pMapId)
{
	...
	if (mf.isEof()) return false;

	const char* lPointer = reinterpret_cast<const char*>(mf.getPointer());

	// Not using the libWDT here doubles performance. You might want to look at
	// your lib again and improve it.
	const int lFlags = *(reinterpret_cast<const int*>(lPointer + 8 + 4 + 8));
	if (lFlags & 1) return false;

	const int* lData =
		reinterpret_cast<const int*>(lPointer + 8 + 4 + 8 + 0x20 + 8);
	for (int i = 0; i < 8192; i += 2)
	{
		if (lData[i] & 1) return true;
	}
...
void World::update_selection_pivot()
{
	if (has_multiple_model_selected())
	{
		...
		for (auto const& entry : _current_selection)
		{
			...
			{
				pivot += GetFromSelection<selected_model_type>(entry)->pos;
				model_count++;
			}
			else if (entry.which() == eEntry_WMO)
			{
				pivot += GetFromSelection<selected_wmo_type>(entry)->pos;
				model_count++;
			}
		}
...
bool World::is_selected(selection_type selection) const
{
	...
	{
		uint uid = GetFromSelection<selected_model_type>(selection)->uid;
		auto const& it = std::find_if(
			_current_selection.begin(), _current_selection.end(),
			[uid](selection_type type) {
				return type.type() == typeid(selected_model_type) &&
					   GetFromSelection<selected_model_type>(type)->uid == uid;
			});
	...
	{
		uint uid = GetFromSelection<selected_wmo_type>(selection)->mUniqueID;
		auto const& it = std::find_if(
			_current_selection.begin(), _current_selection.end(),
			[uid](selection_type type) {
				return type.type() == typeid(selected_wmo_type) &&
					   GetFromSelection<selected_wmo_type>(type)->mUniqueID ==
						   uid;
			});
		if (it != _current_selection.end())
		{
...
bool World::is_selected(std::uint32_t uid) const
{
	for (selection_type const& entry : _current_selection)
	{
		...
		{
			if (GetFromSelection<selected_wmo_type>(entry)->mUniqueID == uid)
			{
				return true;
			}
		}
		...
		{
			if (GetFromSelection<selected_model_type>(entry)->uid == uid)
			{
				return true;
			}
		}
...
{
	auto const it(std::find_if(_current_selection.rbegin(),
							   _current_selection.rend(),
							   [&](selection_type const& entry) {
								   return entry.which() != eEntry_MapChunk;
							   }));

	return it == _current_selection.rend() ? std::nullopt
										   : std::optional<selection_type>(*it);
}
...
{
	std::vector<selection_type>::iterator position =
		std::find(_current_selection.begin(), _current_selection.end(), entry);
...
{
	for (auto it = _current_selection.begin(); it != _current_selection.end();
		 ++it)
	{
		if (it->which() == eEntry_Model &&
			GetFromSelection<selected_model_type>(*it)->uid == uid)
		{
			...
			return;
				 ...
				 GetFromSelection<selected_wmo_type>(*it)->mUniqueID == uid)
		{
			_current_selection.erase(it);
			update_selection_pivot();
			return;
...
void World::snap_selected_models_to_the_ground()
{
	for (auto& entry : _current_selection)
	{
		...
		math::vector_3d& pos =
			entry_is_m2 ? GetFromSelection<selected_model_type>(entry)->pos
						: GetFromSelection<selected_wmo_type>(entry)->pos;
		...
		pos.y =
			GetFromSelection<selected_chunk_type>(hits[0].second).position.y;

		if (entry_is_m2)
		{
			GetFromSelection<selected_model_type>(entry)->recalcExtents();
		}
		else
		{
			GetFromSelection<selected_wmo_type>(entry)->recalcExtents();
		}
...
void World::scale_selected_models(float v, m2_scaling_type type)
{
	for (auto& entry : _current_selection)
	{
		if (entry.which() == eEntry_Model)
		{
			ModelInstance* mi = GetFromSelection<selected_model_type>(entry);
...
void World::set_selected_models_pos(math::vector_3d const& pos,
									bool change_height)
{
	...
	for (auto& entry : _current_selection)
	{
		...
		{
			WMOInstance* wi = GetFromSelection<selected_wmo_type>(entry);
			wi->pos = pos;
			wi->recalcExtents();
		}
...
void World::rotate_selected_models(math::degrees rx, math::degrees ry,
								   math::degrees rz, bool use_pivot)
{
	...
	for (auto& entry : _current_selection)
	{
		...
		if (use_pivot && has_multi_select)
		{
			...
			math::vector_3d& dir =
				entry_is_m2 ? GetFromSelection<selected_model_type>(entry)->dir
							: GetFromSelection<selected_wmo_type>(entry)->dir;

			math::vector_3d diff_pos = pos - _multi_select_pivot.value();
			math::vector_3d rot_result =
				math::matrix_4x4(math::matrix_4x4::rotation_xyz, {rx, ry, rz}) *
				diff_pos;
...
void World::set_selected_models_rotation(math::degrees rx, math::degrees ry,
										 math::degrees rz)
{
...
void World::initGlobalVBOs(GLuint* pDetailTexCoords, GLuint* pAlphaTexCoords)
{
	if (!*pDetailTexCoords && !*pAlphaTexCoords)
	{
		...
		gl.bufferData<GL_ARRAY_BUFFER>(*pDetailTexCoords, sizeof(temp), temp,
									   GL_STATIC_DRAW);
		...
		gl.bufferData<GL_ARRAY_BUFFER>(*pAlphaTexCoords, sizeof(temp), temp,
									   GL_STATIC_DRAW);
	}
}
...
void World::initDisplay()
{
	...
	{
		_horizon_render =
			std::make_unique<noggit::map_horizon::render>(horizon);
	}
...
void World::draw(
	math::matrix_4x4 const& model_view, math::matrix_4x4 const& projection,
	math::vector_3d const& cursor_pos, math::vector_4d const& cursor_color,
	int cursor_type, float brush_radius, bool show_unpaintable_chunks,
	bool draw_contour, float inner_radius_ratio, math::vector_3d const& ref_pos,
	float angle, float orientation, bool use_ref_pos, bool angled_mode,
	bool draw_paintability_overlay, bool draw_chunk_flag_overlay,
	bool draw_areaid_overlay, editing_mode terrainMode,
	math::vector_3d const& camera_pos, bool camera_moved, bool draw_mfbo,
	bool draw_wireframe, bool draw_lines, bool draw_terrain, bool draw_wmo,
	bool draw_water, bool draw_wmo_doodads, bool draw_models,
	bool draw_model_animations, bool draw_hole_lines, bool draw_models_with_box,
	bool draw_hidden_models, std::map<int, misc::random_color>& area_id_colors,
	bool draw_fog, eTerrainType ground_editing_brush, int water_layer,
	display_mode display)
{
	...
	if (!_m2_program)
	{
		_m2_program.reset(new opengl::program{
	...
	{
		_m2_instanced_program.reset(new opengl::program{
			{GL_VERTEX_SHADER,
			 opengl::shader::src_from_qrc("m2_vs", {"instanced"})},
			{GL_FRAGMENT_SHADER, opengl::shader::src_from_qrc("m2_fs")}});
	}
	if (!_m2_box_program)
	{
		_m2_box_program.reset(new opengl::program{
	...
	{
		_m2_ribbons_program.reset(new opengl::program{
	...
	// todo: figure out why I need to use a different light vector for the
	// terrain
	math::vector_3d terrain_light_dir = {-light_dir.z, light_dir.y,
										 -light_dir.x};

	math::vector_3d diffuse_color(skies->color_set[LIGHT_GLOBAL_DIFFUSE] *
								  outdoorLightStats.dayIntensity);
	math::vector_3d ambient_color(skies->color_set[LIGHT_GLOBAL_AMBIENT] *
								  outdoorLightStats.ambientIntensity);
	...
	if (display == display_mode::in_3D)
	{
				...
				[&](WMOInstance& wmo) {
					if (wmo.wmo->finishedLoading() && wmo.wmo->skybox)
					{
						...
						hadSky = wmo.wmo->draw_skybox(
							model_view, camera_pos, m2_shader, frustum,
							culldistance, animtime, draw_model_animations,
							wmo.extents[0], wmo.extents[1], wmo.group_extents);
					}
				},
				[&]() { return hadSky; });
		}
		...
		if (!hadSky)
		{
			skies->draw(model_view, projection, camera_pos, m2_shader, frustum,
						culldistance, animtime, draw_model_animations,
						outdoorLightStats);
		}
	}
	...
	{
		_horizon_render->draw(model_view, projection, &mapIndex,
							  skies->color_set[FOG_COLOR], culldistance,
							  frustum, camera_pos, display);
	}
	...
	if (draw_terrain)
	{
		...
		// the flag stays on if the last chunk drawn before leaving the editing
		// tool has it
		...
		mcnk_shader.uniform("wireframe_type",
							_settings->value("wireframe/type", 0).toInt());
		mcnk_shader.uniform(
			"wireframe_radius",
			_settings->value("wireframe/radius", 1.5f).toFloat());
		mcnk_shader.uniform("wireframe_width",
							_settings->value("wireframe/width", 1.f).toFloat());
		// !\ todo store the color somewhere ?
		QColor c = _settings->value("wireframe/color").value<QColor>();
		math::vector_4d wireframe_color(c.redF(), c.greenF(), c.blueF(),
										c.alphaF());
		mcnk_shader.uniform("wireframe_color", wireframe_color);

		mcnk_shader.uniform("draw_fog", (int)draw_fog);
		mcnk_shader.uniform("fog_color",
							math::vector_4d(skies->color_set[FOG_COLOR], 1));
		...
		for (MapIndex::tile_iterator<false> It = mapIndex.LoadedTilesStart();
			 It != mapIndex.LoadedTilesEnd(); It++)
		{
			MapTile* tile = *It;
			tile->draw(frustum, mcnk_shader, detailtexcoords, culldistance,
					   camera_pos, camera_moved, show_unpaintable_chunks,
					   draw_paintability_overlay, draw_chunk_flag_overlay,
					   draw_areaid_overlay, area_id_colors, animtime, display);
		}
	...
	if (cursor != cursor_mode::terrain && cursor != cursor_mode::none)
	{
		...
		if (terrainMode == editing_mode::ground &&
			ground_editing_brush == eTerrainType_Quadra)
		{
		...
		_cursor_render.draw(mode, mvp, cursor_color, cursor_pos, brush_radius,
							inner_radius_ratio);
	}

	if (terrainMode == editing_mode::object && has_multiple_model_selected())
	{
		_sphere_render.draw(mvp, _multi_select_pivot.value(), cursor_color,
							2.f);
	}
	...
	if (terrainMode == editing_mode::ground &&
		ground_editing_brush == eTerrainType_Vertex)
	{
		...
		{
			_sphere_render.draw(mvp, *pos, math::vector_4d(1.f, 0.f, 0.f, 1.f),
								0.5f);
		}
	...
	if (draw_doodads_wmo)
	{
		_model_instance_storage.for_each_wmo_instance([&](WMOInstance& wmo) {
			for (auto& doodad :
				 wmo.get_visible_doodads(frustum, culldistance, camera_pos,
										 draw_hidden_models, display))
			{
	...
	if (draw_models || draw_doodads_wmo)
	{
...

		{
				...
				for (auto& it : _models_by_filename)
				{
					if (draw_hidden_models || !it.second[0]->model->is_hidden())
					{
						it.second[0]->model->draw(
							model_view, it.second, m2_shader, frustum,
							culldistance, camera_pos, false, animtime,
							draw_model_animations, draw_models_with_box,
							model_with_particles, model_boxes_to_draw, display);
					}
				}
			}
			...
			if (draw_doodads_wmo)
			{
				...
				{
					it.second[0]->model->draw(
						model_view, it.second, m2_shader, frustum, culldistance,
						camera_pos, false, animtime, draw_model_animations,
						draw_models_with_box, model_with_particles,
						model_boxes_to_draw, display);
				}
			}
		}

		if (draw_models_with_box ||
			(draw_hidden_models && !model_boxes_to_draw.empty()))
		{
			...
			opengl::scoped::bool_setter<GL_LINE_SMOOTH, GL_TRUE> const
				line_smooth;
			gl.hint(GL_LINE_SMOOTH_HINT, GL_NICEST);
		...
		for (auto& selection : current_selection())
		{
			if (selection.which() == eEntry_Model)
			{
				auto model = GetFromSelection<selected_model_type>(selection);
	...
	// set anim time only once per frame
	{
		...
		math::vector_4d ocean_color_light(skies->color_set[OCEAN_COLOR_LIGHT],
										  skies->ocean_shallow_alpha());
		math::vector_4d ocean_color_dark(skies->color_set[OCEAN_COLOR_DARK],
										 skies->ocean_deep_alpha());
		math::vector_4d river_color_light(skies->color_set[RIVER_COLOR_LIGHT],
										  skies->river_shallow_alpha());
		math::vector_4d river_color_dark(skies->color_set[RIVER_COLOR_DARK],
										 skies->river_deep_alpha());
				...
				[&](WMOInstance& wmo) {
					...
					if (draw_hidden_models || !is_hidden)
					{
						wmo.draw(wmo_program, model_view, projection, frustum,
								 culldistance, camera_pos, is_hidden,
								 draw_wmo_doodads, draw_fog,
								 _liquid_render.value(), current_selection(),
								 animtime, skies->hasSkies(), display);
					}
				});
	...
	if (draw_water)
	{
		...
		{
			MapTile* tile = *It;
			tile->drawWater(frustum, culldistance, camera_pos, camera_moved,
							_liquid_render.value(), water_shader, animtime,
							water_layer, display);
		}
	...
	if (angled_mode || use_ref_pos)
	{
		...
		else if (use_ref_pos)
		{
			if (angled_mode)
			{
				...
				if (misc::dist(pos.x, pos.z, ref_pos.x, ref_pos.z) >
					10.f + radius)
				{
			...
			else
			{
				...
				_square_render.draw(mvp, pos, radius, math::degrees(0.f),
									math::degrees(0.f), color);
			}
		}
	}
...
selection_result World::intersect(math::matrix_4x4 const& model_view,
								  math::ray const& ray, bool pOnlyMap,
								  bool do_objects, bool draw_terrain,
								  bool draw_wmo, bool draw_models,
								  bool draw_hidden_models)
{
	...
	if (!pOnlyMap && do_objects)
	{
		if (draw_models)
		{
			_model_instance_storage.for_each_m2_instance(
				[&](ModelInstance& model_instance) {
					if (draw_hidden_models ||
						!model_instance.model->is_hidden())
					{
						model_instance.intersect(model_view, ray, &results,
												 animtime);
					}
				});
		}
...
{
	return for_maybe_chunk_at(
			   pos, [&](MapChunk* chunk) { return chunk->getAreaID(); })
		.value_or(-1);
}

void World::clearHeight(math::vector_3d const& pos)
{
	for_all_chunks_on_tile(pos, [](MapChunk* chunk) { chunk->clearHeight(); });
	for_all_chunks_on_tile(pos,
...
void World::setAreaID(math::vector_3d const& pos, int id, bool adt)
{
	...
	{
		for_all_chunks_on_tile(pos,
							   [&](MapChunk* chunk) { chunk->setAreaID(id); });
	}
	else
	{
...
bool World::for_all_chunks_in_range(math::vector_3d const& pos, float radius,
									Fun&& fun)
{
	bool changed(false);

	for (auto It = mapIndex.TilesInRangeStart(pos, radius);
		 It != mapIndex.TilesInRangeEnd(); It++)
	{
		MapTile* tile = *It;
									...
									Fun&& fun, Post&& post)
{
	...
	bool changed(for_all_chunks_in_range(pos, radius, [&](MapChunk* chunk) {
		if (fun(chunk))
		{
...
void World::changeShader(math::vector_3d const& pos,
						 math::vector_4d const& color, float change,
						 float radius, bool editMode)
{
...
math::vector_3d World::pickShaderColor(math::vector_3d const& pos)
{
	...
	for_all_chunks_in_range(pos, 0.1f, [&](MapChunk* chunk) {
...
void World::changeTerrain(math::vector_3d const& pos, float change,
						  float radius, int BrushType, float inner_radius)
{
	for_all_chunks_in_range(pos, radius,
							[&](MapChunk* chunk) {
								return chunk->changeTerrain(pos, change, radius,
															BrushType,
															inner_radius);
							},
							[this](MapChunk* chunk) { recalc_norms(chunk); });
...
void World::flattenTerrain(math::vector_3d const& pos, float remain,
						   float radius, int BrushType,
	...
	for_all_chunks_in_range(pos, radius,
							[&](MapChunk* chunk) {
								return chunk->flattenTerrain(
									pos, remain, radius, BrushType, mode,
									origin, angle, orientation);
							},
							[this](MapChunk* chunk) { recalc_norms(chunk); });
}

void World::blurTerrain(math::vector_3d const& pos, float remain, float radius,
						int BrushType, flatten_mode const& mode)
{
	for_all_chunks_in_range(
		pos, radius,
		[&](MapChunk* chunk) {
			return chunk->blurTerrain(
				pos, remain, radius, BrushType, mode,
				[this](float x, float z) -> std::optional<float> {
					math::vector_3d vec;
					bool res(GetVertex(x, z, &vec));
					if (res)
					{
						std::make_optional(vec.y);
					}
					return std::nullopt;
				});
		},
...
bool World::paintTexture(math::vector_3d const& pos, Brush* brush,
						 float strength, float pressure,
						 scoped_blp_texture_reference texture)
{
	return for_all_chunks_in_range(
		pos, brush->getRadius(), [&](MapChunk* chunk) {
			return chunk->paintTexture(pos, brush, strength, pressure, texture);
		});
}

bool World::sprayTexture(math::vector_3d const& pos, Brush* brush,
						 float strength, float pressure, float spraySize,
						 float sprayPressure,
						 scoped_blp_texture_reference texture)
{
	bool succ = false;
	...
	for (float pz = pos.z - spraySize; pz < pos.z + spraySize; pz += inc)
	{
		...
		{
			if ((sqrt(pow(px - pos.x, 2) + pow(pz - pos.z, 2)) <= spraySize) &&
				((rand() % 1000) < sprayPressure))
			{
				succ |= paintTexture({px, pos.y, pz}, brush, strength, pressure,
									 texture);
			}
		}
	}
...
bool World::replaceTexture(math::vector_3d const& pos, float radius,
						   scoped_blp_texture_reference const& old_texture,
						   scoped_blp_texture_reference new_texture)
{
	return for_all_chunks_in_range(pos, radius, [&](MapChunk* chunk) {
...
void World::overwriteTextureAtCurrentChunk(
	math::vector_3d const& pos, scoped_blp_texture_reference const& oldTexture,
...
{
	for_all_chunks_on_tile(
		pos, [&](MapChunk* chunk) { chunk->setHole(pos, true, hole); });
}
...
void World::for_chunk_at(math::vector_3d const& pos, Fun&& fun)
{
	...
	{
		mapIndex.setChanged(tile);
		fun(tile->getChunk((pos.x - tile->xbase) / CHUNKSIZE,
						   (pos.z - tile->zbase) / CHUNKSIZE));
	}
}

template <typename Fun>
auto World::for_maybe_chunk_at(math::vector_3d const& pos, Fun&& fun)
	-> std::optional<decltype(fun(nullptr))>
{
	MapTile* tile(mapIndex.getTile(pos));
	if (tile && tile->finishedLoading())
	{
		return fun(tile->getChunk((pos.x - tile->xbase) / CHUNKSIZE,
								  (pos.z - tile->zbase) / CHUNKSIZE));
	}
	else
	{
	...
	for (size_t z = 0; z < 64; z++)
	{
		for (size_t x = 0; x < 64; x++)
		{
			...
			bool unload = !mapIndex.tileLoaded(tile) &&
						  !mapIndex.tileAwaitingLoading(tile);
...
void World::addM2(std::string const& filename, math::vector_3d newPos,
				  ...
				  noggit::object_paste_params* paste_params)
{
	...
	std::uint32_t uid = _model_instance_storage.add_model_instance(
		std::move(model_instance), true);

	_models_by_filename[filename].push_back(
		_model_instance_storage.get_model_instance(uid).value());
}

void World::addWMO(std::string const& filename, math::vector_3d newPos,
...
std::uint32_t World::add_model_instance(ModelInstance model_instance,
										bool from_reloading)
{
	return _model_instance_storage.add_model_instance(std::move(model_instance),
													  from_reloading);
}
...
std::uint32_t World::add_wmo_instance(WMOInstance wmo_instance,
									  bool from_reloading)
{
	return _model_instance_storage.add_wmo_instance(std::move(wmo_instance),
													from_reloading);
}
...
void World::remove_models_if_needed(std::vector<uint32_t> const& uids)
{
	...
	for (uint32_t uid : uids)
	{
		...
		_model_instance_storage
			.unload_instance_and_remove_from_selection_if_necessary(uid);
	}

	// deselect the terrain when an adt is unloaded
	if (_current_selection.size() == 1 &&
		_current_selection.at(0).which() == eEntry_MapChunk)
	{
		reset_selection();
	}
...
void World::updateTilesEntry(selection_type const& entry, model_update type)
{
	if (entry.which() == eEntry_WMO)
	{
		updateTilesWMO(GetFromSelection<selected_wmo_type>(entry), type);
	}
	else if (entry.which() == eEntry_Model)
	{
		updateTilesModel(GetFromSelection<selected_model_type>(entry), type);
	}
}
...
unsigned int World::getMapID() { return mapIndex._map_id; }

void World::clearTextures(math::vector_3d const& pos)
{
	for_all_chunks_on_tile(pos,
						   [](MapChunk* chunk) { chunk->eraseTextures(); });
}
...
{
	for_all_chunks_on_tile(pos,
						   [](MapChunk* chunk) { chunk->clear_shadows(); });
}

void World::swapTexture(math::vector_3d const& pos,
...
{
	for_all_chunks_on_tile(
		pos, [](MapChunk* chunk) { chunk->texture_set->removeDuplicate(); });
}

void World::change_texture_flag(math::vector_3d const& pos,
								scoped_blp_texture_reference const& tex,
...
void World::paintLiquid(math::vector_3d const& pos, float radius, int liquid_id,
						bool add, math::radians const& angle,
						math::radians const& orientation, bool lock,
						math::vector_3d const& origin, bool override_height,
						bool override_liquid_id, float opacity_factor)
{
	for_all_chunks_in_range(pos, radius, [&](MapChunk* chunk) {
		chunk->liquid_chunk()->paintLiquid(
			pos, radius, liquid_id, add, angle, orientation, lock, origin,
			override_height, override_liquid_id, chunk, opacity_factor);
		return true;
	});
}
...
{
	for_tile_at(pos, [&](MapTile* tile) { tile->Water.setType(type, layer); });
}
...
bool World::isUnderMap(math::vector_3d const& pos)
{
	...
	if (mapIndex.tileLoaded(tile))
	{
		...
		return (mapIndex.getTile(tile)
					->getChunk(chnkX, chnkZ)
					->getMinHeight()) > pos.y + 2.0f;
	}
						   ...
						   math::degrees vertex_orientation)
{
	...
	{
		v->y =
			misc::angledHeight(ref_pos, *v, vertex_angle, vertex_orientation);
	}
	updateSelectedVertices();
}
...
void World::update_models_by_filename()
{
	...
	_model_instance_storage.for_each_m2_instance(
		[&](ModelInstance& model_instance) {
			_models_by_filename[model_instance.model->filename].push_back(
				&model_instance);
