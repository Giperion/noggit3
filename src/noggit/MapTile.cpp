...
MapTile::MapTile(int pX, int pZ, std::string const& pFilename, bool pBigAlpha,
				 bool pLoadModels, bool use_mclq_green_lava,
				 bool reloading_tile, World* world, tile_mode mode)
...
MapTile::~MapTile() { _world->remove_models_if_needed(uids); }
...
void MapTile::finishLoading()
{
	...
	{
		char const* lCurPos =
			reinterpret_cast<char const*>(theFile.getPointer());
		char const* lEnd = lCurPos + size;

		while (lCurPos < lEnd)
		{
			mTextureFilenames.push_back(
				noggit::mpq::normalized_filename(std::string(lCurPos)));
	...
	if (_load_models)
	{
		...
		ENTRY_MDDF const* mddf_ptr =
			reinterpret_cast<ENTRY_MDDF const*>(theFile.getPointer());
		...
		ENTRY_MODF const* modf_ptr =
			reinterpret_cast<ENTRY_MODF const*>(theFile.getPointer());
	...
	if (mFlags & 1)
	{
		...
		const float xPositions[] = {this->xbase, this->xbase + 266.0f,
									this->xbase + 533.0f};
		const float yPositions[] = {this->zbase, this->zbase + 266.0f,
									this->zbase + 533.0f};
		...
		for (int y = 0; y < 3; y++)
		{
			for (int x = 0; x < 3; x++)
			{
				...
				mMinimumValues[pos] = {
					xPositions[x], static_cast<float>(z.first), yPositions[y]};
				mMaximumValues[pos] = {
					xPositions[x], static_cast<float>(z.second), yPositions[y]};
			}
		}
	}
	...
	if (_load_models)
	{
		...
		{
			add_model(_world->add_wmo_instance(
				WMOInstance(mWMOFilenames[object.nameID], &object),
				_tile_is_being_reloaded));
		}
		...
		{
			add_model(_world->add_model_instance(
				ModelInstance(mModelFilenames[model.nameID], &model),
				_tile_is_being_reloaded));
		}
	...
	for (int nextChunk = 0; nextChunk < 256; ++nextChunk)
	{
		...
		mChunks[nextChunk / 16][nextChunk % 16] =
			std::make_unique<MapChunk>(this, &theFile, mBigAlpha, _mode);
	}
...
bool MapTile::isTile(int pX, int pZ) { return pX == index.x && pZ == index.z; }
...
float MapTile::getMaxHeight()
{
	...
	for (int nextChunk = 0; nextChunk < 256; ++nextChunk)
	{
		maxHeight = std::max(mChunks[nextChunk / 16][nextChunk % 16]->vmax.y,
							 maxHeight);
	}
	return maxHeight;
}
				   ...
				   int animtime, display_mode display)
{
	...
	for (int j = 0; j < 16; ++j)
	{
		...
		{
			mChunks[j][i]->draw(
				frustum, mcnk_shader, tex_coord_vbo, cull_distance, camera,
				need_visibility_update, show_unpaintable_chunks,
...
{
	static std::vector<std::uint8_t> const indices = {
		4, 1, 2, 5, 8, 7, 6, 3, 0, 1, 0, 3, 6, 7, 8, 5, 2, 1};
	...
	if (!_mfbo_buffer_are_setup)
	{
		...
		gl.bufferData<GL_ARRAY_BUFFER>(_mfbo_bottom_vbo,
									   ...
									   mMinimumValues, GL_STATIC_DRAW);
		gl.bufferData<GL_ARRAY_BUFFER>(_mfbo_top_vbo,
									   9 * sizeof(math::vector_3d),
									   mMaximumValues, GL_STATIC_DRAW);

		{
			opengl::scoped::vao_binder const _(_mfbo_bottom_vao);
			opengl::scoped::buffer_binder<GL_ARRAY_BUFFER> const vbo_binder(
				_mfbo_bottom_vbo);
...

	{
		...
		gl.drawElements(GL_TRIANGLE_FAN, indices.size(), GL_UNSIGNED_BYTE,
						indices.data());
	}
...
void MapTile::drawWater(math::frustum const& frustum,
						...
						const math::vector_3d& camera, bool camera_moved,
						liquid_render& render,
						opengl::scoped::use_program& water_shader, int animtime,
						int layer, display_mode display)
{
	if (!Water.hasData(0))
	{
	...
	Water.draw(frustum, cull_distance, camera, camera_moved, render,
			   water_shader, animtime, layer, display);
}
...
std::vector<MapChunk*> MapTile::chunks_in_range(math::vector_3d const& pos,
												float radius) const
{
	...
	for (size_t ty(0); ty < 16; ++ty)
	{
		...
		{
			if (misc::getShortestDist(pos.x, pos.z, mChunks[ty][tx]->xbase,
									  mChunks[ty][tx]->zbase,
									  CHUNKSIZE) <= radius)
			{
...
bool MapTile::GetVertex(float x, float z, math::vector_3d* V)
{
	...
	return xcol >= 0 && xcol <= 15 && ycol >= 0 && ycol <= 15 &&
		   mChunks[ycol][xcol]->GetVertex(x, z, V);
}
...
void MapTile::saveTile(World* world)
{
	...
	for (std::uint32_t uid : uids)
	{
		...
		else
		{
			...
			{
				lObjectInstances.emplace_back(
					*GetFromSelection<selected_wmo_type>(model.value()));
			}
			else
			{
				lModelInstances.emplace_back(
					*GetFromSelection<selected_model_type>(model.value()));
			}
		}
	}
	...
	for (int i = 0; i < 16; ++i)
	{
		for (int j = 0; j < 16; ++j)
		{
			...
			{
				if (lTextures.find(mChunks[i][j]->texture_set->filename(tex)) ==
					lTextures.end())
				{
					lTextures.emplace(mChunks[i][j]->texture_set->filename(tex),
									  -1);
				}
			}
		}
	...
	lID = 0;
	for (auto& texture : lTextures) texture.second = lID++;
	...
	*(lADTFile.GetPointer<int>(8)) = 18;
	...
	lADTFile.GetPointer<MHDR>(lMHDR_Position + 8)->mcin =
		lCurrentPosition - 0x14;
	...
	lADTFile.GetPointer<MHDR>(lMHDR_Position + 8)->mtex =
		lCurrentPosition - 0x14;
	...
	{
		lADTFile.Insert(lCurrentPosition, texture.first.size() + 1,
						texture.first.c_str());
	...
	lADTFile.GetPointer<MHDR>(lMHDR_Position + 8)->mmdx =
		lCurrentPosition - 0x14;
	...
	{
		it->second.filenamePosition =
			lADTFile.GetPointer<sChunkHeader>(lMMDX_Position)->mSize;
		lADTFile.Insert(lCurrentPosition, it->first.size() + 1,
						misc::normalize_adt_filename(it->first).c_str());
	...
	lADTFile.GetPointer<MHDR>(lMHDR_Position + 8)->mmid =
		lCurrentPosition - 0x14;
	...
	lADTFile.GetPointer<MHDR>(lMHDR_Position + 8)->mwmo =
		lCurrentPosition - 0x14;
	...
	for (auto& object : lObjects)
	{
		object.second.filenamePosition =
			lADTFile.GetPointer<sChunkHeader>(lMWMO_Position)->mSize;
		lADTFile.Insert(lCurrentPosition, object.first.size() + 1,
						misc::normalize_adt_filename(object.first).c_str());
	...
	lADTFile.GetPointer<MHDR>(lMHDR_Position + 8)->mwid =
		lCurrentPosition - 0x14;
	...
	lADTFile.GetPointer<MHDR>(lMHDR_Position + 8)->mddf =
		lCurrentPosition - 0x14;

	// MDDF data
	ENTRY_MDDF* lMDDF_Data =
		lADTFile.GetPointer<ENTRY_MDDF>(lCurrentPosition + 8);

	if (world->mapIndex.sort_models_by_size_class())
	{
		std::sort(lModelInstances.begin(), lModelInstances.end(),
				  [](ModelInstance const& m1, ModelInstance const& m2) {
	...
	for (auto const& model : lModelInstances)
	{
		...
		{
			LogError << "There is a problem with saving the doodads. We have a "
	...
	lADTFile.GetPointer<MHDR>(lMHDR_Position + 8)->modf =
		lCurrentPosition - 0x14;

	// MODF data
	ENTRY_MODF* lMODF_Data =
		lADTFile.GetPointer<ENTRY_MODF>(lCurrentPosition + 8);
	...
	for (auto const& object : lObjectInstances)
	{
		...
		{
			LogError << "There is a problem with saving the objects. We have "
	...
	for (int y = 0; y < 16; ++y)
	{
		for (int x = 0; x < 16; ++x)
		{
			mChunks[y][x]->save(lADTFile, lCurrentPosition, lMCIN_Position,
	...
	if (mFlags & 1)
	{
		...
		lADTFile.GetPointer<MHDR>(lMHDR_Position + 8)->mfbo =
			lCurrentPosition - 0x14;

		int16_t* lMFBO_Data =
			lADTFile.GetPointer<int16_t>(lCurrentPosition + 8);
  ...
  if (!mTextureEffects.empty()) {
	...
	lADTFile.Extend(
		lCurrentPosition -
		lADTFile.data.size());  // cleaning unused nulls at the end of file
