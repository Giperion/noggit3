...
namespace noggit
{
	namespace ui
	{
		...
		QSize minimap_widget::sizeHint() const { return QSize(700, 700); }
		...
		void minimap_widget::paintEvent(QPaintEvent*)
		{
			...
			const int smaller_side(
				(qMin(rect().width(), rect().height()) / 64) * 64);
					...
					for (size_t i(0); i < 64; ++i)
					{
						for (size_t j(0); j < 64; ++j)
						{
							...
							if (world()->mapIndex.hasTile(tile))
							{
								...
								{
									if (world()->mapIndex.has_unsaved_changes(
											tile))
									{
										changed = true;
									}
									...
									painter.setPen(
										QColor::fromRgbF(0.f, 0.f, 0.f, 0.6f));
								}
							...
							{
								painter.setPen(
									QColor::fromRgbF(1.0f, 1.0f, 1.0f, 0.05f));
							}

							painter.drawRect(QRect(tile_size * i, tile_size * j,
												   tile_size, tile_size));

							if (changed)
							{
								painter.setPen(
									QColor::fromRgbF(1.0f, 1.0f, 0.0f, 1.f));
								painter.drawRect(
									QRect(tile_size * i + 1, tile_size * j + 1,
			...
			{
				//! \todo Draw something so user realizes this will become the
				//! minimap.
				painter.setPen(Qt::black);
				painter.setFont(QFont("Arial", 30));
				painter.drawText(drawing_rect, Qt::AlignCenter,
		...
		void minimap_widget::mouseDoubleClickEvent(QMouseEvent* event)
		{
			...
			emit map_clicked(::math::vector_3d(
				(event->pos().x() / float(tile_size)) * TILESIZE, 0.0f,
		...
		void minimap_widget::mouseMoveEvent(QMouseEvent* event)
		{
			if (world())
			{
				...
				std::string str("ADT: " + std::to_string(x / tile_size) + "_" +
								std::to_string(y / tile_size));

				QToolTip::showText(mapToGlobal(QPoint(x, y + 5)),
