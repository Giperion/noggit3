...
namespace noggit
{
	namespace ui
	{
			  ...
			  _flatten_mode(true, true)
		{
			...
			QGridLayout* flatten_type_layout(
				new QGridLayout(flatten_type_group));
			...
			flatten_blur_layout->addWidget(
				_lock_up_checkbox = new QCheckBox(this), 0, 0);
			flatten_blur_layout->addWidget(
				_lock_down_checkbox = new QCheckBox(this), 0, 1);

			_lock_up_checkbox->setChecked(_flatten_mode.raise);
			_lock_up_checkbox->setText("raise");
			_lock_up_checkbox->setToolTip(
				"Raise the terrain when using the tool");
			_lock_down_checkbox->setChecked(_flatten_mode.lower);
			_lock_down_checkbox->setText("lower");
			_lock_down_checkbox->setToolTip(
				"Lower the terrain when using the tool");
			...
			_orientation_dial->setSliderPosition(
				_orientation - 90);  // to get ingame orientation
			...
			_lock_h->setRange(std::numeric_limits<float>::lowest(),
			...
			connect(_type_button_box,
					qOverload<int>(&QButtonGroup::buttonClicked),
					[&](int id) { _flatten_type = id; });
			...
			connect(_lock_up_checkbox, &QCheckBox::stateChanged,
					[&](int state) { _flatten_mode.raise = state; });

			connect(_lock_down_checkbox, &QCheckBox::stateChanged,
					[&](int state) { _flatten_mode.lower = state; });
			...
			connect(_lock_x, qOverload<double>(&QDoubleSpinBox::valueChanged),
					[&](double v) { _lock_pos.x = v; });

			connect(_lock_h, qOverload<double>(&QDoubleSpinBox::valueChanged),
					[&](double v) { _lock_pos.y = v; });

			connect(_lock_z, qOverload<double>(&QDoubleSpinBox::valueChanged),
					[&](double v) { _lock_pos.z = v; });
		}

		void flatten_blur_tool::flatten(World* world,
										math::vector_3d const& cursor_pos,
										float dt)
		{
			world->flattenTerrain(
				cursor_pos, 1.f - pow(0.5f, dt * _speed), _radius,
				_flatten_type, _flatten_mode,
				use_ref_pos() ? _lock_pos : cursor_pos,
				math::degrees(angled_mode() ? _angle : 0.0f),
				math::degrees(angled_mode() ? _orientation : 0.0f));
		}

		void flatten_blur_tool::blur(World* world,
									 math::vector_3d const& cursor_pos,
									 float dt)
		{
			world->blurTerrain(cursor_pos, 1.f - pow(0.5f, dt * _speed),
							   _radius, _flatten_type, _flatten_mode);
		}
