...
namespace noggit
{
	namespace ui
	{
		main_window::main_window()
			: QMainWindow(nullptr), _null_widget(new QWidget(this))
		{
		...
		void main_window::check_uid_then_enter_map(math::vector_3d pos,
												   ...
												   bool from_bookmark)
		{
...
#ifdef USE_MYSQL_UID_STORAGE
			bool use_mysql =
				settings.value("project/mysql/enabled", false).toBool();
			...
			if (uid_storage::hasMaxUIDStored(_world->getMapID()))
			{
				if (settings.value("uid_startup_check", true).toBool())
				{
					enterMapAt(pos, camera_pitch, camera_yaw,
							   uid_fix_mode::max_uid, from_bookmark);
				}
				else
				{
					_world->mapIndex.loadMaxUID();
					enterMapAt(pos, camera_pitch, camera_yaw,
							   uid_fix_mode::none, from_bookmark);
				}
			}
#endif
			else
			{
				auto uidFixWindow(
					new uid_fix_window(pos, camera_pitch, camera_yaw));
				uidFixWindow->show();

				connect(uidFixWindow, &noggit::ui::uid_fix_window::fix_uid,
						[this, from_bookmark](
							math::vector_3d pos, math::degrees camera_pitch,
							math::degrees camera_yaw, uid_fix_mode uid_fix) {
		...
		void main_window::enterMapAt(math::vector_3d pos,
									 math::degrees camera_pitch,
									 math::degrees camera_yaw,
									 uid_fix_mode uid_fix, bool from_bookmark)
		{
			auto mapview(new MapView(camera_yaw, camera_pitch, pos, this,
									 std::move(_world), uid_fix,
									 from_bookmark));
			connect(mapview, &MapView::uid_fix_failed,
					[this]() { prompt_uid_fix_failure(); });
		...
		void main_window::loadMap(int mapID)
		{
			...
			for (DBCFile::Iterator it = gMapDB.begin(); it != gMapDB.end();
				 ++it)
			{
				if (it->getInt(MapDB::MapID) == mapID)
				{
					_world = std::make_unique<World>(
						it->getString(MapDB::InternalName), mapID);
		...
		void main_window::build_menu()
		{
			...
			std::array<QListWidget*, 5> type_to_table{
				{continents_table, dungeons_table, raids_table,
				 battlegrounds_table, arenas_table}};
			...
			for (DBCFile::Iterator i = gMapDB.begin(); i != gMapDB.end(); ++i)
			{
				...
				if (e.areaType < 0 || e.areaType > 4 ||
					!World::IsEditableWorld(e.mapID))
					continue;

				auto item(
					new QListWidgetItem(QString::number(e.mapID) + " - " +
											QString::fromUtf8(e.name.c_str()),
			...
			{
				auto item(
					new QListWidgetItem(entry.name.c_str(), bookmarks_table));
			...
			QObject::connect(
				bookmarks_table, &QListWidget::itemDoubleClicked,
				[this](QListWidgetItem* item) {
					auto& entry(
						mBookmarks.at(item->data(Qt::UserRole).toInt()));

					_world.reset();

					for (DBCFile::Iterator it = gMapDB.begin();
						 it != gMapDB.end(); ++it)
					{
						if (it->getInt(MapDB::MapID) == entry.mapID)
						{
							_world = std::make_unique<World>(
								it->getString(MapDB::InternalName),
								entry.mapID);
							check_uid_then_enter_map(
								entry.pos, math::degrees(entry.camera_pitch),
								math::degrees(entry.camera_yaw), true);
		...
		void main_window::createBookmarkList()
		{
			...
			BookmarkEntry b;
			int mapID = -1;
			while (f >> mapID >> b.pos.x >> b.pos.y >> b.pos.z >>
				   b.camera_yaw >> b.camera_pitch >> areaID)
			{
				if (mapID == -1)
				{
					continue;
				...
				temp << MapDB::getMapName(mapID) << ": "
					 << AreaDB::getAreaName(areaID);
				b.name = temp.str();
				b.mapID = mapID;
		...
		void main_window::prompt_exit(QCloseEvent* event)
		{
			...
			prompt.setWindowFlags(Qt::CustomizeWindowHint |
								  Qt::WindowTitleHint);
		...
		void main_window::prompt_uid_fix_failure()
		{
			...
			QMessageBox::critical(nullptr, "UID fix failed",
								  "The UID fix couldn't be done because some "
								  "models were missing or fucked up.\n"
