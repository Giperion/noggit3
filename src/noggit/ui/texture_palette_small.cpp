...
namespace noggit
{
	namespace ui
	{
		...
		{
			if (event->button() == Qt::LeftButton) _start_pos = event->pos();
		...
		texture_palette_small::texture_palette_small(QWidget* parent)
			: widget(parent), layout(new ::QGridLayout(this))
		{
			...
			connect(
				_texture_list, &QListWidget::itemClicked, this,
			...
			connect(_add_button, &QAbstractButton::clicked, this,
					&texture_palette_small::addTexture);

			_remove_button = new QPushButton(this);
			_remove_button->setIcon(
				font_awesome_icon(font_awesome::timescircle));
			button_layout->addWidget(_remove_button);
			connect(_remove_button, &QAbstractButton::clicked, this,
					&texture_palette_small::removeSelectedTexture);
		...
		{
			if (_texture_paths.size() > 12) return;

			std::string filename;
			if (noggit::ui::selected_texture::get())
				filename =
					noggit::ui::selected_texture::get().value()->filename;
			else
		...
		void texture_palette_small::addTextureByFilename(
			const std::string& filename)
		{
			...
			for (auto path : _texture_paths)
				if (path == display_name.toStdString()) return;

			_texture_paths.emplace(display_name.toStdString());

			QListWidgetItem* list_item = new QListWidgetItem(_texture_list);
			list_item->setIcon(render_blp_to_pixmap(
				filename, _texture_list->iconSize().width(),
				_texture_list->iconSize().height()));
			...
			if (_texture_paths.size() == 12) _add_button->setDisabled(true);
		}

		void texture_palette_small::removeTexture(QString filename)
		{
			filename.remove("tileset/");
			QList<QListWidgetItem*> tilesets =
				_texture_list->findItems(filename, Qt::MatchExactly);
		...
		{
			setFixedSize(QSize(std::max(static_cast<size_t>(170 * 3),
										65 + (106 * _texture_paths.size())),
							   132));
		}

		void texture_palette_small::dragEnterEvent(QDragEnterEvent* event)
		{
			if (event->mimeData()->hasText() && _texture_paths.size() < 12 &&
				(_texture_paths.find(event->mimeData()
										 ->text()
										 .remove("tileset/")
										 .toStdString()) ==
				 _texture_paths.end()))
