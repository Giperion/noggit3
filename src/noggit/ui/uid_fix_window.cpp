	...
	namespace ui
	{
			...
			: QDialog(nullptr)
		{
			...
			auto label1 = new QLabel(
				"<big><p>In order to avoid issues with duplicating/missing "
			...
			auto label2 = new QLabel(
				"<hr><h2>Recommended for edited/custom maps</h2>"
				"<p>Takes more time than the max uid method but "
				"it will fix any model duplication, collision and culling "
				"issue "
			...
			auto label3 = new QLabel(
				"<hr><font color=red><h2>/!\\ NOT RECOMMENDED</h2>"
				...
				"Any <b>missing or fuckported</b> model will lead to collision "
				"and culling issues. "
				"Don't come complaining if you encounter any of those issues "
				"afterward.",
				this);
			label3->setWordWrap(true);
			layout->addWidget(label3);

			auto fix_all_fuckporting_edition(
				new QPushButton("Fix all UIDs but potentially fuck up "
								"collisions and culling for models",
								this));
			layout->addWidget(fix_all_fuckporting_edition);

			connect(get_max, &QPushButton::clicked, [=] {
				hide();
				emit fix_uid(pos, camera_pitch, camera_yaw,
							 uid_fix_mode::max_uid);
				deleteLater();
			});
			...
			connect(fix_all, &QPushButton::clicked, [=] {
				hide();
				emit fix_uid(pos, camera_pitch, camera_yaw,
							 uid_fix_mode::fix_all_fail_on_model_loading_error);
				deleteLater();
			});
			...
			connect(fix_all_fuckporting_edition, &QPushButton::clicked, [=] {
				hide();
				emit fix_uid(pos, camera_pitch, camera_yaw,
							 uid_fix_mode::fix_all_fuckporting_edition);
				deleteLater();
			});
		}
	}  // namespace ui
