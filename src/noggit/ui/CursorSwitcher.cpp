...
namespace noggit
{
	namespace ui
	{
		cursor_switcher::cursor_switcher(
			QWidget* parent, math::vector_4d& color,
			...
			: widget(parent)
		{
			...
			group->addButton(butt_sphere,
							 static_cast<int>(cursor_mode::sphere));
			group->addButton(butt_terrain_disk,
							 static_cast<int>(cursor_mode::terrain));
			...
			color_picker->setDisplayMode(
				color_widgets::ColorSelector::AllAlpha);
