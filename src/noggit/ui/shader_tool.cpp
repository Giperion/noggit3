...
namespace noggit
{
	namespace ui
	{
		shader_tool::shader_tool(math::vector_4d& color, QWidget* parent)
			: QWidget(parent), _radius(15.0f), _speed(1.0f), _color(color)
		{
			...
			_color_palette->setSizePolicy(
				QSizePolicy(QSizePolicy::Preferred, QSizePolicy::Preferred));
			layout->addRow(_color_palette);

			auto info_label(
				new QLabel("drag&drop colors to select them", this));
			info_label->setAlignment(Qt::AlignCenter);

			layout->addRow(info_label);

			QObject::connect(_slide_saturation,
							 &color_widgets::GradientSlider::valueChanged, this,
							 &shader_tool::set_hsv);
			QObject::connect(_slide_value,
							 &color_widgets::GradientSlider::valueChanged, this,
							 &shader_tool::set_hsv);
			QObject::connect(_slide_hue,
							 &color_widgets::HueSlider::valueChanged, this,
							 &shader_tool::set_hsv);

			QObject::connect(color_wheel,
							 &color_widgets::ColorWheel::colorSelected, this,
							 &shader_tool::update_color_widgets);
			QObject::connect(color_picker,
							 &color_widgets::ColorSelector::colorChanged, this,
							 &shader_tool::update_color_widgets);

			QObject::connect(_slide_saturation, SIGNAL(valueChanged(int)),
							 _spin_saturation, SLOT(setValue(int)));
			QObject::connect(_slide_value, SIGNAL(valueChanged(int)),
							 _spin_value, SLOT(setValue(int)));
			QObject::connect(_slide_hue, SIGNAL(valueChanged(int)), _spin_hue,
							 SLOT(setValue(int)));

			QObject::connect(_spin_saturation, SIGNAL(valueChanged(int)),
							 _slide_saturation, SLOT(setValue(int)));
			QObject::connect(_spin_hue, SIGNAL(valueChanged(int)), _slide_hue,
							 SLOT(setValue(int)));
			QObject::connect(_spin_value, SIGNAL(valueChanged(int)),
							 _slide_value, SLOT(setValue(int)));

			connect(_color_palette,
					&color_widgets::ColorListWidget::color_added, [&]() {
						_color_palette->setColorAt(
							_color_palette->colors().length() - 1,
							color_wheel->color());
					});
			...
			connect(color_wheel, &color_widgets::ColorWheel::colorChanged,
					color_picker, &color_widgets::ColorSelector::setColor);

			setMinimumWidth(sizeHint().width());
		}

		void shader_tool::changeShader(World* world, math::vector_3d const& pos,
									   float dt, bool add)
		{
		...
		void shader_tool::addColorToPalette() { _color_palette->append(); }
		...
		void shader_tool::set_hsv()
		{
			if (!signalsBlocked())
			{
				color_wheel->setColor(QColor::fromHsv(
					_slide_hue->value(), _slide_saturation->value(),
					_slide_value->value()));
		...
		void shader_tool::update_color_widgets()
		{
			...
			_slide_saturation->setValue(
				qRound(color_wheel->saturation() * 255.0));
			_spin_saturation->setValue(_slide_saturation->value());
			_slide_saturation->setFirstColor(
				QColor::fromHsvF(color_wheel->hue(), 0, color_wheel->value()));
			_slide_saturation->setLastColor(
				QColor::fromHsvF(color_wheel->hue(), 1, color_wheel->value()));

			_slide_value->setValue(qRound(color_wheel->value() * 255.0));
			_spin_value->setValue(_slide_value->value());
			_slide_value->setFirstColor(QColor::fromHsvF(
				color_wheel->hue(), color_wheel->saturation(), 0));
			_slide_value->setLastColor(QColor::fromHsvF(
				color_wheel->hue(), color_wheel->saturation(), 1));

			blockSignals(blocked);
			for (QWidget* w : findChildren<QWidget*>()) w->blockSignals(false);
		}

		QSize shader_tool::sizeHint() const { return QSize(215, height()); }
