...
namespace noggit
{
	namespace ui
	{
			...
			: QWidget(parent)
		{
			...
			_position_x->setRange(std::numeric_limits<float>::lowest(),
								  std::numeric_limits<float>::max());
			_position_x->setDecimals(5);
			_position_z->setRange(std::numeric_limits<float>::lowest(),
								  std::numeric_limits<float>::max());
			_position_z->setDecimals(5);
			_position_y->setRange(std::numeric_limits<float>::lowest(),
								  std::numeric_limits<float>::max());
			_position_y->setDecimals(5);

			_scale->setRange(ModelInstance::min_scale(),
			...
			connect(_rotation_x, &QDoubleSpinBox::editingFinished, [&, world] {
			...
			connect(_rotation_z, &QDoubleSpinBox::editingFinished, [&, world] {
				if (world->has_multiple_model_selected())
				{
					// avoid rotation changes when losing focus
			...
			connect(_rotation_y, &QDoubleSpinBox::editingFinished, [&, world] {
				if (world->has_multiple_model_selected())
				{
					// avoid rotation changes when losing focus
			...
			connect(_position_x,
					qOverload<double>(&QDoubleSpinBox::valueChanged),
					[&, world](double v) {
						world->set_selected_models_pos(v, _position_y->value(),
													   _position_z->value());
					});
			connect(_position_z,
					qOverload<double>(&QDoubleSpinBox::valueChanged),
					[&, world](double v) {
						world->set_selected_models_pos(_position_x->value(),
													   _position_y->value(), v);
					});
			connect(_position_y,
					qOverload<double>(&QDoubleSpinBox::valueChanged),
					[&, world](double v) {
						world->set_selected_models_pos(_position_x->value(), v,
													   _position_z->value());
					});
					...
					[world](double v) {
						if (!world->has_multiple_model_selected())
						{
							world->scale_selected_models(
								v, World::m2_scaling_type::set);
						}
					});
			connect(_scale, &QDoubleSpinBox::editingFinished, [&, world] {
				if (world->has_multiple_model_selected())
				{
					...
					{
						world->scale_selected_models(
							_scale->value(), World::m2_scaling_type::mult);
					}
					else  // reset value
					{
		...
		void rotation_editor::updateValues(World* world)
		{
			...
			if (world->has_multiple_model_selected())
			{
				...
				// default value for rotation and scaling, affect the models
