...
namespace noggit
{
	namespace ui
	{
		texture_picker::texture_picker(current_texture* current_texture_window,
									   QWidget* parent)
			: widget(parent), layout(new ::QGridLayout(this)), _chunk(nullptr)
		{
			...
			btn_right->setIcon(
				font_awesome_icon(font_awesome::angledoubleright));
		...
		void texture_picker::getTextures(selection_type lSelection)
		{
			...
			{
				MapChunk* chunk =
					GetFromSelection<selected_chunk_type>(lSelection).chunk;
				_chunk = chunk;
				update(false);
			}
		}

		void texture_picker::setTexture(size_t id,
