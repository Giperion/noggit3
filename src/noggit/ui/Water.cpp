...
namespace noggit
{
	namespace ui
	{
		water::water(unsigned_int_property* current_layer,
					 bool_toggle_property* display_all_layers, QWidget* parent)
			: QWidget(parent),
			  _liquid_id(5),
			  _radius(10.0f),
			  _angle(10.0f),
			  ...
			  tile(0, 0)
		{
			...
			for (DBCFile::Iterator i = gLiquidTypeDB.begin();
				 i != gLiquidTypeDB.end(); ++i)
			{
				...
				waterType->addItem(QString::fromUtf8(ss.str().c_str()),
								   QVariant(liquid_id));
			}
			...
			connect(&_angled_mode, &bool_toggle_property::changed, angle_group,
					&QGroupBox::setChecked);
			connect(angle_group, &QGroupBox::toggled, &_angled_mode,
					&bool_toggle_property::set);
			...
			_x_spin->setRange(std::numeric_limits<float>::lowest(),
							  std::numeric_limits<float>::max());
			_z_spin->setRange(std::numeric_limits<float>::lowest(),
							  std::numeric_limits<float>::max());
			_h_spin->setRange(std::numeric_limits<float>::lowest(),
							  std::numeric_limits<float>::max());
			...
			connect(&_locked, &bool_toggle_property::changed, lock_group,
					&QGroupBox::setChecked);
			connect(lock_group, &QGroupBox::toggled, &_locked,
					&bool_toggle_property::set);
			...
			override_layout->addWidget(
				new checkbox("Liquid ID", &_override_liquid_id, this));
			override_layout->addWidget(
				new checkbox("Height", &_override_height, this));
			...
			layout->addRow(new pushbutton("Regen ADT opacity", [this] {
				emit regenerate_water_opacity(get_opacity_factor());
			}));
			layout->addRow(
				new pushbutton("Crop water", [this] { emit crop_water(); }));

			auto layer_group(new QGroupBox("Layers", this));
			auto layer_layout(new QFormLayout(layer_group));

			layer_layout->addRow(
				new checkbox("Show all layers", display_all_layers));
			...
			connect(current_layer, &unsigned_int_property::changed, waterLayer,
					&QSpinBox::setValue);
		...
		void water::updateData()
		{
			std::stringstream mt;
			mt << _liquid_id << " - "
			   << LiquidTypeDB::getLiquidName(_liquid_id);
		...
		void water::paintLiquid(World* world, math::vector_3d const& pos,
								bool add)
		{
			world->paintLiquid(
				pos, _radius, _liquid_id, add,
				math::degrees(_angled_mode.get() ? _angle : 0.0f),
				math::degrees(_angled_mode.get() ? _orientation : 0.0f),
				_locked.get(), _lock_pos, _override_height.get(),
				_override_liquid_id.get(), get_opacity_factor());
		}
		...
		void water::toggle_lock() { _locked.toggle(); }

		void water::toggle_angled_mode() { _angled_mode.toggle(); }
		...
		float water::get_opacity_factor() const
		{
			switch (_opacity_mode)
			{
				...
				case ocean_opacity:
					return 0.007f;
				case custom_opacity:
					return _custom_opacity_factor;
			}
		}

		QSize water::sizeHint() const { return QSize(215, height()); }
	}  // namespace ui
}  // namespace noggit
