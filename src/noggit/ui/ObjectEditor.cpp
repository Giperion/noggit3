...
namespace noggit
{
	namespace ui
	{
		object_editor::object_editor(
			...
			object_paste_params* paste_params, QWidget* parent)
			: QWidget(parent),
			  ...
			  pasteMode(PASTE_ON_TERRAIN)
		{
			...
			rotation_group->setChecked(
				_settings->value("model/random_rotation", false).toBool());
			tilt_group->setCheckable(true);
			tilt_group->setChecked(
				_settings->value("model/random_tilt", false).toBool());
			scale_group->setCheckable(true);
			scale_group->setChecked(
				_settings->value("model/random_size", false).toBool());

			QCheckBox* copyAttributesCheck =
				new QCheckBox("Copy rotation, tilt, and scale", this);
			...
			auto object_movement_box(
				new QGroupBox("Single Selection Movement", this));
			auto object_movement_layout = new QFormLayout(object_movement_box);

			// single model selection
			auto object_movement_cb(
				new checkbox("Mouse move follow\ncursor on the ground",
							 move_model_to_cursor_position, this));

			object_movement_layout->addRow(object_movement_cb);

			// multi model selection
			auto multi_select_movement_box(
				new QGroupBox("Multi Selection Movement", this));
			auto multi_select_movement_layout =
				new QFormLayout(multi_select_movement_box);

			auto multi_select_movement_cb(
				new checkbox("Mouse move snap\nmodels to the ground",
							 snap_multi_selection_to_ground, this));

			auto object_median_pivot_point(new checkbox(
				"Rotate around pivot point", use_median_pivot_point, this));
			...
			QPushButton* rotEditorButton =
				new QPushButton("Pos/Rotation Editor", this);
			QPushButton* visToggleButton =
				new QPushButton("Toggle Hidden Models Visibility", this);
			QPushButton* clearListButton =
				new QPushButton("Clear Hidden Models List", this);
			...
			QPushButton* last_m2_from_wmv =
				new QPushButton("Last M2 from WMV", this);
			QPushButton* last_wmo_from_wmv =
				new QPushButton("Last WMO from WMV", this);
			QPushButton* helper_models_btn =
				new QPushButton("Helper Models", this);
			...
			connect(copyAttributesCheck, &QCheckBox::stateChanged,
					[this](int s) { _copy_model_stats = s; });

			pasteModeGroup->button(pasteMode)->setChecked(true);

			connect(object_median_pivot_point, &QCheckBox::stateChanged,
					[this](bool b) { _use_median_pivot_point = b; });

			connect(pasteModeGroup,
			...
			modelImport->move(mv_pos.x() + (mv_size.width() / 2),
							  mv_pos.y() + (mv_size.height() / 2));
		}
		...
		object_editor::~object_editor()
		{
			for (auto& instance : _model_instance_created)
			{
				...
				{
					ModelInstance* mi =
						GetFromSelection<selected_model_type>(instance);
					delete mi;
				}
				else if (instance.which() == eEntry_WMO)
				{
					WMOInstance* wi =
						GetFromSelection<selected_wmo_type>(instance);
					delete wi;
				}
			}
		}

		void object_editor::showImportModels() { modelImport->show(); }
										...
										object_paste_params* paste_params)
		{
			...
			for (auto& selection : selected)
			{
				...
				math::vector_3d model_pos =
					selection.which() == eEntry_Model
						? GetFromSelection<selected_model_type>(selection)->pos
						: GetFromSelection<selected_wmo_type>(selection)->pos;
				...
				switch (pasteMode)
				{
					...
					case PASTE_ON_SELECTION:
						if (last_entry)
						{
							math::vector_3d last_entry_pos =
								last_entry->which() == eEntry_Model
									? GetFromSelection<selected_model_type>(
										  last_entry.value())
										  ->pos
									: GetFromSelection<selected_wmo_type>(
										  last_entry.value())
										  ->pos;

							pos = last_entry_pos + model_pos;
						}
						else  // paste to mouse cursor when there's no selected
							  // model
						{
							pos = cursor_pos + model_pos;
						}
				...
				if (selection.which() == eEntry_Model)
				{
					...
					if (_copy_model_stats)
					{
						...
						scale = GetFromSelection<selected_model_type>(selection)
									->scale;
						rotation =
					...
					world->addM2(
						GetFromSelection<selected_model_type>(selection)
							->model->filename,
						pos, scale, rotation, paste_params);
				}
				else if (selection.which() == eEntry_WMO)
				{
					...
					if (_copy_model_stats)
					{
						...
						rotation =
							GetFromSelection<selected_wmo_type>(selection)->dir;
					}

					world->addWMO(GetFromSelection<selected_wmo_type>(selection)
									  ->wmo->filename,
								  pos, rotation);
				}
			}
		...
		{
			pasteModeGroup->button((pasteMode + 1) % PASTE_MODE_COUNT)
				->setChecked(true);
		}

		void object_editor::replace_selection(
			std::vector<selection_type> new_selection)
		{
			...
			if (selected.size() == 1)
			{
				...
				{
					ss << GetFromSelection<selected_model_type>(selectedObject)
							  ->model->filename;
				}
				else if (selectedObject.which() == eEntry_WMO)
				{
					ss << GetFromSelection<selected_wmo_type>(selectedObject)
							  ->wmo->filename;
				}
				else
				{
		...
		void object_editor::copy(std::string const& filename)
		{
			...
			else if (StrEndsWith(filename, ".wmo"))
			{
		...
		void object_editor::copy_current_selection(World* world)
		{
			...
			for (auto& selection : current_selection)
			{
				if (selection.which() == eEntry_Model)
				{
					...
					clone->pos = pivot ? original->pos - pivot.value()
									   : math::vector_3d();
		...
		void object_editor::SaveObjecttoTXT(World* world)
		{
			...
			std::ofstream stream(
				_settings->value("project/import_file", "import.txt")
					.toString()
					.toStdString(),
				std::ios_base::app);
			for (auto& selection : world->current_selection())
			{
				...
				{
					path = GetFromSelection<selected_wmo_type>(selection)
							   ->wmo->filename;
				}
				else if (selection.which() == eEntry_Model)
				{
					path = GetFromSelection<selected_model_type>(selection)
							   ->model->filename;
				}
		...
		{
			std::string wmv_log_file(_settings->value("project/wmv_log_file")
										 .toString()
										 .toStdString());
		...
		QSize object_editor::sizeHint() const { return QSize(215, height()); }
	}  // namespace ui
}  // namespace noggit
