...
namespace noggit
{
	namespace ui
	{
		struct model_item : QStandardItem
		{
			...
			virtual QVariant data(int role) const
			{
				if (role == Qt::DecorationRole)
				{
					if (!_rendered)
					{
						//! \note The one time Qt is const correct and we don't
						//! want that.
						auto that(const_cast<model_item*>(this));
						that->_rendered = true;
						that->_pixmap =
							render_blp_to_pixmap(data(Qt::DisplayRole)
													 .toString()
													 .prepend("tileset/")
													 .toStdString(),
												 256, 256);
			...
			: widget(parent, Qt::Window)
		{
			...
			{
				QSettings settings;
				std::filesystem::path const prefix(std::filesystem::path(
					settings.value("project/path").toString().toStdString()));
				size_t const prefix_size(prefix.string().length());

				if (std::filesystem::exists(prefix))
				{
					for (std::filesystem::directory_entry const& entry_abs :
						 std::filesystem::directory_iterator(prefix))
					{
						std::string entry(mpq::normalized_filename(
							entry_abs.path().string().substr(prefix_size)));

						if (entry.find("tileset") != std::string::npos &&
							entry.find(".blp") != std::string::npos)
						{
							size_t suffix_pos(entry.find("_s.blp"));
							if (suffix_pos == std::string::npos)
							{
								tilesets.emplace_back(entry);
							}
							else
							{
								std::string specular(entry);
								specular.erase(suffix_pos, strlen("_s"));
								tilesets_with_specular_variant.emplace(
									specular);
							}
						}
					}
			...
			for (auto const& texture : tilesets)
			{
				...
				item->setData(tilesets_with_specular_variant.count(texture)
								  ? "true"
								  : "false",
							  has_specular_role);
			...
			filter->addItems({"",			"base",		   "brick",  "brush",
							  "bush",		"cobblestone", "crack",  "creep",
							  "crop",		"crystal",	 "dark",   "dead",
							  "dirt",		"fern",		   "floor",  "flower",
							  "footprints", "grass",	   "ice",	"ivy",
							  "lava",		"leaf",		   "light",  "mineral",
							  "moss",		"mud",		   "needle", "pebbl",
							  "road",		"rock",		   "root",   "rubble",
							  "sand",		"shore",	   "slime",  "smooth",
							  "snow",		"water",	   "waves",  "web",
							  "weed"});
			connect(filter, &QComboBox::currentTextChanged, [=](QString text) {
				search_filter->setFilterRegExp(text);
			});

			auto only_specular(
				new QCheckBox("only with specular texture variant"));
			connect(only_specular, &QCheckBox::toggled, [=](bool on) {
			...
			connect(size_slider, &QSlider::valueChanged, [=](int size) {
		...
		// selected_texture:
		std::optional<scoped_blp_texture_reference> selected_texture::texture =
			std::nullopt;
