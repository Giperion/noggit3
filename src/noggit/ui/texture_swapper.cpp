...
namespace noggit
{
	namespace ui
	{
		texture_swapper::texture_swapper(QWidget* parent,
										 ...
										 World* world)
			: QWidget(parent), _texture_to_swap(), _radius(15.f)
		{
			setWindowTitle("Swap");
			...
			_radius_slider =
				new QSlider(Qt::Orientation::Horizontal, brush_content);
			...
			connect(select, &QPushButton::clicked, [&]() {
				...
				if (_texture_to_swap)
				{
					_texture_to_swap_display->set_texture(
						_texture_to_swap.value()->filename);
				}
			});

			connect(swap_adt, &QPushButton::clicked,
					[this, camera_pos, world]() {
						if (_texture_to_swap)
						{
							world->swapTexture(*camera_pos,
											   _texture_to_swap.value());
						}
					});
		...
		{
			_texture_to_swap =
				std::move(scoped_blp_texture_reference(filename));
		}
	}  // namespace ui
}  // namespace noggit
