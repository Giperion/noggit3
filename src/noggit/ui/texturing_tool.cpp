...
namespace noggit
{
	namespace ui
	{
			  ...
			  _texturing_mode(texturing_mode::paint)
		{
			...
			_hardness_slider =
				new QSlider(Qt::Orientation::Horizontal, tool_widget);
			...
			_brush_level_slider =
				new QSlider(Qt::Orientation::Vertical, tool_widget);
			...
			QString _brush_level_slider_style =
				...
				"  background-color: qlineargradient(x1:0.5, y1:0, x2:0.5, "
				"y2:1, stop: 0 black, stop: 1 #FFFFFF); \n "
				"  width: 35px; \n"
				"  margin: 0 0 0 0; \n "
				"} \n "
			...
			slider_layout_right->addWidget(_brush_level_slider, 0,
										   Qt::AlignHCenter);
			...
			_show_unpaintable_chunks_cb =
				new QCheckBox("Show unpaintable chunks", tool_widget);
			...
			_spray_size_slider =
				new QSlider(Qt::Orientation::Horizontal, _spray_content);
			...
			_spray_pressure_slider =
				new QSlider(Qt::Orientation::Horizontal, _spray_content);
			_spray_pressure_slider->setRange(0, 10 * 100);
			_spray_pressure_slider->setSliderPosition(
				std::round(_spray_pressure * 100));
			spray_layout->addRow(_spray_pressure_slider);

			_texture_switcher =
				new texture_swapper(tool_widget, camera_pos, world);
			...
			auto anim_speed_slider =
				new QSlider(Qt::Orientation::Horizontal, _anim_group);
			anim_speed_slider->setRange(0, 7);
			anim_speed_slider->setSingleStep(1);
			anim_speed_slider->setTickInterval(1);
			anim_speed_slider->setTickPosition(
			...
			connect(_anim_group, &QGroupBox::toggled, [&](bool b) {
				_anim_group->setTitle(QString(b ? "Add anim" : "Remove anim"));
				_anim_prop.set(b);
			});

			connect(anim_speed_slider, &QSlider::valueChanged,
					&_anim_speed_prop, &noggit::unsigned_int_property::set);
			connect(anim_orientation_dial, &QDial::valueChanged,
					&_anim_rotation_prop, &noggit::unsigned_int_property::set);
			...
			connect(tabs, &QTabWidget::currentChanged, [this](int index) {
				switch (index)
				{
					case 0:
						...
						break;
					case 1:
						_texturing_mode = texturing_mode::swap;
						break;
					case 2:
						_texturing_mode = texturing_mode::anim;
						break;
				}
			});

			connect(_radius_spin,
			...
			connect(_hardness_spin,
					qOverload<double>(&QDoubleSpinBox::valueChanged),
					[&](double v) {
						QSignalBlocker const blocker(_hardness_slider);
						_hardness = v;
						_hardness_slider->setSliderPosition(
							(int)std::round(v * 100.0f));
						update_brush_hardness();
					});

			connect(_hardness_slider, &QSlider::valueChanged, [&](int v) {
			...
			connect(_pressure_spin,
					qOverload<double>(&QDoubleSpinBox::valueChanged),
					[&](double v) {
						QSignalBlocker const blocker(_pressure_slider);
						_pressure = v;
						_pressure_slider->setSliderPosition(
							(int)std::round(v * 100.0f));
					});

			connect(_pressure_slider, &QSlider::valueChanged, [&](int v) {
			...
			connect(_brush_level_slider, &QSlider::valueChanged, [&](int v) {
			...
			connect(_spray_size_spin,
					qOverload<double>(&QDoubleSpinBox::valueChanged),
					[&](double v) {
						QSignalBlocker const blocker(_spray_size_slider);
						_spray_size = v;
						_spray_size_slider->setSliderPosition(
							(int)std::round(v * 100.0f));
						update_spray_brush();
					});

			connect(_spray_size_slider, &QSlider::valueChanged, [&](int v) {
			...
			connect(_spray_pressure_slider, &QSlider::valueChanged, [&](int v) {
			...
			connect(_spray_mode_group, &QGroupBox::toggled,
					[&](bool b) { _spray_content->setVisible(b); });
		...
		void texturing_tool::toggle_brush_level_min_max()
		{
			if (_brush_level_spin->value() > _brush_level_spin->minimum())
				...
				_brush_level_spin->setValue(_brush_level_spin->maximum());
		}
		...
		float texturing_tool::brush_radius() const
		{
			...
			switch (_texturing_mode)
			{
				case texturing_mode::paint:
					return _texture_brush.getRadius();
				case texturing_mode::swap:
					return (_texture_switcher->brush_mode()
								? _texture_switcher->radius()
								: 0.f);
				default:
					return 0.f;
			}
		...
		{
			return _show_unpaintable_chunks &&
				   _texturing_mode == texturing_mode::paint;
		}

		void texturing_tool::paint(World* world, math::vector_3d const& pos,
								   float dt,
								   scoped_blp_texture_reference texture)
		{
			...
			{
				std::optional<scoped_blp_texture_reference> to_swap(
					_texture_switcher->texture_to_swap());
				if (to_swap)
				{
					if (_texture_switcher->brush_mode())
					{
						world->replaceTexture(pos, _texture_switcher->radius(),
											  to_swap.value(), texture);
					}
					else
					{
						world->overwriteTextureAtCurrentChunk(
							pos, to_swap.value(), texture);
					}
				}
			}
			else if (_texturing_mode == texturing_mode::paint)
			{
				...
				{
					world->sprayTexture(pos, &_spray_brush, alpha_target(),
										strength, _texture_brush.getRadius(),
										_spray_pressure, texture);

					if (_inner_radius_cb->isChecked())
					{
						world->paintTexture(pos, &_inner_brush, alpha_target(),
											strength, texture);
					}
				}
				else
				{
					world->paintTexture(pos, &_texture_brush, alpha_target(),
										strength, texture);
				}
			}
			else if (_texturing_mode == texturing_mode::anim)
		...
		void texturing_tool::change_tex_flag(
			World* world, math::vector_3d const& pos, bool add,
			scoped_blp_texture_reference texture)
		{
			std::size_t flag = 0;
			...
			// the texture's flag glow is set if the property is true, removed
			// otherwise
		...
		QSize texturing_tool::sizeHint() const { return QSize(215, height()); }
	}  // namespace ui
}  // namespace noggit
