...
union wmo_group_flags {
	...
	struct
	{
		...
		uint32_t unk : 1;  // 0x8000000 requires intBatchCount == 0,
						   // extBatchCount == 0, UNREACHABLE.
		uint32_t unused : 4;
	};
};
...
class WMOGroup
{
   public:
	...
	void drawLiquid(math::matrix_4x4 const& transform, liquid_render& render,
					bool draw_fog, int animtime);
	...
	bool is_visible(math::matrix_4x4 const& transform_matrix,
					...
					math::vector_3d const& camera, display_mode display) const;
...
struct WMOLight
{
	...
	static void setupOnce(GLint light, math::vector_3d dir,
...
class WMO : public AsyncObject
{
   public:
	...
	void draw(opengl::scoped::use_program& wmo_shader,
			  ...
			  int animtime, bool world_has_skies, display_mode display);
	bool draw_skybox(
	...
	std::map<uint32_t, std::vector<wmo_doodad_instance>> doodads_per_group(
	...
	virtual bool is_required_when_saving() const override { return true; }
...
struct scoped_wmo_reference
{
	...
	scoped_wmo_reference(scoped_wmo_reference&& other)
		: _valid(other._valid), _filename(other._filename), _wmo(other._wmo)
	{
		other._valid = false;
	}
