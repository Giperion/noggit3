...
class MapTile : public AsyncObject
{
   public:
	MapTile(int x0, int z0, std::string const& pFilename, bool pBigAlpha,
			...
			World*, tile_mode mode = tile_mode::edit);
	~MapTile();

	void finishLoading();

	//! \todo on destruction, unload ModelInstances and WMOInstances on this
	//! tile:
	// a) either keep up the information what tiles the instances are on at all
	// times
	//    (even while moving), to then check if all tiles it was on were
	//    unloaded, or
	// b) do the reference count lazily by iterating over all instances and
	// checking
	//    what MapTiles they span. if any of those tiles is still loaded, keep
	//    it, otherwise remove it.
	//
	// I think b) is easier. It only requires
	// `std::set<C2iVector> XInstance::spanning_tiles() const` followed by
	// `if_none (isTileLoaded (x, y)): unload instance`, which is way easier
	// than constantly updating the reference counters. Note that both
	// approaches do not cover the issue that the instance might not be saved to
	// any tile, thus the movement might have been lost.
	...
	std::vector<MapChunk*> chunks_in_range(math::vector_3d const& pos,
										   float radius) const;
	...
	void drawWater(math::frustum const& frustum, const float& cull_distance,
   ...
   private:
	...
	std::vector<TileWater*> chunksLiquids;  // map chunks liquids for old style
											// water render!!! (Not MH2O)
