...
class MapView : public QOpenGLWidget
{
   ...
   public:
	...
	MapView(math::degrees ah0, math::degrees av0, math::vector_3d camera_pos,
			noggit::ui::main_window*, std::unique_ptr<World>,
   ...
   private:
	...
	struct HotKey
	{
		...
		HotKey(Qt::Key k, size_t m, std::function<void()> f,
			   std::function<bool()> c)
	...
	void addHotkey(Qt::Key key, size_t modifiers,
				   std::function<void()> function,
