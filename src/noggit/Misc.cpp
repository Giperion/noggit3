...
namespace misc
{
						  ...
						  const std::string& replace)
	{
		...
		while (
			found !=
			std::string::npos)  // fixed unknown letters replace. Now it works
								// correctly and replace all found symbold
								// instead of only one at previous versions
		{
	...
	{
		return getShortestDist(pos.x, pos.z, square_pos.x, square_pos.z,
							   unitSize);
	}
	...
	{
		return r1[0].x <= r2[1].x && r2[0].x <= r1[1].x && r1[0].z <= r2[1].z &&
			   r2[0].z <= r1[1].z;
	}
	...
	{
		return (origin.y + ((pos.x - origin.x) * math::cos(orientation) +
	...
	{
		return float_equals(v1.x, v2.x) && float_equals(v1.y, v2.y) &&
			   float_equals(v1.z, v2.z);
	}
}  // namespace misc
