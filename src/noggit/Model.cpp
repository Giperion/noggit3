...
Model::Model(const std::string& filename)
	: AsyncObject(filename), _finished_upload(false)
{
...
void Model::finishLoading()
{
	...
	if (header.Flags & 8)
	{
		...
		uint32_t const* blend_override_info = reinterpret_cast<uint32_t const*>(
			f.getBuffer() + sizeof(ModelHeader));
		uint32_t n_blend_override = *blend_override_info++;
		uint32_t ofs_blend_override = *blend_override_info;

		blend_override =
			M2Array<uint16_t>(f, ofs_blend_override, n_blend_override);
	}

	_vertex_box_points = math::box_points(
		misc::transform_model_box_coords(header.bounding_box_min),
		misc::transform_model_box_coords(header.bounding_box_max));

	animated =
		isAnimated(f);  // isAnimated will set animGeometry and animTextures
	...
	{
		_global_sequences =
			M2Array<int>(f, header.ofsGlobalSequences, header.nGlobalSequences);
	}
...
bool Model::isAnimated(const MPQFile& f)
{
	...
	ModelBoneDef const* bo =
		reinterpret_cast<ModelBoneDef const*>(f.getBuffer() + header.ofsBones);

	animGeometry = false;
	animBones = false;
	_per_instance_animation = false;

	ModelVertex const* verts = reinterpret_cast<ModelVertex const*>(
		f.getBuffer() + header.ofsVertices);
	for (size_t i = 0; i < header.nVertices && !animGeometry; ++i)
	{
		for (size_t b = 0; b < 4; b++)
		{
			if (verts[i].weights[b] > 0)
			{
				...
				bool billboard =
					(bb.flags & (0x78));  // billboard | billboard_lock_[xyz]

				if ((bb.flags & 0x200) || billboard)
				{
					if (billboard)
					{
						// if we have billboarding, the model will need
	...
	if (animGeometry || header.nParticleEmitters || header.nRibbonEmitters ||
		header.nLights)
	{
		animBones = true;
	}
	...
	if (header.nColors)
	{
		ModelColorDef const* cols = reinterpret_cast<ModelColorDef const*>(
			f.getBuffer() + header.ofsColors);
	...
	{
		ModelTransDef const* trs = reinterpret_cast<ModelTransDef const*>(
			f.getBuffer() + header.ofsTransparency);
...
void Model::initCommon(const MPQFile& f)
{
	...
	// textures
	ModelTextureDef const* texdef = reinterpret_cast<ModelTextureDef const*>(
		f.getBuffer() + header.ofsTextures);
	...
	for (size_t i = 0; i < header.nTextures; ++i)
	{
		if (texdef[i].type == 0)
		{
			...
			_textureFilenames[i] = std::string(
				blp_ptr, texdef[i].nameLen - (invalid_size ? 0 : 1));
		}
		else
		{
...
#else
			//! \note special texture - only on characters and such... Noggit
			//! should not even render these. \todo Check if this is actually
			//! correct. Or just remove it.
	...
	if (header.nColors)
	{
		ModelColorDef const* colorDefs = reinterpret_cast<ModelColorDef const*>(
			f.getBuffer() + header.ofsColors);
		for (size_t i = 0; i < header.nColors; ++i)
		{
	...
	// init transparency
	_transparency_lookup = M2Array<int16_t>(f, header.ofsTransparencyLookup,
											header.nTransparencyLookup);

	if (header.nTransparency)
	{
		ModelTransDef const* trDefs = reinterpret_cast<ModelTransDef const*>(
			f.getBuffer() + header.ofsTransparency);
		for (size_t i = 0; i < header.nTransparency; ++i)
		{
	...
	if (header.nViews > 0)
	{
		...
		ModelView const* view =
			reinterpret_cast<ModelView const*>(g.getBuffer());
		uint16_t const* indexLookup =
			reinterpret_cast<uint16_t const*>(g.getBuffer() + view->ofs_index);
		uint16_t const* triangles = reinterpret_cast<uint16_t const*>(
			g.getBuffer() + view->ofs_triangle);
		...
		// render ops
		ModelGeoset const* model_geosets = reinterpret_cast<ModelGeoset const*>(
			g.getBuffer() + view->ofs_submesh);
		ModelTexUnit const* texture_unit =
			reinterpret_cast<ModelTexUnit const*>(g.getBuffer() +
												  view->ofs_texture_unit);

		_texture_lookup =
			M2Array<uint16_t>(f, header.ofsTexLookup, header.nTexLookup);
		_texture_animation_lookups =
			M2Array<int16_t>(f, header.ofsTexAnimLookup, header.nTexAnimLookup);
		_texture_unit_lookup =
			M2Array<int16_t>(f, header.ofsTexUnitLookup, header.nTexUnitLookup);
		...
		_render_flags = M2Array<ModelRenderFlags>(f, header.ofsRenderFlags,
												  header.nRenderFlags);
...
void Model::fix_shader_id_blend_override()
{
	for (auto& pass : _render_passes)
	{
		...
		// fuckporting check
		if (pass.texture_coord_combo_index + pass.texture_count - 1 >=
		...
		if (!blend_mode_override)
		{
			uint16_t texture_unit_lookup =
				_texture_unit_lookup[pass.texture_coord_combo_index];
		...
		else
		{
			...
			for (int i = 0; i < pass.texture_count; ++i)
			{
				...
				uint16_t texture_unit_lookup =
					_texture_unit_lookup[pass.texture_coord_combo_index + i];
			...
			shader |= (runtime_shader_val[1] & 0xFFFF) |
					  ((runtime_shader_val[0] << 4) & 0xFFFF);
		}
		...
		for (auto& pass : _render_passes)
		{
			...
			if (pass.material_layer == 0)
			{
				if (pass.texture_count >= 1 &&
					_render_flags[pass.renderflag_index].blend == 0)
				{
			...
			bool xor_unlit =
				((_render_flags[pass.renderflag_index].flags.unlit ^
				  _render_flags[first_pass->renderflag_index].flags.unlit) &
				 1) == 0;

			if ((some_flags & 0xFF) == 1)
			{
				if ((_render_flags[pass.renderflag_index].blend == 1 ||
					 _render_flags[pass.renderflag_index].blend == 2) &&
					pass.texture_count == 1 && xor_unlit &&
					pass.texture_combo_index == first_pass->texture_combo_index)
				{
					if (_transparency_lookup[pass.transparency_combo_index] ==
						_transparency_lookup[first_pass
												 ->transparency_combo_index])
					{
			...
			int16_t texture_unit_lookup =
				_texture_unit_lookup[pass.texture_coord_combo_index];

			if ((some_flags & 0xFF) < 2)
			{
				if ((_render_flags[pass.renderflag_index].blend == 0) &&
					(pass.texture_count == 2) &&
					((lower_bits == 4) || (lower_bits == 6)))
				{
					if (texture_unit_lookup == 0 &&
						(_texture_unit_lookup[pass.texture_coord_combo_index +
											  1] == -1))
					{
			...
			if ((some_flags >> 8) != 0)
			{
				if ((some_flags >> 8) == 1)
				{
					if (((_render_flags[pass.renderflag_index].blend != 4) &&
						 (_render_flags[pass.renderflag_index].blend != 6)) ||
						(pass.texture_count != 1) || (texture_unit_lookup >= 0))
					{
						some_flags &= 0xFF00;
					}
					else if (_transparency_lookup
								 [pass.transparency_combo_index] ==
							 _transparency_lookup
								 [first_pass->transparency_combo_index])
					{
						pass.shader_id = 0x8000;
						first_pass->shader_id =
							_render_flags[pass.renderflag_index].blend != 4
								? 0xE
								: 0x8002;
						...
						first_pass->uv_animations[1] =
							pass.animation_combo_index;
				...
				else
				{
					...
					if (((_render_flags[pass.renderflag_index].blend != 2) &&
						 (_render_flags[pass.renderflag_index].blend != 1)) ||
						(pass.texture_count != 1) || xor_unlit ||
						((pass.texture_combo_index & 0xff) !=
						 (first_pass->texture_combo_index & 0xff)))
					{
						some_flags &= 0xFF00;
					}
					else if (_transparency_lookup
								 ...
								 [first_pass->transparency_combo_index])
					{
						...
						pass.shader_id = 0x8000;
						first_pass->shader_id =
							((first_pass->shader_id == 0x8002 ? 2 : 0) -
							 0x7FFF) &
							0xFFFF;
			...
			pass.textures[1] =
				pass.texture_count > 1 ? pass.texture_combo_index + 1 : 0;
			pass.uv_animations[0] = pass.animation_combo_index;
			pass.uv_animations[1] =
				pass.texture_count > 1 ? pass.animation_combo_index + 1 : 0;
		...
		if (need_reducing)
		{
			...
			for (int i = 0; i < passes.size(); ++i)
			{
				...
				if (renderflag_index == previous_render_flag)
				{
					...
					pass.texture_combo_index =
						_render_passes[i - 1].texture_combo_index;
					pass.texture_coord_combo_index =
						_render_passes[i - 1].texture_coord_combo_index;
				}
				else
				{
...
bool ModelRenderPass::prepare_draw(opengl::scoped::use_program& m2_shader,
								   Model* m)
{
	...
	math::vector_4d mesh_color =
		math::vector_4d(1.0f, 1.0f, 1.0f, m->trans);  // ??
	...
	{
		::math::vector_3d c(m->_colors[color_index].color.getValue(
			0, m->_anim_time, m->_global_animtime));
		if (m->_colors[color_index].opacity.uses(m->_current_anim_seq))
		{
			mesh_color.w = m->_colors[color_index].opacity.getValue(
				m->_current_anim_seq, m->_anim_time, m->_global_animtime);
		}
	...
	{
		auto& transparency(
			m->_transparency[m->_transparency_lookup[transparency_combo_index]]
				.trans);
		if (transparency.uses(0))
		{
			mesh_color.w =
				mesh_color.w *
				transparency.getValue(0, m->_anim_time, m->_global_animtime);
		}
	}
	...
	switch (static_cast<M2Blend>(renderflag.blend))
	{
		...
		case M2Blend::No_Add_Alpha:
			...
			m2_shader.uniform("alpha_test", (1.f / 255.f) * mesh_color.w);
			m2_shader.uniform("fog_mode",
							  2);  // Warning: wiki is unsure on that
			break;
		case M2Blend::Add:
			gl.enable(GL_BLEND);
	...
	GLint tu1 = static_cast<GLint>(tu_lookups[0]),
		  tu2 = static_cast<GLint>(tu_lookups[1]);
	...
	{
		m2_shader.uniform("tex_matrix_1",
						  m->_texture_animations[tex_anim_lookup].mat);
		if (texture_count > 1)
		{
			tex_anim_lookup = m->_texture_animation_lookups[uv_animations[1]];
			if (tex_anim_lookup != -1)
			{
				m2_shader.uniform("tex_matrix_2",
								  m->_texture_animations[tex_anim_lookup].mat);
			}
			else
			{
	...
	m2_shader.uniform("pixel_shader", static_cast<GLint>(pixel_shader.value()));
...
void ModelRenderPass::init_uv_types(Model* m)
{
	...
	if (m->_texture_unit_lookup.size() <
		texture_coord_combo_index + texture_count)
	{
		throw std::out_of_range(
			"model: texture_coord_combo_index out of range " + m->filename);
	}
	...
	for (int i = 0; i < texture_count; ++i)
	{
		switch (m->_texture_unit_lookup[texture_coord_combo_index + i])
		{
			...
			case 0:
				...
				break;
			case 1:
				tu_lookups[i] = texture_unit_lookup::t2;
				break;
		}
	}
}
...
{
	math::vector_3d min = m->header.bounding_box_min,
					max = m->header.bounding_box_max;
	...
	indices = {0, 1, 2, 2, 3, 0, 0, 4, 5, 5, 1, 0, 0, 3, 7, 7, 4, 0,
			   1, 5, 6, 6, 2, 1, 2, 6, 7, 7, 3, 2, 5, 6, 7, 7, 4, 5};
}

namespace
{
	// https://wowdev.wiki/M2/.skin/WotLK_shader_selection
	std::optional<ModelPixelShader> GetPixelShader(uint16_t texture_count,
												   uint16_t shader_id)
	{
		...
		else
		{
			if (!texture1_fragment_mode)
			{
				switch (texture2_fragment_mode)
				{
					case 0:
						pixel_shader =
							ModelPixelShader::Combiners_Opaque_Opaque;
						break;
					case 3:
					...
					case 6:
						pixel_shader =
							ModelPixelShader::Combiners_Opaque_Mod2xNA;
						break;
					case 7:
	...
	std::optional<ModelPixelShader> M2GetPixelShaderID(uint16_t texture_count,
													   uint16_t shader_id)
	{
		std::optional<ModelPixelShader> pixel_shader;
...
void Model::initAnimated(const MPQFile& f)
{
	...
	if (header.nAnimations > 0)
	{
		...
		memcpy(animations.data(), f.getBuffer() + header.ofsAnimations,
		...
		for (auto& anim : animations)
		{
			...
			tempname << lodname << anim.animID << "-" << anim.subAnimID
					 << ".anim";
			if (MPQFile::exists(tempname.str()))
			{
				animation_files.push_back(
					std::make_unique<MPQFile>(tempname.str()));
			}
		}
	}
	...
	if (animBones)
	{
		ModelBoneDef const* mb = reinterpret_cast<ModelBoneDef const*>(
			f.getBuffer() + header.ofsBones);
		for (size_t i = 0; i < header.nBones; ++i)
		{
			bones.emplace_back(f, mb[i], _global_sequences.data(),
							   animation_files);
		}
	}
	...
	if (animTextures)
	{
		ModelTexAnimDef const* ta = reinterpret_cast<ModelTexAnimDef const*>(
			f.getBuffer() + header.ofsTexAnims);
		for (size_t i = 0; i < header.nTexAnims; ++i)
		{
			_texture_animations.emplace_back(f, ta[i],
											 _global_sequences.data());
		}
	}
	...
	{
		ModelParticleEmitterDef const* pdefs =
			reinterpret_cast<ModelParticleEmitterDef const*>(
				f.getBuffer() + header.ofsParticleEmitters);
		for (size_t i = 0; i < header.nParticleEmitters; ++i)
		{
			try
			{
				_particles.emplace_back(this, f, pdefs[i],
										_global_sequences.data());
			}
	...
	{
		ModelRibbonEmitterDef const* rdefs =
			reinterpret_cast<ModelRibbonEmitterDef const*>(
				f.getBuffer() + header.ofsRibbonEmitters);
	...
	if (header.nLights)
	{
		ModelLightDef const* lDefs = reinterpret_cast<ModelLightDef const*>(
			f.getBuffer() + header.ofsLights);
...
void Model::calcBones(math::matrix_4x4 const& model_view, int _anim, int time,
					  int animation_time)
{
	for (size_t i = 0; i < header.nBones; ++i)
	{
	...
	for (size_t i = 0; i < header.nBones; ++i)
	{
		bones[i].calcMatrix(model_view, bones.data(), _anim, time,
							animation_time);
	}
}

void Model::animate(math::matrix_4x4 const& model_view, int anim_id,
					int anim_time)
{
	if (_animations_seq_per_id.empty() ||
		_animations_seq_per_id[anim_id].empty())
	{
		...
		if (_current_vertices.empty())
		{
			...
			opengl::scoped::buffer_binder<GL_ARRAY_BUFFER> const binder(
				_vertices_buffer);
			gl.bufferData(GL_ARRAY_BUFFER,
						  _current_vertices.size() * sizeof(ModelVertex),
						  _current_vertices.data(), GL_STATIC_DRAW);
		}
	...
	_current_anim_seq =
		a.Index;  //_animations_seq_lookup[anim_id][current_sub_anim];
	_anim_time = t;
	...
	if (animGeometry)
	{
		...
		for (auto& vertex : _current_vertices)
		{
			...
			for (size_t b(0); b < 4; ++b)
			{
				if (vertex.weights[b] <= 0) continue;

				::math::vector_3d tv =
					bones[vertex.bones[b]].mat * vertex.position;
				::math::vector_3d tn =
					bones[vertex.bones[b]].mrot * vertex.normal;
		...
		gl.bufferData(GL_ARRAY_BUFFER,
					  _current_vertices.size() * sizeof(ModelVertex),
					  _current_vertices.data(), GL_STREAM_DRAW);
	}
...
void TextureAnim::calc(int anim, int time, int animtime)
{
	...
	if (trans.uses(anim))
	{
		mat *= math::matrix_4x4(math::matrix_4x4::translation,
								trans.getValue(anim, time, animtime));
	}
	if (rot.uses(anim))
	{
		mat *= math::matrix_4x4(math::matrix_4x4::rotation,
								rot.getValue(anim, time, animtime));
	}
	if (scale.uses(anim))
	{
		mat *= math::matrix_4x4(math::matrix_4x4::scale,
								scale.getValue(anim, time, animtime));
	}
}

ModelColor::ModelColor(const MPQFile& f, const ModelColorDef& mcd, int* global)
	: color(mcd.color, f, global), opacity(mcd.opacity, f, global)
{
}

ModelTransparency::ModelTransparency(const MPQFile& f, const ModelTransDef& mcd,
									 int* global)
...
{
	math::vector_4d ambcol(ambColor.getValue(0, time, animtime) *
							   ambIntensity.getValue(0, time, animtime),
						   1.0f);
	math::vector_4d diffcol(diffColor.getValue(0, time, animtime) *
								diffIntensity.getValue(0, time, animtime),
							1.0f);
	math::vector_4d p;
...
TextureAnim::TextureAnim(const MPQFile& f, const ModelTexAnimDef& mta,
						 int* global)
...
Bone::Bone(const MPQFile& f, const ModelBoneDef& b, int* global,
...
void Bone::calcMatrix(math::matrix_4x4 const& model_view, Bone* allbones,
					  int anim, int time, int animtime)
{
	if (calc) return;
	...
	if (flags.transformed || flags.billboard ||
		...
		flags.cylindrical_billboard_lock_z)
	{
		...
		if (trans.uses(anim))
		{
			m *= math::matrix_4x4(math::matrix_4x4::translation,
								  trans.getValue(anim, time, animtime));
		}

		if (rot.uses(anim))
		{
			m *= math::matrix_4x4(math::matrix_4x4::rotation,
								  q = rot.getValue(anim, time, animtime));
		}

		if (scale.uses(anim))
		{
			m *= math::matrix_4x4(math::matrix_4x4::scale,
								  scale.getValue(anim, time, animtime));
		}
		...
		if (flags.billboard)
		{
			...
			math::vector_3d vUp(model_view[1], model_view[5],
								model_view[9]);  // Spherical billboarding
			// math::vector_3d vUp = math::vector_3d(0,1,0); // Cylindrical
			// billboarding
			vRight = vRight * -1;
			m(0, 2, vRight.x);
			m(1, 2, vRight.y);
	...
	if (rot.uses(anim))
	{
		if (parent >= 0)
		{
			mrot = allbones[parent].mrot *
				   math::matrix_4x4(math::matrix_4x4::rotation, q);
		}
		else
		{
...
void Model::draw(math::matrix_4x4 const& model_view, ModelInstance& instance,
				 opengl::scoped::use_program& m2_shader,
				 math::frustum const& frustum, const float& cull_distance,
				 const math::vector_3d& camera, int animtime,
				 bool  // draw_particles
				 ,
				 bool  // all_boxes
				 ,
				 display_mode display)
{
...

	{
		...
		// m2_shader.attrib("bones_weight",  4, GL_UNSIGNED_BYTE,  GL_FALSE,
		// sizeof (ModelVertex), reinterpret_cast<void*> (sizeof
		// (::math::vector_3d))); m2_shader.attrib("bones_indices", 4,
		// GL_UNSIGNED_BYTE,  GL_FALSE, sizeof (ModelVertex),
		// reinterpret_cast<void*> (sizeof (::math::vector_3d) + 4));
		m2_shader.attrib(
			"normal", 3, GL_FLOAT, GL_FALSE, sizeof(ModelVertex),
			reinterpret_cast<void*>(sizeof(::math::vector_3d) + 8));
		m2_shader.attrib(
			"texcoord1", 2, GL_FLOAT, GL_FALSE, sizeof(ModelVertex),
			reinterpret_cast<void*>(sizeof(::math::vector_3d) * 2 + 8));
		m2_shader.attrib(
			"texcoord2", 2, GL_FLOAT, GL_FALSE, sizeof(ModelVertex),
			reinterpret_cast<void*>(sizeof(::math::vector_3d) * 2 + 8 +
									sizeof(::math::vector_2d)));
	}
	...
	for (ModelRenderPass& p : _render_passes)
	{
		...
		{
			gl.drawElements(GL_TRIANGLES, p.index_count, GL_UNSIGNED_SHORT,
							_indices.data() + p.index_start);
			p.after_draw();
		}
	}
...
void Model::draw(math::matrix_4x4 const& model_view,
				 ...
				 opengl::scoped::use_program& m2_shader,
				 math::frustum const& frustum, const float& cull_distance,
				 const math::vector_3d& camera, bool  // draw_fog
				 ,
				 int animtime, bool draw_particles, bool all_boxes,
				 ...
				 display_mode display)
{
	...
	{
		opengl::scoped::buffer_binder<GL_ARRAY_BUFFER> const transform_binder(
			_transform_buffer);
		gl.bufferData(GL_ARRAY_BUFFER,
					  transform_matrix.size() * sizeof(::math::matrix_4x4),
					  transform_matrix.data(), GL_DYNAMIC_DRAW);
	...
	for (ModelRenderPass& p : _render_passes)
	{
		if (p.prepare_draw(m2_shader, this))
		{
			gl.drawElementsInstanced(
				GL_TRIANGLES, p.index_count, GL_UNSIGNED_SHORT,
				_indices.data() + p.index_start, transform_matrix.size());
			p.after_draw();
		}
	}
...
void Model::draw_box(opengl::scoped::use_program& m2_box_shader,
					 std::size_t box_count)
{
	static std::vector<uint16_t> const indices(
	...
	gl.drawElementsInstanced(GL_LINE_STRIP, indices.size(), GL_UNSIGNED_SHORT,
							 indices.data(), box_count);
}

std::vector<float> Model::intersect(math::matrix_4x4 const& model_view,
									math::ray const& ray, int animtime)
{
	...
	{
		FakeGeometry& fake_geom = _fake_geometry.value();

		for (size_t i = 0; i < fake_geom.indices.size(); i += 3)
		{
			if (std::optional<float> distance = ray.intersect_triangle(
					fake_geom.vertices[fake_geom.indices[i + 0]],
	...
	for (ModelRenderPass& pass : _render_passes)
	{
		for (size_t i(pass.index_start);
			 i < pass.index_start + pass.index_count; i += 3)
		{
			if (std::optional<float> distance = ray.intersect_triangle(
					_current_vertices[_indices[i + 0]].position,
...
void Model::lightsOn(opengl::light lbase)
{
	...
	for (unsigned int i = 0, l = lbase; i < header.nLights; ++i)
		_lights[i].setup(_anim_time, l++, _global_animtime);
}
...
void Model::upload()
{
	for (const std::string& texture : _textureFilenames)
...

	{
		opengl::scoped::buffer_binder<GL_ARRAY_BUFFER> const binder(_box_vbo);
		gl.bufferData(GL_ARRAY_BUFFER,
					  _vertex_box_points.size() * sizeof(math::vector_3d),
					  _vertex_box_points.data(), GL_STATIC_DRAW);
	}
