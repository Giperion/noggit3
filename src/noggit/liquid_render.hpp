...
class liquid_render
{
   public:
	...
	void prepare_draw(opengl::scoped::use_program& water_shader, int liquid_id,
					  int animtime);

	opengl::program const& shader_program() const { return program; }
   ...
   private:
	...
	std::optional<int> _current_liquid_id;
	...
	std::map<int, std::vector<scoped_blp_texture_reference>>
		_textures_by_liquid_id;
};
