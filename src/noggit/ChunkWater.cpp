...
void ChunkWater::from_mclq(std::vector<mclq>& layers)
{
	...
	for (mclq& liquid : layers)
	{
		...
		{
			case 1:
				_layers.emplace_back(pos, liquid, 2);
				break;
			case 3:
				_layers.emplace_back(pos, liquid, 4);
				break;
			case 4:
				_layers.emplace_back(pos, liquid, 1);
				break;
			case 6:
				_layers.emplace_back(pos, liquid,
...
void ChunkWater::draw(math::frustum const& frustum, const float& cull_distance,
					  ...
					  int layer, display_mode display)
{
	...
	{
		_layers[layer].draw(render, water_shader, camera, camera_moved,
							animtime);
	}
}
...
void ChunkWater::paintLiquid(math::vector_3d const& pos, float radius,
							 int liquid_id, bool add,
							 math::radians const& angle,
							 math::radians const& orientation, bool lock,
							 math::vector_3d const& origin,
							 bool override_height, bool override_liquid_id,
							 MapChunk* chunk, float opacity_factor)
{
	if (override_liquid_id && !override_height)
	{
		...
		if (!layer_found)
		{
			liquid_layer layer(math::vector_3d(xbase, 0.0f, zbase), pos.y,
							   liquid_id);
			copy_height_to_layer(layer, pos, radius);
	...
	for (liquid_layer& layer : _layers)
	{
		...
		{
			layer.paintLiquid(pos, radius, add, angle, orientation, lock,
							  origin, override_height, chunk, opacity_factor);
			painted = true;
		}
		else
		{
			layer.paintLiquid(pos, radius, false, angle, orientation, lock,
							  origin, override_height, chunk, opacity_factor);
		}
	}
...
void ChunkWater::copy_height_to_layer(liquid_layer& target,
