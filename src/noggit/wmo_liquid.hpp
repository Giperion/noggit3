...
struct WMOMaterial
{
	union {
		...
		struct
		{
			...
			uint32_t unculled : 1;
			uint32_t
				ext_light : 1;  // darkened used for the intern face of windows
			uint32_t sidn : 1;
	...
	uint32_t texture_offset_1;  // Start position for the first texture filename
								// in the MOTX data block
	CImVector sidn_color;		// emissive color
	CImVector frame_sidn_color;  // runtime value
	uint32_t texture_offset_2;   // Start position for the second texture
								 // filename in the MOTX data block
	CArgb diffuse_color;
...
{
   public:
	wmo_liquid(MPQFile* f, WMOLiquidHeader const& header,
			   WMOMaterial const& mat, int group_liquid, bool use_dbc_type,
			   bool is_ocean);
	wmo_liquid(wmo_liquid const& other);

	void draw(math::matrix_4x4 const& transform, liquid_render& render,
			  int animtime);
