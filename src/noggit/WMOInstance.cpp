...
{
	extents[0] =
		math::vector_3d(d->extents[0][0], d->extents[0][1], d->extents[0][2]);
	extents[1] =
		math::vector_3d(d->extents[1][0], d->extents[1][1], d->extents[1][2]);
...
void WMOInstance::draw(opengl::scoped::use_program& wmo_shader,
					   ...
					   const math::vector_3d& camera, bool force_box,
					   bool draw_doodads, bool draw_fog, liquid_render& render,
					   std::vector<selection_type> selection, int animtime,
					   bool world_has_skies, display_mode display)
{
	...
	bool const is_selected =
		selection.size() > 0 &&
		std::find_if(
			selection.begin(), selection.end(), [id](selection_type type) {
...

	{
		...
		wmo->draw(wmo_shader, model_view, projection, _transform_mat,
				  _transform_mat_transposed, is_selected, frustum,
				  cull_distance, camera, draw_doodads, draw_fog, render,
				  animtime, world_has_skies, display);
	}
	...
	if (force_box || is_selected)
	{
		...
		math::vector_4d color = force_box
									? math::vector_4d(0.0f, 0.0f, 1.0f, 1.0f)
									: math::vector_4d(0.0f, 1.0f, 0.0f, 1.0f);
		opengl::primitives::wire_box(extents[0], extents[1])
			.draw(model_view, projection,
				  math::matrix_4x4(math::matrix_4x4::unit), color);
	}
}
...
void WMOInstance::update_transform_matrix()
{
	math::matrix_4x4 mat(
		...
		math::matrix_4x4(math::matrix_4x4::rotation_yzx,
						 {math::degrees(-dir.z), math::degrees(dir.y - 90.0f),
						  math::degrees(dir.x)}));
...
void WMOInstance::recalcExtents()
{
	...
	auto&& root_points =
		_transform_mat * math::aabb(wmo_min, wmo_max).all_corners();
	...
	for (int i = 0; i < (int)wmo->groups.size(); ++i)
	{
		...
		auto&& group_points =
			_transform_mat *
			math::aabb(group.BoundingBoxMin  // no need to use
											 // misc::transform_model_box_coords
					   ,
					   group.BoundingBoxMax  // they are already in world coord
											 // (see group ctor)
					   )
				.all_corners();
...
std::vector<wmo_doodad_instance*> WMOInstance::get_visible_doodads(
	...
	math::vector_3d const& camera, bool draw_hidden_models,
	display_mode display)
{
	...
	if (!wmo->is_hidden() || draw_hidden_models)
	{
		...
		{
			if (wmo->groups[i].is_visible(_transform_mat, frustum,
										  cull_distance, camera, display))
			{
