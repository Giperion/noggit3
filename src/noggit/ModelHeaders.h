...
struct ModelAnimation
{
	...
	uint32_t uid;
	uint32_t d2;
	uint32_t playSpeed;  // note: this can't be play speed because it's 0 for
						 // some models
...
struct ModelView
{
	char id[4];  // Signature
	uint32_t n_index,
		ofs_index;  // Vertices in this model (index into vertices[])
	uint32_t n_triangle, ofs_triangle;  // indices
	uint32_t n_vertex_property,
		ofs_vertex_property;					// additional vtx properties
...
struct ModelTexUnit
{
	...
	uint16_t
		texture_combo_index;  // Texture id (index into global texture list)
	uint16_t texture_coord_combo_index;
	uint16_t transparency_combo_index;  // transparency id (index into
										// transparency list)
...
struct ModelParticleParams
{
	...
	FakeAnimationBlock
		opacity;  // (short, short)    Looks like opacity (short), Most likely
				  // they all have 3 timestamps for {start, middle, end}.
	FakeAnimationBlock sizes;  // (short, vec2f)  It carries two floats per key.
							   // (x and y scale)
	int32_t d[2];
	FakeAnimationBlock
...
struct ModelParticleEmitterDef
{
	...
	int16_t rows;
	AnimationBlock
		EmissionSpeed;  // All of the following blocks should be floats.
	AnimationBlock
