...
class WMOInstance
{
   ...
   private:
	...
	math::matrix_4x4 _transform_mat_transposed =
		math::matrix_4x4::uninitialized;
   ...
   public:
	...
	void draw(opengl::scoped::use_program& wmo_shader,
			  ...
			  math::matrix_4x4 const& projection, math::frustum const& frustum,
			  const float& cull_distance, const math::vector_3d& camera,
			  bool force_box, bool draw_doodads, bool draw_fog,
			  liquid_render& render, std::vector<selection_type> selection,
			  int animtime, bool world_has_skies, display_mode display);

	void update_transform_matrix();

	math::matrix_4x4 transform_matrix() const { return _transform_mat; }
	math::matrix_4x4 transform_matrix_inverted() const
	{
		return _transform_mat_inverted;
	}
	math::matrix_4x4 transform_matrix_transposed() const
	{
		return _transform_mat_transposed;
	}
	...
	std::vector<wmo_doodad_instance*> get_visible_doodads(
