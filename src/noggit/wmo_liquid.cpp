...
namespace
{
	...
	{
		liquid_basic_types const basic(
			static_cast<liquid_basic_types>(x & liquid_basic_types_MASK));
		switch (basic)
		{
			default:
...
// todo: use material
wmo_liquid::wmo_liquid(MPQFile* f, WMOLiquidHeader const& header,
					   WMOMaterial const&, int group_liquid, bool use_dbc_type,
					   bool is_ocean)
	  ...
	  ytiles(header.B)
{
	...
	else
	{
		...
		{
			// todo: investigage
			// This method is most likely wrong since "liquid" is the last
			// SMOLTile's liquid value and it can vary from one liquid tile to
			// another.
...
{
	LiquidVertex const* map =
		reinterpret_cast<LiquidVertex const*>(f->getPointer());
	SMOLTile const* tiles = reinterpret_cast<SMOLTile const*>(
		f->getPointer() + (xtiles + 1) * (ytiles + 1) * sizeof(LiquidVertex));
	...
	for (int j = 0; j < ytiles + 1; j++)
	{
		for (int i = 0; i < xtiles + 1; ++i)
		{
			...
			lVertices[p] = math::vector_3d(pos.x + UNITSIZE * i, map[p].height,
										   pos.z - UNITSIZE * j);
		}
	}
	...
	for (int j = 0; j < ytiles; j++)
	{
		for (int i = 0; i < xtiles; ++i)
		{
			...
			if (!(tile.liquid & 0x8))
			{
				...
				{
					depths.emplace_back(
						static_cast<float>(map[p].water_vertex.flow1) / 255.0f);
					tex_coords.emplace_back(i, j);

					depths.emplace_back(
						static_cast<float>(map[p + 1].water_vertex.flow1) /
						255.0f);
					tex_coords.emplace_back(i + 1, j);

					depths.emplace_back(
						static_cast<float>(
							map[p + xtiles + 1 + 1].water_vertex.flow1) /
						255.0f);
					tex_coords.emplace_back(i + 1, j + 1);

					depths.emplace_back(
						static_cast<float>(
							map[p + xtiles + 1].water_vertex.flow1) /
						255.0f);
				...
				else
				{
					...
					// depth isn't used for lava, it's just to fill up the
					// buffer
					...
					tex_coords.emplace_back(
						static_cast<float>(map[p].magma_vertex.s) / 255.f,
						static_cast<float>(map[p].magma_vertex.t) / 255.f);
					tex_coords.emplace_back(
						static_cast<float>(map[p + 1].magma_vertex.s) / 255.f,
						static_cast<float>(map[p + 1].magma_vertex.t) / 255.f);
					tex_coords.emplace_back(
						static_cast<float>(
							map[p + xtiles + 1 + 1].magma_vertex.s) /
							255.f,
						static_cast<float>(
							map[p + xtiles + 1 + 1].magma_vertex.t) /
							255.f);
					tex_coords.emplace_back(
						static_cast<float>(map[p + xtiles + 1].magma_vertex.s) /
							255.f,
						static_cast<float>(map[p + xtiles + 1].magma_vertex.t) /
							255.f);
				}
...
void wmo_liquid::upload(opengl::scoped::use_program& water_shader)
{
	...
	gl.bufferData<GL_ELEMENT_ARRAY_BUFFER, std::uint16_t>(
		_indices_buffer, indices, GL_STATIC_DRAW);

	gl.bufferData<GL_ARRAY_BUFFER, math::vector_3d>(_vertices_buffer, vertices,
													GL_STATIC_DRAW);
	gl.bufferData<GL_ARRAY_BUFFER, math::vector_2d>(_tex_coord_buffer,
													tex_coords, GL_STATIC_DRAW);
	gl.bufferData<GL_ARRAY_BUFFER, float>(_depth_buffer, depths,
										  GL_STATIC_DRAW);
...

	{
		...
		opengl::scoped::buffer_binder<GL_ARRAY_BUFFER> const tex_coord_binder(
			_tex_coord_buffer);
		water_shader.attrib("tex_coord", 2, GL_FLOAT, GL_FALSE, 0, 0);

		opengl::scoped::buffer_binder<GL_ARRAY_BUFFER> const depth_binder(
			_depth_buffer);
...
void wmo_liquid::draw(math::matrix_4x4 const& transform, liquid_render& render,
					  int animtime)
{
