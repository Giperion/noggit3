...
void MapView::ResetSelectedObjectRotation()
{
	for (auto& selection : _world->current_selection())
	{
		...
		{
			WMOInstance* wmo = GetFromSelection<selected_wmo_type>(selection);
		...
		{
			ModelInstance* m2 =
				GetFromSelection<selected_model_type>(selection);
...
void MapView::changeZoneIDValue(int set) { _selected_area_id = set; }
...
void MapView::createGUI()
{
	...
	connect(terrainTool, &noggit::ui::terrain_tool::updateVertices,
			[this](int vertex_mode, math::degrees const& angle,
				   math::degrees const& orientation) {
				makeCurrent();
				opengl::context::scoped_setter const _(::gl, context());
	...
	texturingTool = new noggit::ui::texturing_tool(
		&_camera.position, _world.get(), &_show_texture_palette_small_window,
		_texturing_dock);
	...
	guiWater =
		new noggit::ui::water(&_displayed_water_layer,
							  &_display_all_water_layers, _water_editor_dock);
	...
	connect(guiWater, &noggit::ui::water::crop_water, [this] {
		makeCurrent();
		opengl::context::scoped_setter const _(::gl, context());
	...
	shaderTool =
		new noggit::ui::shader_tool(shader_color, _vertex_shading_dock);
	...
	objectEditor = new noggit::ui::object_editor(
		this, _world.get(), &_move_model_to_cursor_position,
		&_snap_multi_selection_to_ground, &_use_median_pivot_point,
		&_object_paste_params, _object_editor_dock);
	...
	connect(texturingTool->_current_texture,
			&noggit::ui::current_texture::clicked,
	...
	connect(_texture_palette_small,
			&noggit::ui::texture_palette_small::selected,
			[=](std::string const& filename) {
				makeCurrent();
				opengl::context::scoped_setter const _(::gl, context());
	...
	connect(this, &QObject::destroyed, _texture_palette_small,
			&QObject::deleteLater);
	...
	TexturePicker =
		new noggit::ui::texture_picker(texturingTool->_current_texture, this);
	TexturePicker->hide();
	connect(this, &QObject::destroyed, TexturePicker, &QObject::deleteLater);

	connect(TexturePicker, &noggit::ui::texture_picker::set_texture,
	...
	if (_settings->value("undock_tool_properties/enabled", 1).toBool())
	{
		for (auto dock : _tool_properties_docks)
		{
			...
			dock->move(_main_window->geometry().topRight().x() -
						   dock->rect().width() - 20,
					   _main_window->geometry().topRight().y() + 40);
		}
	}
	...
	_texture_palette_dock->setAllowedAreas(Qt::BottomDockWidgetArea |
										   Qt::TopDockWidgetArea);
	_texture_palette_dock->setSizePolicy(
		QSizePolicy(QSizePolicy::Minimum, QSizePolicy::Minimum));

	_main_window->addDockWidget(Qt::BottomDockWidgetArea,
								_texture_palette_dock);

	if (_settings->value("undock_small_texture_palette/enabled", 1).toBool())
	{
		_texture_palette_dock->setFloating(true);
		_texture_palette_dock->move(
			_main_window->geometry().bottomLeft().x() + 50,
			_main_window->geometry().bottomLeft().y() - 200);
	}

	connect(this, &QObject::destroyed, _texture_palette_dock,
			&QObject::deleteLater);

	// create toolbar

	_toolbar = new noggit::ui::toolbar(
		[this](editing_mode mode) { set_editing_mode(mode); });
...
#define ADD_TOGGLE(menu_, name_, shortcut_, property_)                      \
	...
	{                                                                       \
		...
		connect(action, &QAction::toggled, &property_,                      \
				&noggit::bool_toggle_property::set);                        \
		connect(&property_, &noggit::bool_toggle_property::changed, action, \
				&QAction::setChecked);                                      \
	} while (false)
...
#define ADD_TOGGLE_NS(menu_, name_, property_)                              \
	...
	ADD_ACTION(file_menu, "Save current tile", "Ctrl+Shift+S",
			   [this] { save(save_mode::current); });
	ADD_ACTION(file_menu, "Save changed tiles", QKeySequence::Save,
			   [this] { save(save_mode::changed); });
	ADD_ACTION(file_menu, "Save all tiles", "Ctrl+Shift+A",
			   [this] { save(save_mode::all); });
	...
	edit_menu->addSeparator();
	ADD_ACTION(edit_menu, "Delete", Qt::Key_Delete,
			   [this] { DeleteSelectedObject(); });
	ADD_ACTION(edit_menu, "Reset rotation", "Ctrl+R",
			   [this] { ResetSelectedObjectRotation(); });
	ADD_ACTION(edit_menu, "Set to ground", Qt::Key_PageDown,
			   [this] { snap_selected_models_to_the_ground(); });
	...
	assist_menu->addSeparator();
	ADD_ACTION(assist_menu, "Last M2 from WMV", "Shift+V", [this] {
		objectEditor->import_last_model_from_wmv(eEntry_Model);
	});
	ADD_ACTION(assist_menu, "Last WMO from WMV", "Alt+V", [this] {
		objectEditor->import_last_model_from_wmv(eEntry_WMO);
	});
	...
	ADD_ACTION_NS(assist_menu, "Clear height map", [this] {
		makeCurrent();
		opengl::context::scoped_setter const _(::gl, context());
		_world->clearHeight(_camera.position);
	});
	ADD_ACTION_NS(assist_menu, "Remove texture duplicates", [this] {
		makeCurrent();
		opengl::context::scoped_setter const _(::gl, context());
		_world->removeTexDuplicateOnADT(_camera.position);
	});
	ADD_ACTION_NS(assist_menu, "Clear textures", [this] {
		makeCurrent();
		opengl::context::scoped_setter const _(::gl, context());
		_world->clearTextures(_camera.position);
	});
	ADD_ACTION_NS(assist_menu, "Clear textures + set base", [this] {
		makeCurrent();
		opengl::context::scoped_setter const _(::gl, context());
		_world->setBaseTexture(_camera.position);
	});
	ADD_ACTION_NS(assist_menu, "Clear shadows", [this] {
		makeCurrent();
		opengl::context::scoped_setter const _(::gl, context());
		_world->clear_shadows(_camera.position);
	});
	ADD_ACTION_NS(assist_menu, "Clear models", [this] {
		makeCurrent();
		opengl::context::scoped_setter const _(::gl, context());
		_world->clearAllModelsOnADT(_camera.position);
		_rotation_editor_need_update = true;
	});
	ADD_ACTION_NS(assist_menu, "Clear duplicate models", [this] {
		makeCurrent();
		opengl::context::scoped_setter const _(::gl, context());
	...
	connect(&_show_minimap_borders, &noggit::bool_toggle_property::changed,
			[this] { _minimap->draw_boundaries(_show_minimap_borders.get()); });
	...
	connect(&_show_minimap_skies, &noggit::bool_toggle_property::changed,
			[this] { _minimap->draw_skies(_show_minimap_skies.get()); });
	...
	auto hide_widgets = [=] {
		QWidget* widget_list[] = {TexturePalette,
								  TexturePicker,
								  guidetailInfos,
		...
		{
			for (auto widget : _main_window->displayed_widgets) widget->show();
	...
	ADD_TOGGLE(view_menu, "Cursor switcher", "Ctrl+Alt+C",
			   _show_cursor_switcher_window);
	connect(&_show_cursor_switcher_window,
			&noggit::bool_toggle_property::changed, _cursor_switcher.get(),
			&QWidget::setVisible);
	...
	ADD_TOGGLE(view_menu, "Texture palette", Qt::Key_X,
			   _show_texture_palette_window);
	connect(
		&_show_texture_palette_window, &noggit::bool_toggle_property::changed,
		TexturePalette, [this] {
	...
	ADD_TOGGLE_NS(view_menu, "Small texture palette",
				  _show_texture_palette_small_window);

	addHotkey(Qt::Key_H, MOD_none,
			  [this] { _show_texture_palette_small_window.toggle(); },
			  [this] { return terrainMode == editing_mode::paint; });

	connect(&_show_texture_palette_small_window,
			&noggit::bool_toggle_property::changed, _texture_palette_dock,
			[this] {
				if (terrainMode == editing_mode::paint && !ui_hidden)
				{
					_texture_palette_dock->setVisible(
						_show_texture_palette_small_window.get());
				}
				else
				{
	...
	addHotkey(Qt::Key_F1, MOD_shift, [this] {
		if (alloff)
		{
	...
	ADD_ACTION_NS(help_menu, "Forum", [] {
		ShellExecute(nullptr, "open",
					 "http://www.modcraft.io/index.php?board=48.0", nullptr,
					 nullptr, SW_SHOWNORMAL);
	});
	ADD_ACTION_NS(help_menu, "Homepage", [] {
		ShellExecute(nullptr, "open",
					 "https://bitbucket.org/berndloerwald/noggit3/", nullptr,
					 nullptr, SW_SHOWNORMAL);
	});

	ADD_ACTION_NS(help_menu, "Discord", [] {
		ShellExecute(nullptr, "open", "https://discord.gg/UbdFHyM", nullptr,
					 nullptr, SW_SHOWNORMAL);
	});
#endif

	ADD_ACTION(file_menu, "Add bookmark", Qt::Key_F5, [this] {
	...
	ADD_ACTION(view_menu, "Pause time", Qt::Key_J,
			   [this] { mTimespeed = 0.0f; });

	addHotkey(Qt::Key_C, MOD_ctrl,
			  [this] { objectEditor->copy_current_selection(_world.get()); },
			  [this] { return terrainMode == editing_mode::object; });
	addHotkey(Qt::Key_C, MOD_none,
			  [this] { objectEditor->copy_current_selection(_world.get()); },
			  [this] { return terrainMode == editing_mode::object; });

	addHotkey(Qt::Key_C, MOD_shift,
			  [this] {
				  do
				  {
					  cursor_type.set(
						  (cursor_type.get() + 1) %
						  static_cast<unsigned int>(cursor_mode::mode_count));
				  } while (cursor_type.get() ==
						   static_cast<unsigned int>(
							   cursor_mode::unused));  // hack to not get the
													   // unused cursor type
			  },
			  [this] { return terrainMode != editing_mode::object; });

	addHotkey(Qt::Key_V, MOD_ctrl,
			  [this] {
				  objectEditor->pasteObject(_cursor_pos, _camera.position,
											_world.get(),
											&_object_paste_params);
			  },
			  [this] { return terrainMode == editing_mode::object; });
	addHotkey(Qt::Key_V, MOD_none,
			  [this] {
				  objectEditor->pasteObject(_cursor_pos, _camera.position,
											_world.get(),
											&_object_paste_params);
	...
	addHotkey(
		...
		[this] { return terrainMode == editing_mode::object; });
	addHotkey(Qt::Key_V, MOD_alt,
			  [this] { objectEditor->import_last_model_from_wmv(eEntry_WMO); },
			  [this] { return terrainMode == editing_mode::object; });

	addHotkey(Qt::Key_C, MOD_none, [this] { _world->clearVertexSelection(); },
			  [this] { return terrainMode == editing_mode::ground; });

	addHotkey(Qt::Key_B, MOD_ctrl,
			  [this] {
	...
	ADD_ACTION(file_menu, "Save minimaps", "Ctrl+Shift+P",
			   [this] { Saving = true; });
	...
	ADD_ACTION(file_menu, "Write coordinates to port.txt", Qt::Key_G, [this] {
	...
	addHotkey(Qt::Key_Y, MOD_none, [this] { terrainTool->nextType(); },
			  [this] { return terrainMode == editing_mode::ground; });

	addHotkey(Qt::Key_Y, MOD_none, [this] { flattenTool->nextFlattenType(); },
	...
	addHotkey(Qt::Key_T, MOD_none, [&] { flattenTool->toggleFlattenAngle(); },
			  [&] { return terrainMode == editing_mode::flatten_blur; });

	addHotkey(Qt::Key_T, MOD_space, [&] { flattenTool->nextFlattenMode(); },
			  [&] { return terrainMode == editing_mode::flatten_blur; });

	addHotkey(Qt::Key_T, MOD_none, [&] { texturingTool->toggle_tool(); },
			  [&] { return terrainMode == editing_mode::paint; });

	addHotkey(Qt::Key_T, MOD_none,
			  [&] { _world->setHoleADT(_camera.position, false); },
			  [&] { return terrainMode == editing_mode::holes; });

	addHotkey(Qt::Key_T, MOD_alt,
			  [&] { _world->setHoleADT(_camera.position, true); },
			  [&] { return terrainMode == editing_mode::holes; });

	addHotkey(Qt::Key_T, MOD_none, [&] { guiWater->toggle_angled_mode(); },
			  [&] { return terrainMode == editing_mode::water; });

	addHotkey(Qt::Key_T, MOD_none, [&] { objectEditor->togglePasteMode(); },
			  ...
			  [&] {
				  if (_world->has_selection())
				  {
					  for (auto& selection : _world->current_selection())
					  {
						  ...
						  {
							  GetFromSelection<selected_wmo_type>(selection)
								  ->wmo->toggle_visibility();
						  }
					  }
				  }
			  },
	...
	addHotkey(Qt::Key_H, MOD_space, [&] { _draw_hidden_models.toggle(); },
			  [&] { return terrainMode == editing_mode::object; });

	addHotkey(Qt::Key_R, MOD_space,
			  [&] { texturingTool->toggle_brush_level_min_max(); },
			  [&] { return terrainMode == editing_mode::paint; });

	addHotkey(Qt::Key_H, MOD_shift,
			  [&] {
	...
	addHotkey(Qt::Key_F, MOD_space, [&] { flattenTool->toggleFlattenLock(); },
			  [&] { return terrainMode == editing_mode::flatten_blur; });
	addHotkey(Qt::Key_F, MOD_none, [&] { flattenTool->lockPos(_cursor_pos); },
			  [&] { return terrainMode == editing_mode::flatten_blur; });
	addHotkey(Qt::Key_F, MOD_space, [&] { guiWater->toggle_lock(); },
			  [&] { return terrainMode == editing_mode::water; });
	addHotkey(Qt::Key_F, MOD_none, [&] { guiWater->lockPos(_cursor_pos); },
			  [&] { return terrainMode == editing_mode::water; });
	addHotkey(Qt::Key_F, MOD_none,
			  [&] {
	...
	addHotkey(Qt::Key_Plus, MOD_alt,
			  [this] { terrainTool->changeRadius(0.01f); },
			  [this] { return terrainMode == editing_mode::ground; });

	addHotkey(Qt::Key_Plus, MOD_alt,
			  [this] { flattenTool->changeRadius(0.01f); },
			  [this] { return terrainMode == editing_mode::flatten_blur; });
	...
	addHotkey(Qt::Key_Plus, MOD_shift,
			  [this] { _world->fogdistance += 60.0f; });

	addHotkey(Qt::Key_Minus, MOD_alt,
			  [this] { terrainTool->changeRadius(-0.01f); },
			  [this] { return terrainMode == editing_mode::ground; });
	...
	addHotkey(Qt::Key_Minus, MOD_shift,
			  [this] { _world->fogdistance -= 60.0f; });
	...
	addHotkey(Qt::Key_1, MOD_alt,
			  [this] { texturingTool->set_brush_level(0.0f); });
	addHotkey(Qt::Key_2, MOD_alt,
			  [this] { texturingTool->set_brush_level(255.0f * 0.25f); });
	addHotkey(Qt::Key_3, MOD_alt,
			  [this] { texturingTool->set_brush_level(255.0f * 0.5f); });
	addHotkey(Qt::Key_4, MOD_alt,
			  [this] { texturingTool->set_brush_level(255.0f * 0.75f); });
	addHotkey(Qt::Key_5, MOD_alt,
			  [this] { texturingTool->set_brush_level(255.0f); });

	addHotkey(Qt::Key_1, MOD_none,
			  [this] { set_editing_mode(editing_mode::ground); },
			  [this] { return !_mod_num_down; });
	addHotkey(Qt::Key_2, MOD_none,
			  [this] { set_editing_mode(editing_mode::flatten_blur); },
			  [this] { return !_mod_num_down; });
	addHotkey(Qt::Key_3, MOD_none,
			  [this] { set_editing_mode(editing_mode::paint); },
			  [this] { return !_mod_num_down; });
	addHotkey(Qt::Key_4, MOD_none,
			  [this] { set_editing_mode(editing_mode::holes); },
			  [this] { return !_mod_num_down; });
	addHotkey(Qt::Key_5, MOD_none,
			  [this] { set_editing_mode(editing_mode::areaid); },
			  [this] { return !_mod_num_down; });
	addHotkey(Qt::Key_6, MOD_none,
			  [this] { set_editing_mode(editing_mode::flags); },
			  [this] { return !_mod_num_down; });
	addHotkey(Qt::Key_7, MOD_none,
			  [this] { set_editing_mode(editing_mode::water); },
			  [this] { return !_mod_num_down; });
	addHotkey(Qt::Key_8, MOD_none,
			  [this] { set_editing_mode(editing_mode::mccv); },
			  [this] { return !_mod_num_down; });
	addHotkey(Qt::Key_9, MOD_none,
			  [this] { set_editing_mode(editing_mode::object); },
			  [this] { return !_mod_num_down; });

	addHotkey(Qt::Key_0, MOD_ctrl,
			  [this] { change_selected_wmo_doodadset(0); });
	addHotkey(Qt::Key_1, MOD_ctrl,
			  [this] { change_selected_wmo_doodadset(1); });
	addHotkey(Qt::Key_2, MOD_ctrl,
			  [this] { change_selected_wmo_doodadset(2); });
	addHotkey(Qt::Key_3, MOD_ctrl,
			  [this] { change_selected_wmo_doodadset(3); });
	addHotkey(Qt::Key_4, MOD_ctrl,
			  [this] { change_selected_wmo_doodadset(4); });
	addHotkey(Qt::Key_5, MOD_ctrl,
			  [this] { change_selected_wmo_doodadset(5); });
	addHotkey(Qt::Key_6, MOD_ctrl,
			  [this] { change_selected_wmo_doodadset(6); });
	addHotkey(Qt::Key_7, MOD_ctrl,
			  [this] { change_selected_wmo_doodadset(7); });
	addHotkey(Qt::Key_8, MOD_ctrl,
			  [this] { change_selected_wmo_doodadset(8); });
	addHotkey(Qt::Key_9, MOD_ctrl,
			  [this] { change_selected_wmo_doodadset(9); });

	connect(_main_window, &noggit::ui::main_window::exit_prompt_opened, this,
			&MapView::on_exit_prompt);
}
...
MapView::MapView(math::degrees camera_yaw0, math::degrees camera_pitch0,
				 math::vector_3d camera_pos,
				 noggit::ui::main_window* main_window,
				 std::unique_ptr<World> world, uid_fix_mode uid_fix,
				 bool from_bookmark)
	  ...
	  _texture_palette_dock(new QDockWidget(this))
{
	...
	_minimap_dock->move(_main_window->rect().center() -
						_minimap->rect().center());
	...
	cursor_type.set(_settings
						->value("cursor/default_type",
								static_cast<unsigned int>(cursor_mode::terrain))
						.toUInt());
	...
	connect(&cursor_type, &noggit::unsigned_int_property::changed,
			[&](unsigned int type) {
	...
	_cursor_switcher.reset(
		new noggit::ui::cursor_switcher(this, cursor_color, cursor_type));
...
void MapView::initializeGL()
{
	...
	if (uid_warning)
	{
		QMessageBox::warning(
			...
			"It is recommanded to fix those models (listed in the log file) "
			"and run the uid fix all again.",
			QMessageBox::Ok);
	}
}
...
void MapView::paintGL()
{
	...
	if (_world->uid_duplicates_found() && !_uid_duplicate_warning_shown)
	{
		...
		QMessageBox::critical(
			this, "UID ALREADY IN USE",
			"Please enable 'Always check for max UID', mysql uid store or "
			"synchronize your "
			"uid.ini file if you're sharing the map between several "
			"mappers.\n\n"
...
MapView::~MapView()
{
	...
	if (!_uid_fix_failed)
	{
		delete TexturePicker;  // explicitly delete this here to avoid opengl
							   // context related crash
		delete objectEditor;
...
void MapView::tick(float dt)
{
	...
	_mod_num_down =
		QApplication::keyboardModifiers().testFlag(Qt::KeypadModifier);
	...
	if (_world->has_selection())
	{
		...
		if (terrainMode == editing_mode::object)
		{
			...
			if (!(keyx != 0 || keyy != 0 || keyz != 0 || keyr != 0 ||
				  keys != 0))
			{
			...
			if (keys != 0.f)
			{
				_world->scale_selected_models(keys * numpad_moveratio / 50.f,
											  World::m2_scaling_type::add);
				_rotation_editor_need_update = true;
			}
			if (keyr != 0.f)
			{
				_world->rotate_selected_models(
					...
					math::degrees(0.f), _use_median_pivot_point.get());
				_rotation_editor_need_update = true;
			}
			...
			if (MoveObj)
			{
				if (_mod_alt_down)
				{
					_world->scale_selected_models(std::pow(2.f, mv * 4.f),
			...
			{
				_world->move_selected_models(keyx * numpad_moveratio,
											 keyy * numpad_moveratio,
											 keyz * numpad_moveratio);
				_rotation_editor_need_update = true;
			}
			...
			if (look)
			{
				...
				{
					_world->rotate_selected_models(
						math::degrees(rh + rv), math::degrees(0.f),
						math::degrees(0.f), _use_median_pivot_point.get());
				}
				if (_mod_shift_down)  // Y
				{
					_world->rotate_selected_models(
						math::degrees(0.f), math::degrees(rh + rv),
						math::degrees(0.f), _use_median_pivot_point.get());
				}
				if (_mod_alt_down)  // Z
				{
					_world->rotate_selected_models(
						math::degrees(0.f), math::degrees(0.f),
						math::degrees(rh + rv), _use_median_pivot_point.get());
				}
				...
				switch (terrainMode)
				{
					case editing_mode::ground:
						if (_display_mode == display_mode::in_3D && !underMap)
						{
							if (_mod_shift_down)
							{
								terrainTool->changeTerrain(
									_world.get(), _cursor_pos, 7.5f * dt);
							}
							else if (_mod_ctrl_down)
							{
								terrainTool->changeTerrain(
					...
					case editing_mode::flatten_blur:
						if (_display_mode == display_mode::in_3D && !underMap)
						{
							if (_mod_shift_down)
							{
								flattenTool->flatten(_world.get(), _cursor_pos,
													 dt);
							}
							else if (_mod_ctrl_down)
							{
								flattenTool->blur(_world.get(), _cursor_pos,
												  dt);
							}
						}
						break;
					...
					case editing_mode::holes:
						// no undermap check here, else it's impossible to
						// remove holes
						if (_mod_shift_down)
						{
					...
					case editing_mode::areaid:
						if (!underMap)
						{
							if (_mod_shift_down)
							{
								// draw the selected AreaId on current selected
								// chunk
								_world->setAreaID(_cursor_pos,
												  _selected_area_id, false);
							}
							else if (_mod_ctrl_down)
							{
								// pick areaID from chunk
								MapChunk* chnk(
									GetFromSelection<selected_chunk_type>(
										selection)
										.chunk);
					...
					case editing_mode::flags:
						if (!underMap)
						{
							...
							if (_mod_shift_down)
							{
								_world->mapIndex.setFlag(true, _cursor_pos,
														 0x2);
							}
							else if (_mod_ctrl_down)
							{
								_world->mapIndex.setFlag(false, _cursor_pos,
														 0x2);
							}
						}
						break;
					case editing_mode::water:
						if (_display_mode == display_mode::in_3D && !underMap)
						{
							if (_mod_shift_down)
							{
								guiWater->paintLiquid(_world.get(), _cursor_pos,
													  true);
							}
							else if (_mod_ctrl_down)
							{
					...
					case editing_mode::mccv:
						if (!underMap)
						{
							if (_mod_shift_down)
							{
								shaderTool->changeShader(_world.get(),
														 _cursor_pos, dt, true);
							}
							if (_mod_ctrl_down)
							{
								shaderTool->changeShader(
									_world.get(), _cursor_pos, dt, false);
							}
						}
						break;
				}
	...
	{
		//! \todo this is total bullshit. there should be a seperate view and
		//! camera class for tilemode
		if (moving)
		{
	...
	status +=
		(QString("; coordinates client: (%1, %2, %3), server: (%4, %5, %6)")
	...
	else if (currentSelection.size() == 1)
	{
		switch (currentSelection.begin()->which())
		{
			case eEntry_Model:
			{
				auto instance(GetFromSelection<selected_model_type>(
					*currentSelection.begin()));
				_status_selection->setText(QString("%1: %2")
											   .arg(instance->uid)
											   .arg(QString::fromStdString(
												   instance->model->filename)));
				break;
			}
			case eEntry_WMO:
			{
				auto instance(GetFromSelection<selected_wmo_type>(
					*currentSelection.begin()));
				_status_selection->setText(
					QString("%1: %2")
			...
			{
				auto chunk(GetFromSelection<selected_chunk_type>(
							   *currentSelection.begin())
							   .chunk);
				_status_selection->setText(
	...
	_status_area->setText(QString::fromStdString(
...

	{
		...
		if (_tablet_active &&
			_settings->value("tablet/enabled", false).toBool())
		{
	...
	{
		auto avg_frame_duration(std::accumulate(_last_frame_durations.begin(),
												_last_frame_durations.end(),
												0.) /
								qreal(_last_frame_durations.size()));
		_status_fps->setText(
			"FPS: " + QString::number(int(1. / avg_frame_duration)) +
			" - Average frame time: " +
			QString::number(avg_frame_duration * 1000.0) + "ms");
	...
	if (guidetailInfos->isVisible())
	{
		if (currentSelection.size() > 0)
		{
			...
			switch (lastSelection.which())
			{
				...
				{
					auto instance(
						GetFromSelection<selected_model_type>(lastSelection));
					select_info
						<< "filename: " << instance->model->filename
						<< "\nunique ID: " << instance->uid
						<< "\nposition X/Y/Z: " << instance->pos.x << " / "
						<< instance->pos.y << " / " << instance->pos.z
						<< "\nrotation X/Y/Z: " << instance->dir.x << " / "
						<< instance->dir.y << " / " << instance->dir.z
						<< "\nscale: " << instance->scale << "\ntextures Used: "
						<< instance->model->header.nTextures
						<< "\nsize category: " << instance->size_cat;

					for (unsigned int j = 0;
						 j < std::min(instance->model->header.nTextures, 6U);
						 j++)
					{
						select_info << "\n " << (j + 1) << ": "
									<< instance->model->_textures[j]->filename;
					}
				...
				{
					auto instance(
						GetFromSelection<selected_wmo_type>(lastSelection));
					select_info
						<< "filename: " << instance->wmo->filename
						...
						<< "\ntextures used: "
						<< instance->wmo->textures.size();

					const unsigned int texture_count(std::min(
						(unsigned int)(instance->wmo->textures.size()), 8U));
					for (unsigned int j = 0; j < texture_count; j++)
					{
						select_info << "\n " << (j + 1) << ": "
									<< instance->wmo->textures[j]->filename;
					}
				...
				{
					auto chunk(
						GetFromSelection<selected_chunk_type>(lastSelection)
							.chunk);
					mcnk_flags const& flags = chunk->header_flags;

					select_info
						<< "MCNK " << chunk->px << ", " << chunk->py << " ("
						<< chunk->py * 16 + chunk->px << ") of tile ("
						<< chunk->mt->index.x << " " << chunk->mt->index.z
						<< ")"
						<< "\narea ID: " << chunk->getAreaID() << " (\""
						<< gAreaDB.getAreaName(chunk->getAreaID()) << "\")"
						<< "\nflags: "
					...
					//! \todo get a list of textures and their flags as well as
					//! detail doodads.
...
float MapView::aspect_ratio() const { return float(width()) / float(height()); }
...
math::ray MapView::intersect_ray() const
{
	...
	if (_display_mode == display_mode::in_3D)
	{
		...
		math::vector_3d const pos(((projection() * model_view()).inverted() *
...
selection_result MapView::intersect_result(bool terrain_only)
{
	selection_result results(_world->intersect(
		model_view().transposed(), intersect_ray(), terrain_only,
		terrainMode == editing_mode::object, _draw_terrain.get(),
		_draw_wmo.get(), _draw_models.get(), _draw_hidden_models.get()));
...
void MapView::doSelection(bool selectTerrainOnly)
{
	...
	{
		const selection_type& hit(results.front().second);

		if (QGuiApplication::queryKeyboardModifiers().testFlag(
				Qt::ShiftModifier))
		{
		...
		_cursor_pos =
			hit.which() == eEntry_Model
				? GetFromSelection<selected_model_type>(hit)->pos
				: hit.which() == eEntry_WMO
					  ? GetFromSelection<selected_wmo_type>(hit)->pos
					  : hit.which() == eEntry_MapChunk
							? GetFromSelection<selected_chunk_type>(hit)
								  .position
...
void MapView::update_cursor_pos()
{
	...
	if (!results.empty())
	{
		...
		_cursor_pos = GetFromSelection<selected_chunk_type>(hit).position;
	}
}
...
math::matrix_4x4 MapView::projection() const
{
	...
	if (_display_mode == display_mode::in_2D)
	{
		...
		return math::ortho(-half_width, half_width, -half_height, half_height,
						   -1.f, far_z);
	}
	else
	{
...
void MapView::draw_map()
{
	...
	//! \note Select terrain below mouse, if no item selected or the item is
	//! map.
	if (!(_world->has_selection() || _locked_cursor_mode.get()))
	{
		doSelection(true);
	}

	_world->draw(
		model_view().transposed(), projection().transposed(), _cursor_pos,
		terrainMode == editing_mode::mccv ? shader_color : cursor_color,
		cursor_type.get(), radius, texturingTool->show_unpaintable_chunks(),
		_draw_contour.get(), inner_radius, ref_pos, angle, orientation,
		use_ref_pos, angled_mode, terrainMode == editing_mode::paint,
		terrainMode == editing_mode::flags, terrainMode == editing_mode::areaid,
		terrainMode, _camera.position, _camera_moved_since_last_draw,
		_draw_mfbo.get(), _draw_wireframe.get(), _draw_lines.get(),
		_draw_terrain.get(), _draw_wmo.get(), _draw_water.get(),
		_draw_wmo_doodads.get(), _draw_models.get(),
		_draw_model_animations.get(),
		_draw_hole_lines.get() || terrainMode == editing_mode::holes,
		_draw_models_with_box.get(), _draw_hidden_models.get(), _area_id_colors,
		_draw_fog.get(), terrainTool->_edit_type,
...
void MapView::keyPressEvent(QKeyEvent* event)
{
	...
	{
		if (event->key() == hotkey.key && modifier == hotkey.modifiers &&
			hotkey.condition())
		{
			makeCurrent();
			opengl::context::scoped_setter const _(::gl, context());
	...
	if (event->key() == Qt::Key_Space) _mod_space_down = true;
	...
	{
		_camera.position =
			math::vector_3d(_cursor_pos.x, _cursor_pos.y + 50, _cursor_pos.z);
		;
		_minimap->update();
	}
...
{
	if (event->key() == Qt::Key_Space) _mod_space_down = false;
	...
	if ((event->key() == Qt::Key_2 || event->key() == Qt::Key_8) &&
	...
	if ((event->key() == Qt::Key_3 || event->key() == Qt::Key_1) &&
		event->modifiers() & Qt::KeypadModifier)
	{
		keyy = 0.0f;
	}
...
void MapView::mouseMoveEvent(QMouseEvent* event)
{
	...
	if ((look || freelook) && !(_mod_shift_down || _mod_ctrl_down ||
								_mod_alt_down || _mod_space_down))
	{
		_camera.add_to_yaw(math::degrees(relative_movement.dx() / XSENS));
		_camera.add_to_pitch(
			math::degrees(mousedir * relative_movement.dy() / YSENS));
		_camera_moved_since_last_draw = true;
		_minimap->update();
	}
	...
	if (MoveObj)
	{
		mh = -aspect_ratio() * relative_movement.dx() /
	...
	if (rightMouse && _mod_shift_down)
	{
		if (terrainMode == editing_mode::ground)
		{
			terrainTool->moveVertices(_world.get(),
									  -relative_movement.dy() / YSENS);
		}
	}
	...
	if (_display_mode == display_mode::in_2D && leftMouse && _mod_alt_down &&
		_mod_shift_down)
	{
...
{
	if (StrEndsWith(model, ".m2"))
	{
		ModelInstance mi(model);
		_world->set_current_selection(
			GetFromSelection<selected_model_type>(&mi));
	}
	else if (StrEndsWith(model, ".wmo"))
	{
		WMOInstance wi(model);
		_world->set_current_selection(GetFromSelection<selected_wmo_type>(&wi));
	}
...
void MapView::change_selected_wmo_doodadset(int set)
{
	for (auto& selection : _world->current_selection())
	{
		if (selection.which() == eEntry_WMO)
		{
			auto wmo = GetFromSelection<selected_wmo_type>(selection);
...
void MapView::wheelEvent(QWheelEvent* event)
{
	...
	auto&& delta_for_range([&](float range) {
		...
		return (_mod_ctrl_down ? 0.01f : 0.1f) *
			   ...
			   * (_mod_alt_down ? event->angleDelta().x()
								: event->angleDelta().y()) /
			   320.f;
	});
...
void MapView::mouseReleaseEvent(QMouseEvent* event)
{
	switch (event->button())
	{
		...
		case Qt::RightButton:
			...
			if (_display_mode == display_mode::in_2D) updown = 0;
...
void MapView::save(save_mode mode)
{
	...
	if (AsyncLoader::instance().important_object_failed_loading())
	{
		...
		first_warning.setText(
			"Error:\nSome models could not be loaded and saving will cause "
			"collision and culling issues, would you still like to save ?");
		// roles are swapped to force the user to pay attention and both are
		// "accept" roles so that escape does nothing
		...
		if (first_warning.clickedButton() == yes)
		{
			...
			second_warning.setText(
				"If you save you will have to save again all the adt "
				"containing the defective/missing models once you've fixed "
				"said models to correct all the issues.\n"
				"By clicking yes you accept to bear all the consequences of "
				"your action and forfeit the right to complain to the "
				"developers about any culling and collision issues.\n\n"
				"So... do you REALLY want to save ?");
			no = second_warning.addButton("No",
										  QMessageBox::ButtonRole::YesRole);
			yes = second_warning.addButton("Yes",
	...
	if (mode == save_mode::current && save &&
		(QMessageBox::warning(
			 nullptr, "Save current map tile only",
			 "This can cause a collision bug when placing objects between two "
			 "ADT borders!\n\n"
	...
	{
		QMessageBox::warning(
			nullptr, "Map NOT saved",
			"The map wasn NOT saved, don't forget to save before leaving",
			QMessageBox::Ok);
	}
}

void MapView::addHotkey(Qt::Key key, size_t modifiers,
