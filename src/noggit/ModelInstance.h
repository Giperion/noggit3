...
class ModelInstance
{
   public:
	...
	constexpr static float max_scale()
	{
	...
	// longest side of an AABB transformed model's bounding box from the M2
	// header
	float size_cat;
	...
	void intersect(math::matrix_4x4 const& model_view, math::ray const&,
				   selection_result*, int animtime);

	math::matrix_4x4 const& transform_matrix_transposed() const
	{
	...
	bool is_visible(math::frustum const& frustum, const float& cull_distance,
