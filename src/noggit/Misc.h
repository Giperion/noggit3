...
namespace misc
{
	...
	float getShortestDist(float x, float z, float squareX, float squareZ,
						  float unitSize);
	float getShortestDist(math::vector_3d const& pos,
						  math::vector_3d const& square_pos, float unitSize);
	bool square_is_in_circle(float x, float z, float radius, float square_x,
							 float square_z, float square_size);
	bool rectOverlap(math::vector_3d const*, math::vector_3d const*);
	// used for angled tools, get the height a point (pos) should be given an
	// origin, angle and orientation
	float angledHeight(math::vector_3d const& origin,
					   math::vector_3d const& pos, math::radians const& angle,
					   math::radians const& orientation);
	void extract_v3d_min_max(math::vector_3d const& point, math::vector_3d& min,
							 math::vector_3d& max);
	std::vector<math::vector_3d> intersection_points(
		math::vector_3d const& vmin, math::vector_3d const& vmax);
	...
	{
		return std::abs(a - b) < (std::max(1.f, std::max(a, b)) *
	...
	{
		return std::distance(range.begin(),
...
class sExtendableArray
{
   public:
	...
	void Allocate(unsigned long pSize) { data.resize(pSize); }

	void Extend(long pAddition) { data.resize(data.size() + pAddition); }
	...
	void Insert(unsigned long pPosition, unsigned long pAddition,
				const char* pAdditionalData)
	{
		data.insert(data.begin() + pPosition, pAdditionalData,
...
void SetChunkHeader(sExtendableArray& pArray, int pPosition, int pMagix,
					int pSize = 0);
