...
class liquid_layer
{
   public:
	...
	liquid_layer(MPQFile& f, std::size_t base_pos, math::vector_3d const& base,
	...
	void save(sExtendableArray& adt, int base_pos, int& info_pos,
			  int& current_pos) const;

	void draw(liquid_render& render, opengl::scoped::use_program& water_shader,
			  math::vector_3d const& camera, bool camera_moved, int animtime);
	void update_indices();
