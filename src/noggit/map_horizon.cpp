...
struct color
{
	...
	operator uint32_t() const { return to_int(); }
...
struct ranged_color
{
	ranged_color(const color& c, const int16_t& start, const int16_t& stop)
		: _color(c), _start(start), _stop(stop)
	{
	}
...
static inline uint32_t color_for_height(int16_t height)
{
	...
	for (size_t i(0); i < num_colors - 1; ++i)
	{
		...
		{
			t = float(height - colors[i]._start) /
				float(colors[i]._stop - colors[i]._start);
			correct_color = i;
			break;
		}
	}
	...
	return lerp_color(colors[correct_color]._color,
					  colors[correct_color + 1]._color, t);
}
namespace noggit
{
	map_horizon::map_horizon(const std::string& basename,
							 const MapIndex* const index)
	{
				...
				case 'MAOF':
				{
					...
					for (size_t y(0); y < 64; ++y)
					{
						for (size_t x(0); x < 64; ++x)
						{
							...
							//! \todo There also is MAHO giving holes into this
							//! heightmap.
							wdl_file.read(_tiles[y][x]->height_17,
										  17 * 17 * sizeof(int16_t));
							wdl_file.read(_tiles[y][x]->height_16,
		...
		for (size_t y(0); y < 64; ++y)
		{
			for (size_t x(0); x < 64; ++x)
			{
				if (_tiles[y][x])
				{
					//! \todo There also is a second heightmap appended which
					//! has additional 16*16 pixels. \todo There also is MAHO
					//! giving holes into this heightmap.
				...
				else if (index->hasTile(tile_index(x, y)))
				{
					for (size_t j(0); j < 16; ++j)
					{
						for (size_t i(0); i < 16; ++i)
						{
							_qt_minimap.setPixel(x * 16 + i, y * 16 + j,
												 color(200, 100, 25));
						}
					}
				}
	...
	map_horizon::minimap::minimap(const map_horizon& horizon)
	{
		...
		for (size_t y(0); y < 64; ++y)
		{
			for (size_t x(0); x < 64; ++x)
			{
				if (!horizon._tiles[y][x]) continue;

				//! \todo There also is a second heightmap appended which has
				//! additional 16*16 pixels.

				// use the (nearly) full resolution available to us.
				// the data is layed out as a triangle fans with with 17 outer
				// values and 16 midpoints per tile. which in turn means:
				//      _tiles[y][x]->height_17[16][16] == _tiles[y][x +
				//      1]->height_17[0][0]
				for (size_t j(0); j < 16; ++j)
				{
					for (size_t i(0); i < 16; ++i)
					{
						texture[(y * 16 + j) * 1024 + x * 16 + i] =
							color_for_height(
		...
		bind();
		gl.texImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, 1024, 1024, 0, GL_BGRA,
	...
	map_horizon::render::render(const map_horizon& horizon)
	{
		...
		for (size_t y(0); y < 64; ++y)
		{
			for (size_t x(0); x < 64; ++x)
			{
				...
				_batches[y][x] =
					map_horizon_batch(vertices.size(), 17 * 17 + 16 * 16);
		...
		gl.bufferData<GL_ARRAY_BUFFER, math::vector_3d>(
			_vertex_buffer, vertices, GL_STATIC_DRAW);
	}

	static inline uint32_t outer_index(const map_horizon_batch& batch, int y,
									   int x)
	{
		return batch.vertex_start + y * 17 + x;
	};

	static inline uint32_t inner_index(const map_horizon_batch& batch, int y,
									   int x)
	{
		return batch.vertex_start + 17 * 17 + y * 16 + x;
	};

	void map_horizon::render::draw(
		math::matrix_4x4 const& model_view, math::matrix_4x4 const& projection,
		MapIndex* index, const math::vector_3d& color,
		...
		const math::vector_3d& camera, display_mode display)
	{
		...
		{
			for (size_t x(current_index.x - lrr); x < current_index.x + lrr;
				 ++x)
			{
				// x and y are unsigned so negative signed int value are
				// positive and > 63
				if (x > 63 || y > 63)
				{
					continue;
				...
				for (size_t j(0); j < 16; ++j)
				{
					for (size_t i(0); i < 16; ++i)
					{
						...
						if (index->tileLoaded({y, x}) &&
							index->getTile({y, x})->getChunk(j, i)->is_visible(
								cull_distance, frustum, camera, display))
						{
							continue;
						}
		...
		gl.bufferData<GL_ELEMENT_ARRAY_BUFFER, std::uint32_t>(
			_index_buffer, indices, GL_STATIC_DRAW);
		...
		if (!_map_horizon_program)
		{
			_map_horizon_program.reset(new opengl::program{
				...
				{GL_FRAGMENT_SHADER,
				 opengl::shader::src_from_qrc("horizon_fs")}});
