...
namespace noggit
{
	...
	std::uint32_t world_model_instances_storage::add_model_instance(
		ModelInstance instance, bool from_reloading)
	{
...

		{
			...
			uid_after =
				unsafe_add_model_instance_no_world_upd(std::move(instance));
		}
	...
	std::uint32_t
	world_model_instances_storage::unsafe_add_model_instance_no_world_upd(
	...
	std::uint32_t world_model_instances_storage::add_wmo_instance(
		WMOInstance instance, bool from_reloading)
	{
...

		{
			std::lock_guard<std::mutex> const lock(_mutex);
			uid_after =
				unsafe_add_wmo_instance_no_world_upd(std::move(instance));
		}
	...
	std::uint32_t
	world_model_instances_storage::unsafe_add_wmo_instance_no_world_upd(
	...
	void world_model_instances_storage::delete_instances_from_tile(
		tile_index const& tile)
	{
	...
	void world_model_instances_storage::delete_instances(
		std::vector<selection_type> const& instances)
	{
		...
		for (auto& it : instances)
		{
			...
			{
				selected_model_type instance =
					GetFromSelection<selected_model_type>(it);
			...
			{
				selected_wmo_type instance =
					GetFromSelection<selected_wmo_type>(it);
	...
	void world_model_instances_storage::
		unload_instance_and_remove_from_selection_if_necessary(
			std::uint32_t uid)
	{
		std::unique_lock<std::mutex> const lock(_mutex);
		...
		{
			LogError << "Trying to unload an instance that wasn't stored"
					 << endl;
			return;
		}
	...
	world_model_instances_storage::get_model_instance(std::uint32_t uid)
	{
		std::unique_lock<std::mutex> const lock(_mutex);
	...
	std::optional<ModelInstance*>
	world_model_instances_storage::unsafe_get_model_instance(std::uint32_t uid)
	{
	...
	std::optional<WMOInstance*> world_model_instances_storage::get_wmo_instance(
		std::uint32_t uid)
	{
		std::unique_lock<std::mutex> const lock(_mutex);
	...
	world_model_instances_storage::unsafe_get_wmo_instance(std::uint32_t uid)
	{
	...
	std::optional<selection_type> world_model_instances_storage::get_instance(
		std::uint32_t uid)
	{
		std::unique_lock<std::mutex> const lock(_mutex);
	...
	bool world_model_instances_storage::unsafe_uid_is_used(
		std::uint32_t uid) const
	{
		return _instance_count_per_uid.find(uid) !=
			   _instance_count_per_uid.end();
	}
	...
	void world_model_instances_storage::clear_duplicates()
	{
		...
		for (auto lhs(_m2s.begin()); lhs != _m2s.end(); ++lhs)
		{
			for (auto rhs(std::next(lhs)); rhs != _m2s.end();)
			{
				...
				if (lhs->second.is_a_duplicate_of(rhs->second))
				{
					_world->updateTilesModel(&rhs->second,
											 model_update::remove);
		...
		Log << "Deleted " << deleted_uids << " duplicate Model/WMO"
			<< endl;
	}
}  // namespace noggit
