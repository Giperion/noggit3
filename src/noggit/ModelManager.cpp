...
{
	_.apply([&](std::string const&, Model& model) { model.animcalc = false; });
}

void ModelManager::updateEmitters(float dt)
{
	_.apply(
		[&](std::string const&, Model& model) { model.updateEmitters(dt); });
}

void ModelManager::clear_hidden_models()
{
	_.apply([&](std::string const&, Model& model) { model.show(); });
}
