...
class Bone
{
   ...
   public:
	...
	bool calc;
	void calcMatrix(math::matrix_4x4 const& model_view, Bone* allbones,
					int anim, int time, int animtime);
	Bone(const MPQFile& f, const ModelBoneDef& b, int* global,
...
struct ModelRenderPass : ModelTexUnit
{
	...
	bool operator<(const ModelRenderPass& m) const
	{
		...
		{
			return blend_mode == m.blend_mode
					   ? (ordering_thingy < m.ordering_thingy)
...
class Model : public AsyncObject
{
   public:
	template <typename T>
	static std::vector<T> M2Array(MPQFile const& f, uint32_t offset,
								  uint32_t count)
	{
	...
	void draw(math::matrix_4x4 const& model_view, ModelInstance& instance,
			  opengl::scoped::use_program& m2_shader,
			  math::frustum const& frustum, const float& cull_distance,
			  const math::vector_3d& camera, int animtime, bool draw_particles,
			  bool all_boxes, display_mode display);
	void draw(math::matrix_4x4 const& model_view,
			  std::vector<ModelInstance*> instances,
			  opengl::scoped::use_program& m2_shader,
			  math::frustum const& frustum, const float& cull_distance,
			  const math::vector_3d& camera, bool draw_fog, int animtime,
			  bool draw_particles, bool all_boxes,
	...
	void draw_box(opengl::scoped::use_program& m2_box_shader,
				  std::size_t box_count);

	std::vector<float> intersect(math::matrix_4x4 const& model_view,
								 math::ray const&, int animtime);
	...
	virtual bool is_required_when_saving() const { return true; }
   ...
   private:
	...
	void animate(math::matrix_4x4 const& model_view, int anim_id,
				 int anim_time);
	void calcBones(math::matrix_4x4 const& model_view, int anim, int time,
				   int animation_time);
	...
	std::map<uint16_t, std::map<uint16_t, ModelAnimation>>
		_animations_seq_per_id;
