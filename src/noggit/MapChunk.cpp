...
MapChunk::MapChunk(MapTile* maintile, MPQFile* f, bool bigAlpha, tile_mode mode)
	: _mode(mode), mt(maintile), use_big_alphamap(bigAlpha)
{
	uint32_t fourcc;
	uint32_t size;
	...
	texture_set =
		std::make_unique<TextureSet>(header, f, base, maintile, bigAlpha,
									 !!header_flags.flags.do_not_fix_alpha_map,
									 mode == tile_mode::uid_fix_all);
	...
	// - MCVT ----------------------------------------------
	{
		...
		for (int j = 0; j < 17; ++j)
		{
			for (int i = 0; i < ((j % 2) ? 8 : 9); ++i)
			{
				...
				math::vector_3d v =
					math::vector_3d(xbase + xpos, ybase + h, zbase + zpos);
				*ttv++ = v;
	...
	// - MCNR ----------------------------------------------
	{
		...
		{
			f->read(nor, 3);
			*ttn++ = math::vector_3d(nor[0] / 127.0f, nor[2] / 127.0f,
									 nor[1] / 127.0f);
		}
	}
	// - MCSH ----------------------------------------------
	...
	if (header.ofsMCCV)
	{
		...
		for (int i = 0; i < mapbufsize; ++i)
		{
			f->read(t, 4);
			mccv[i] =
				math::vector_3d((float)t[2] / 127.0f, (float)t[1] / 127.0f,
								(float)t[0] / 127.0f);
		}
	}
	else
	...
	if (header.sizeLiquid > 8)
	{
		...
		f->read(&fourcc, 4);
		f->seekRelative(
			4);  // ignore the size here, the valid size is in the header
...
int MapChunk::indexLoD(int x, int y) { return (x + 1) * 9 + x * 8 + y; }

int MapChunk::indexNoLoD(int x, int y) { return x * 8 + x * 9 + y; }
...
void MapChunk::upload()
{
	...
	if (_mode == tile_mode::uid_fix_all)
	{
		throw std::logic_error(
			"Trying to render an ADT/chunk that is supposed to be used for the "
			"uid fix all only");
	}
	...
	// fill vertex buffers
	gl.bufferData<GL_ARRAY_BUFFER>(_vertices_vbo, sizeof(mVertices), mVertices,
								   GL_STATIC_DRAW);
	gl.bufferData<GL_ARRAY_BUFFER>(_normals_vbo, sizeof(mNormals), mNormals,
								   GL_STATIC_DRAW);
	gl.bufferData<GL_ARRAY_BUFFER>(_mccv_vbo, sizeof(mccv), mccv,
								   GL_STATIC_DRAW);
...
{
	{
		opengl::scoped::buffer_binder<GL_ELEMENT_ARRAY_BUFFER> const _(
			_indices_buffer);
		gl.bufferData(GL_ELEMENT_ARRAY_BUFFER,
					  strip_with_holes.size() * sizeof(StripType),
					  strip_with_holes.data(), GL_STATIC_DRAW);
	}

	for (int lod_level = 0; lod_level < 4; ++lod_level)
	{
		opengl::scoped::buffer_binder<GL_ELEMENT_ARRAY_BUFFER> const _(
			lod_indices[lod_level]);
		gl.bufferData(GL_ELEMENT_ARRAY_BUFFER,
					  strip_lods[lod_level].size() * sizeof(StripType),
					  strip_lods[lod_level].data(), GL_STATIC_DRAW);
	}

	_need_indice_buffer_update = false;
}

std::optional<int> MapChunk::get_lod_level(math::vector_3d const& camera_pos,
		...
		for (int y = 0; y < 8; ++y)
		{
			...
			for (int lod_level = 0; lod_level < 4; ++lod_level)
			{
				...
				if ((x % n) == 0 && (y % n) == 0)
				{
					...
					strip_lods[lod_level].emplace_back(
						indexNoLoD(y + n, x));  // 17
					strip_lods[lod_level].emplace_back(
						indexNoLoD(y + n, x + n));  // 18
					strip_lods[lod_level].emplace_back(
						indexNoLoD(y + n, x + n));  // 18
					strip_lods[lod_level].emplace_back(
						indexNoLoD(y, x + n));							   // 1
...
bool MapChunk::GetVertex(float x, float z, math::vector_3d* V)
{
	...
	const int column = static_cast<int>(
		(xdiff - UNITSIZE * 0.5f * (row % 2)) / UNITSIZE + 0.5f);
	if ((row < 0) || (column < 0) || (row > 16) ||
		(column > ((row % 2) ? 8 : 9)))
		return false;
...
void MapChunk::clearHeight()
{
	...
	if (_uploaded)
	{
		gl.bufferData<GL_ARRAY_BUFFER>(_vertices_vbo, sizeof(mVertices),
...
bool MapChunk::is_visible(const float& cull_distance,
						  ...
						  display_mode display) const
{
	static const float chunk_radius = std::sqrt(
		CHUNKSIZE * CHUNKSIZE / 2.0f);  // was (vmax - vmin).length() * 0.5f;

	float dist = display == display_mode::in_3D
					 ? (camera - vcenter).length() - chunk_radius
					 : std::abs(camera.y - vmax.y);

	return frustum.intersects(_intersect_points) && dist < cull_distance;
}

void MapChunk::update_vao(opengl::scoped::use_program& mcnk_shader,
...
void MapChunk::draw(math::frustum const& frustum,
					...
					int animtime, display_mode display)
{
	...
	bool cantPaint = noggit::ui::selected_texture::get() &&
					 ...
					 show_unpaintable_chunks && draw_paintability_overlay;
	...
	if (texture_set->num())
	{
		...
		for (int i = 0; i < texture_set->num(); ++i)
		{
			...
			{
				mcnk_shader.uniform("tex_anim_" + std::to_string(i),
									texture_set->anim_uv_offset(i, animtime));
			}
		}
	}
	...
	{
		mcnk_shader.uniform("draw_impassible_flag",
							(int)header_flags.flags.impass);
	}

	if (draw_areaid_overlay)
	{
		mcnk_shader.uniform("areaid_color",
							(math::vector_4d)area_id_colors[areaID]);
	}
	...
	if (lod_changed)
	{
		gl.bindBuffer(GL_ELEMENT_ARRAY_BUFFER,
					  !_lod_level ? _indices_buffer : lod_indices[*_lod_level]);
		_lod_level_indice_count = !_lod_level ? strip_with_holes.size()
											  : strip_lods[*_lod_level].size();
	}

	gl.drawElements(GL_TRIANGLES, _lod_level_indice_count, GL_UNSIGNED_SHORT,
					nullptr);
...
void MapChunk::intersect(math::ray const& ray, selection_result* results)
{
	...
	{
		if (auto distance =
				ray.intersect_triangle(mVertices[strip_without_holes[i + 0]],
									   mVertices[strip_without_holes[i + 1]],
									   mVertices[strip_without_holes[i + 2]]))
		{
			results->emplace_back(
				*distance,
				selected_chunk_type(this, i, ray.position(*distance)));
		}
	}
}
...
{
	auto point([&](math::vector_3d& v, float xdiff, float zdiff) {
		return math::vector_3d(v.x + xdiff,
	...
	for (int i = 0; i < mapbufsize; ++i)
	{
		...
		//! \todo: find out why recalculating normals without changing the
		//! terrain result in slightly different normals
...
bool MapChunk::changeTerrain(math::vector_3d const& pos, float change,
							 float radius, int BrushType, float inner_radius)
{
	...
	for (int i = 0; i < mapbufsize; ++i)
	{
		...
		{
			if ((std::abs(xdiff) < std::abs(radius / 2)) &&
				(std::abs(zdiff) < std::abs(radius / 2)))
			{
				dist = std::sqrt(xdiff * xdiff + zdiff * zdiff);
				mVertices[i].y +=
					change * (1.0f - dist * inner_radius / radius);
				changed = true;
			}
		}
		else
			...
			if (dist < radius)
			{
				...
				switch (BrushType)
				{
					...
					case eTerrainType_Linear:
						mVertices[i].y +=
							change *
							(1.0f - dist * (1.0f - inner_radius) / radius);
						break;
					...
					case eTerrainType_Polynom:
						mVertices[i].y +=
							change * ((dist / radius) * (dist / radius) +
									  dist / radius + 1.0f);
						break;
					...
					case eTerrainType_Gaussian:
						mVertices[i].y +=
							dist < radius * inner_radius
								? change * std::exp(-(
											   std::pow(radius * inner_radius /
															radius,
														2) /
											   (2 * std::pow(0.39f, 2))))
								: change *
									  std::exp(-(std::pow(dist / radius, 2) /
												 (2 * std::pow(0.39f, 2))));
...
bool MapChunk::ChangeMCCV(math::vector_3d const& pos,
...
bool MapChunk::flattenTerrain(math::vector_3d const& pos, float remain,
							  float radius, int BrushType,
							  flatten_mode const& mode,
							  math::vector_3d const& origin,
							  math::degrees angle, math::degrees orientation)
{
	bool changed(false);
	...
	for (int i(0); i < mapbufsize; ++i)
	{
		...
		mVertices[i].y = math::interpolation::linear(
			BrushType == eFlattenType_Flat
				? remain
				: BrushType == eFlattenType_Linear
					  ? remain * (1.f - dist / radius)
					  : BrushType == eFlattenType_Smooth
							? pow(remain, 1.f + dist / radius)
...
bool MapChunk::blurTerrain(
	math::vector_3d const& pos, float remain, float radius, int BrushType,
	flatten_mode const& mode,
	std::function<std::optional<float>(float, float)> height)
{
	bool changed(false);
	...
	for (int i(0); i < mapbufsize; ++i)
	{
		...
		for (int j = -Rad * 2; j <= Rad * 2; ++j)
		{
			...
			for (int k = -Rad; k <= Rad; ++k)
			{
				...
				float dist2 =
					misc::dist(tx, tz, mVertices[i].x, mVertices[i].z);
				if (dist2 > radius) continue;
				auto h(height(tx, tz));
				if (h)
				{
...
void MapChunk::eraseTextures() { texture_set->eraseTextures(); }

void MapChunk::change_texture_flag(scoped_blp_texture_reference const& tex,
...
void MapChunk::switchTexture(scoped_blp_texture_reference const& oldTexture,
							 scoped_blp_texture_reference newTexture)
{
	texture_set->replace_texture(oldTexture, std::move(newTexture));
}

bool MapChunk::paintTexture(math::vector_3d const& pos, Brush* brush,
							float strength, float pressure,
							scoped_blp_texture_reference texture)
{
	return texture_set->paintTexture(xbase, zbase, pos.x, pos.z, brush,
									 strength, pressure, std::move(texture));
}

bool MapChunk::replaceTexture(math::vector_3d const& pos, float radius,
							  scoped_blp_texture_reference const& old_texture,
							  scoped_blp_texture_reference new_texture)
{
	return texture_set->replace_texture(xbase, zbase, pos.x, pos.z, radius,
										old_texture, std::move(new_texture));
}
...
{
	shadow.bind();
	gl.texImage2D(GL_TEXTURE_2D, 0, GL_RED, 64, 64, 0, GL_RED, GL_UNSIGNED_BYTE,
				  _shadow_map);
...
void MapChunk::setHole(math::vector_3d const& pos, bool big, bool add)
{
	...
	{
		int v = 1 << ((int)((pos.z - zbase) / MINICHUNKSIZE) * 4 +
					  (int)((pos.x - xbase) / MINICHUNKSIZE));
...
void MapChunk::setAreaID(int ID) { areaID = ID; }

int MapChunk::getAreaID() { return areaID; }
...
void MapChunk::save(sExtendableArray& lADTFile, int& lCurrentPosition,
					...
					std::vector<ModelInstance>& lModelInstances)
{
	...
	lADTFile.Extend(8 + 0x80);  // This is only the size of the header. More
								// chunks will increase the size.
	SetChunkHeader(lADTFile, lCurrentPosition, 'MCNK', lMCNK_Size);
	lADTFile.GetPointer<MCIN>(lMCIN_Position + 8)
		->mEntries[py * 16 + px]
		.offset = lCurrentPosition;  // check this

	// MCNK data
	lADTFile.Insert(lCurrentPosition + 8, 0x80,
					reinterpret_cast<char*>(&(header)));
	MapChunkHeader* lMCNK_header =
		lADTFile.GetPointer<MapChunkHeader>(lCurrentPosition + 8);
	...
	for (int i = 0; i < lod_texture_map.size(); ++i)
	{
		...
		// it's a uint2 array so we need to write the uint2 in the order they
		// will be on disk, this means writing to the highest bits of the uint8
		// first
		const size_t bit_index((3 - ((i) % 4)) * 2);

		lMCNK_header->low_quality_texture_map[array_index] |=
			((lod_texture_map[i] & 3) << bit_index);
	}
	...
	lADTFile.GetPointer<MapChunkHeader>(lMCNK_Position + 8)->ofsHeight =
		lCurrentPosition - lMCNK_Position;
	...
	if (hasMCCV)
	{
		...
		lADTFile.GetPointer<MapChunkHeader>(lMCNK_Position + 8)->ofsMCCV =
			lCurrentPosition - lMCNK_Position;

		unsigned int* lmccv =
			lADTFile.GetPointer<unsigned int>(lCurrentPosition + 8);
	...
	lADTFile.GetPointer<MapChunkHeader>(lMCNK_Position + 8)->ofsNormal =
		lCurrentPosition - lMCNK_Position;
	...
	// Unknown MCNR bytes
	// These are not in as we have data or something but just to make the files
	// more blizzlike.
	//        {
	lADTFile.Extend(13);
	lCurrentPosition += 13;
	...
	lADTFile.GetPointer<MapChunkHeader>(lMCNK_Position + 8)->ofsLayer =
		lCurrentPosition - lMCNK_Position;
	lADTFile.GetPointer<MapChunkHeader>(lMCNK_Position + 8)->nLayers =
		texture_set->num();
	...
	{
		ENTRY_MCLY* lLayer =
			lADTFile.GetPointer<ENTRY_MCLY>(lCurrentPosition + 8 + 0x10 * j);
	...
	lChunkExtents[1] =
		math::vector_3d(xbase + CHUNKSIZE, 0.0f, zbase + CHUNKSIZE);
	...
	lADTFile.GetPointer<MapChunkHeader>(lMCNK_Position + 8)->ofsRefs =
		lCurrentPosition - lMCNK_Position;
	lADTFile.GetPointer<MapChunkHeader>(lMCNK_Position + 8)->nDoodadRefs =
		lDoodadIDs.size();
	lADTFile.GetPointer<MapChunkHeader>(lMCNK_Position + 8)->nMapObjRefs =
		lObjectIDs.size();
	...
	lID = 0;
	for (std::list<int>::iterator it = lDoodadIDs.begin();
		 it != lDoodadIDs.end(); ++it)
	{
		lReferences[lID] = *it;
		lID++;
	}

	for (std::list<int>::iterator it = lObjectIDs.begin();
		 it != lObjectIDs.end(); ++it)
	{
		lReferences[lID] = *it;
		lID++;
	}
	...
	if (has_shadows())
	{
		...
		lADTFile.GetPointer<MapChunkHeader>(lMCNK_Position + 8)->ofsShadow =
			lCurrentPosition - lMCNK_Position;
		lADTFile.GetPointer<MapChunkHeader>(lMCNK_Position + 8)->sizeShadow =
			0x200;
	...
	lADTFile.GetPointer<MapChunkHeader>(lMCNK_Position + 8)->ofsAlpha =
		lCurrentPosition - lMCNK_Position;
	lADTFile.GetPointer<MapChunkHeader>(lMCNK_Position + 8)->sizeAlpha =
		8 + lMCAL_Size;
	...
	lADTFile.GetPointer<MapChunkHeader>(lMCNK_Position + 8)->ofsSndEmitters =
		lCurrentPosition - lMCNK_Position;
	lADTFile.GetPointer<MapChunkHeader>(lMCNK_Position + 8)->nSndEmitters =
		lMCSE_Size / 0x1C;
	...
	lADTFile.GetPointer<MCIN>(lMCIN_Position + 8)->mEntries[py * 16 + px].size =
		lMCNK_Size + sizeof(sChunkHeader);
}
...
void MapChunk::selectVertex(math::vector_3d const& pos, float radius,
