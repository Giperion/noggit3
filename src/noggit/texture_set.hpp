...
struct tmp_edit_alpha_values
{
	...
	alpha_layer& operator[](std::size_t i) { return map.at(i); }
};
...
class TextureSet
{
   public:
	TextureSet() = delete;
	TextureSet(MapChunkHeader const& header, MPQFile* f, size_t base,
	...
	void replace_texture(scoped_blp_texture_reference const& texture_to_replace,
						 scoped_blp_texture_reference replacement_texture);
	bool paintTexture(float xbase, float zbase, float x, float z, Brush* brush,
					  float strength, float pressure,
					  scoped_blp_texture_reference texture);
	bool replace_texture(float xbase, float zbase, float x, float z,
						 float radius,
