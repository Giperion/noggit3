...
namespace noggit
{
	class world_model_instances_storage
	{
	   public:
		...
		world_model_instances_storage(world_model_instances_storage const&) =
			delete;
		world_model_instances_storage(world_model_instances_storage&&) = delete;
		world_model_instances_storage& operator=(
			world_model_instances_storage const&) = delete;
		world_model_instances_storage& operator=(
			world_model_instances_storage&&) = delete;
		...
		std::optional<selection_type> get_instance(std::uint32_t uid);
		...
		void unload_instance_and_remove_from_selection_if_necessary(
			std::uint32_t uid);
	   ...
	   private:  // private functions aren't thread safe
		...
		std::uint32_t unsafe_add_model_instance_no_world_upd(
			ModelInstance instance);
		std::uint32_t unsafe_add_wmo_instance_no_world_upd(
			WMOInstance instance);
		std::optional<ModelInstance*> unsafe_get_model_instance(
			std::uint32_t uid);
