...
static const double MAPCHUNK_RADIUS =
	47.140452079103168293389624140323;  // sqrt((533.33333/16)^2 +
										// (533.33333/16)^2)
...
struct ENTRY_MCLY
{
	...
	uint32_t flags;
	uint32_t ofsAlpha;
	uint32_t effectID = 0xFFFF;  // default value, see
								 // https://wowdev.wiki/ADT/v18#MCLY_sub-chunk
};
...
struct MH2O_Header
{
	...
	MH2O_Header() : ofsInformation(0), nLayers(0), ofsRenderMask(0) {}
};
...
struct MH2O_Render
{
	// seems to be usable as visibility information (as per
	// https://wowdev.wiki/ADT/v18#MH2O_chunk_.28WotLK.2B.29)
...
struct mclq_tile
{
	...
	// it's technically the 4 first bits set to 1 (0xF) but it's easier to use
	// this way
