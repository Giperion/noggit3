...
namespace opengl
{
	shader::shader(GLenum type, std::string const& source) try
	...
	shader::~shader() { gl.deleteShader(_handle); }
	...
	std::string shader::src_from_qrc(std::string const& shader_alias)
	{
		...
		{
			throw std::logic_error("Could not load " + shader_alias +
								   " from the qrc file");
		}
									 ...
									 std::vector<std::string> const& defines)
	{
		...
		{
			throw std::logic_error("shader " + shader_alias +
								   " has no #version directive");
		}

		// #version is always the first thing in the shader, insert defines
		// after it
		...
		: _handle(gl.createProgram())
	{
		struct scoped_attach
		{
			...
			~scoped_attach() { gl.detachShader(_program, _shader); }
			GLuint _program;
			GLuint _shader;
		};
	...
	program::program(program&& other) : _handle(std::nullopt)
	{
	...
	namespace scoped
	{
		use_program::use_program(program const& p) : _program(p)
		{
		...
		void use_program::attrib(std::string const& name,
								 std::vector<float> const& data)
		{
			GLuint const location(attrib_location(name));
			gl.enableVertexAttribArray(location);
			_enabled_vertex_attrib_arrays.emplace(location);
			gl.vertexAttribPointer(location, 1, GL_FLOAT, GL_FALSE, 0,
								   data.data());
		}
		void use_program::attrib(std::string const& name,
								 std::vector<math::vector_2d> const& data)
		{
			GLuint const location(attrib_location(name));
			gl.enableVertexAttribArray(location);
			_enabled_vertex_attrib_arrays.emplace(location);
			gl.vertexAttribPointer(location, 2, GL_FLOAT, GL_FALSE, 0,
								   data.data());
		}
		void use_program::attrib(std::string const& name,
								 std::vector<math::vector_3d> const& data)
		{
			GLuint const location(attrib_location(name));
			gl.enableVertexAttribArray(location);
			_enabled_vertex_attrib_arrays.emplace(location);
			gl.vertexAttribPointer(location, 3, GL_FLOAT, GL_FALSE, 0,
								   data.data());
		}
		void use_program::attrib(std::string const& name,
								 math::vector_3d const* data)
		{
			GLuint const location(attrib_location(name));
			gl.enableVertexAttribArray(location);
			_enabled_vertex_attrib_arrays.emplace(location);
		...
		void use_program::attrib(std::string const& name,
								 math::matrix_4x4 const* data, GLuint divisor)
		{
			GLuint const location(attrib_location(name));
			math::vector_4d const* vec4_ptr =
				reinterpret_cast<math::vector_4d const*>(data);

			for (GLuint i = 0; i < 4; ++i)
			{
				gl.enableVertexAttribArray(location + i);
				_enabled_vertex_attrib_arrays.emplace(location + i);
				gl.vertexAttribPointer(location + i, 4, GL_FLOAT, GL_FALSE,
									   sizeof(math::matrix_4x4), vec4_ptr + i);
				gl.vertexAttribDivisor(location + i, divisor);
			}
		}
		void use_program::attrib(std::string const& name, GLsizei size,
		...
		void use_program::attrib(std::string const& name, GLuint buffer,
		...
		void use_program::attrib_divisor(std::string const& name,
										 GLuint divisor, GLsizei range)
		{
			GLuint const location(attrib_location(name));
		...
		GLuint use_program::attrib_location(std::string const& name)
		{
			...
			else
			{
				...
				if (loc == -1)
				{
					throw std::invalid_argument("attribute " + name +
												" does not exist in shader\n");
				}
				_attribs[name] = loc;
				return loc;
