...
namespace opengl
{
	struct context
	{
		...
		struct save_current_context
		{
			...
			save_current_context& operator=(save_current_context const&) =
				delete;
		...
		void programString(GLenum target, GLenum format, GLsizei len,
						   GLvoid const* pointer);
		void getProgramiv(GLuint program, GLenum pname, GLint* params);
		void programLocalParameter4f(GLenum, GLuint, GLfloat, GLfloat, GLfloat,
									 GLfloat);
		...
		void bufferData(GLuint buffer, GLsizeiptr size, GLvoid const* data,
						GLenum usage);
		template <GLenum target, typename T>
		void bufferData(GLuint buffer, std::vector<T> const& data,
						GLenum usage);

		void drawElements(GLenum mode, GLuint index_buffer, GLsizei count,
