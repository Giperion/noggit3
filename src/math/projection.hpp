...
namespace math
{
	inline matrix_4x4 perspective(math::degrees fovy, float aspect, float zNear,
								  float zFar)
	{
		// assuming
		...
		// multiply matrix by
		// math::matrix_4x4 frustum ( 2 * nearVal / (right - left), 0.0f, (right
		// + left) / (right - left), 0.0f
		//                          , 0.0f, 2 * nearVal / (top - bottom), (top +
		//                          bottom) / (top - bottom), 0.0f , 0.0f, 0.0f,
	...
	inline matrix_4x4 ortho(float left, float right, float bottom, float top,
							float z_near, float z_far)
	{
		...
		return {v0,  0.f, 0.f, tx, 0.f, v1,  0.f, ty,
				0.f, 0.f, v2,  tz, 0.f, 0.f, 0.f, 1.f};
	}

	inline matrix_4x4 look_at(vector_3d const& eye, vector_3d const& center,
							  vector_3d const& up)
	{
		...
		return {x.x, x.y, x.z, x * -eye, y.x, y.y, y.z, y * -eye,
				z.x, z.y, z.z, z * -eye, 0.f, 0.f, 0.f, 1.f};
	}
}  // namespace math
