...
namespace math
{
	class frustum
	{
		...
		class plane
		{
		   public:
			plane() = default;
			plane(vector_4d const& vec) : _normal(vec.xyz()), _distance(vec.w)
			{
				normalize();
			}
			...
			const vector_3d& normal() const { return _normal; }
	   ...
	   public:
		...
		bool intersects(const vector_3d& v1, const vector_3d& v2) const;
