...
namespace math
{
	...
	bool frustum::intersects(
		const std::vector<vector_3d>& intersect_points) const
	{
		for (auto const& plane : _planes)
		{
			for (auto const& point : intersect_points)
			{
				if (plane.normal() * point > -plane.distance())
				{
					//! \note C does not know how to continue out of two loops
					//! otherwise.
	...
	bool frustum::intersects(const vector_3d& v1, const vector_3d& v2) const
	{
								   ...
								   const float& radius) const
	{
		for (auto const& plane : _planes)
		{
			const float distance(plane.normal() * position + plane.distance());
			if (distance < -radius)
			{
				return false;
			}
