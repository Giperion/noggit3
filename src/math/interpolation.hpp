...
namespace math
{
	namespace interpolation
	{
		...
		static T slerp(const float& percentage, const T& start, const T& end)
		{
			...
			{
				//! \note Don't call linear here, as this will recurse with
				//! quaternions.
				return T(start * (1.0f - percentage) + end * percentage);
			}
			...
			return T(start * cos(a) +
					 T(end - start * dot).normalize() * sin(a));
		}

		template <typename T>
		static T hermite(const float& percentage, const T& start, const T& end,
