...
namespace math
{
	...
	struct vector_3d_base
	{
		...
		vector_3d_base<T>(T x_, T y_, T z_) : x(x_), y(y_), z(z_) {}
		...
		friend vector_3d_base<T> operator*(const T& d,
										   const vector_3d_base<T>& v)
		{
			return v * d;
		}
		...
		inline vector_3d_base<T> operator%(const vector_3d_base<T>& v) const
		{
			return vector_3d_base<T>(y * v.z - z * v.y, z * v.x - x * v.z,
									 x * v.y - y * v.x);
		}
		...
		inline bool is_inside_of(const vector_3d_base<T>& a,
								 const vector_3d_base<T>& b) const
		{
			return a.x < x && b.x > x && a.y < y && b.y > y && a.z < z &&
				   b.z > z;
		}
	...
	template <typename T>
	inline vector_3d_base<T> min(vector_3d_base<T> const& lhs,
								 vector_3d_base<T> const& rhs)
	{
		return {std::min(lhs.x, rhs.x), std::min(lhs.y, rhs.y),
				std::min(lhs.z, rhs.z)};
	}
	template <typename T>
	inline vector_3d_base<T> max(vector_3d_base<T> const& lhs,
								 vector_3d_base<T> const& rhs)
	{
	...
	std::vector<math::vector_3d> apply(Fun&& fun,
